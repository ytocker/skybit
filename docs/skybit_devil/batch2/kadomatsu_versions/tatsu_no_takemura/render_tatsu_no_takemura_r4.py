"""
Round-4 concept renderer (FINAL, scoped) for TATSU-NO-TAKEMURA — the coiling
...
WHAT CHANGED VS ROUND 3 (the lone residual gate; everything else FROZEN):
the reared cut-MAW must WIN at true 32px as the single brightest cream WEDGE —
on the DAY chip especially, where round 3 dimmed out and rounded to a disc. Only
the maw + the foot value were touched:
  • the bright CUT_HI sheen is pushed UP and flooded across nearly the whole oval
    (CUT_HI lifted to 252,248,224) so the head's averaged value at downscale
    out-values even the bright day sky; the gullet stays dark to hold contrast;
  • a near-white CUT_PEAK wedge is pinned to the UPPER-LEFT lip of the slant so
    the diagonal asymmetry survives the downscale instead of rounding to a disc;
  • the base-foot kit is knocked DOWN a value step (softer blessing-glow + a thin
    ink wash) so nothing competes with the head for the brightest cream point.
The 2.5-loop coil + framing + blackout, the coarse 3-step belly-collars, the
slanted gap-cap maw, and the thread-thin teal are all KEPT verbatim from round 3.
...
CUT_HI    = (252, 248, 224)   # lit cut-rim sheen — BRIGHTEST value on the form (R4: pushed up)
CUT_PEAK  = (255, 254, 244)   # R4: near-white sheen PEAK biased onto the upper-LEFT lip so the
                              # slant asymmetry out-values the bright DAY sky at true 32px
...
def slanted_maw(surf, cx, cy, half_w, s, slant_frac=1.0):
    ...
    pygame.draw.ellipse(surf, INK, outer.inflate(int(2.4 * s), int(2.4 * s)))
    # R4: the cut-CREAM ring-wall base, then a near-FULL bright CUT_HI sheen that
    # floods almost the whole oval (only a hairline cream rim survives) so when the
    # 32px downscale averages the form, the dominant value is the BRIGHTEST cream on
    # the sheet — it out-values the bright DAY sky instead of being swallowed by it.
    # The deep gullet below stays the lone dark note (bright lip vs dark gullet =
    # the unmistakable "open cut" read).
    pygame.draw.ellipse(surf, CUT_CREAM, outer)
    sheen = pygame.Rect(cx - ew, cy - eh + slant, int(ew * 1.86), int(eh * 1.86))
    pygame.draw.ellipse(surf, CUT_HI, sheen)
    pygame.draw.ellipse(surf, CUT_CREAM,
                        sheen.inflate(-int(ew * 0.10), -int(eh * 0.10)))
    # R4: a near-white CUT_PEAK WEDGE pinned to the UPPER-LEFT lip of the slant — a
    # filled triangular sheen running along the high (upper-left) cut edge. Biasing
    # the brightest value off-centre toward the high lip is what keeps the asymmetry
    # alive through the downscale so the head reads as a slanted WEDGE, never a
    # symmetric disc/eye.
    peak = [(cx - ew, cy + slant),                               # high lip (upper-left)
            (cx - int(ew * 0.10), cy - eh + slant + int(eh * 0.22)),
            (cx + int(ew * 0.30), cy - slant + int(eh * 0.30)),
            (cx - int(ew * 0.30), cy - slant + int(eh * 0.66))]
    pygame.draw.polygon(surf, CUT_PEAK, peak)
    ...
    # a WIDER bright lip riding the ring-wall around the gullet — the near rim that
    # frames the dark cavity. R4: thickened + pushed to CUT_HI so the lit ring-wall
    # stays the brightest band right up against the dark gullet (steep lip->cavity
    # value jump = the "open cut" tell at 32px).
    lip = pygame.Rect(cav.left - int(iw * 0.3), cav.top - int(ih * 0.26),
                      int(iw * 2.6), int(ih * 2.1))
    pygame.draw.arc(surf, CUT_HI, lip, math.radians(186), math.radians(360),
                    max(3, int(3.6 * s)))
...
def draw_tatsu(surf, cx, cy, s):
    ...
    #
    # R4: the whole foot kit is rendered to its OWN layer and knocked down a value
    # step (a thin dark wash + a softer blessing-glow) before compositing, so at
    # true 32px nothing at the base out-values the reared maw — the cream maw must
    # be the single brightest cream point, not tied by the foot's glow/plum/straw.
    fx, fy = pts[-1]
    foot = pygame.Surface(surf.get_size(), pygame.SRCALPHA)
    straw_collar(foot, int(fx), int(fy + 8 * s), int(24 * s), int(17 * s), s)
    bound_face(foot, int(fx), int(fy - int(4 * s)), int(8 * s), s, lit=False)
    plum_blossom(foot, int(fx - 26 * s), int(fy + 14 * s), int(7 * s), s)
    plum_blossom(foot, int(fx + 28 * s), int(fy + 16 * s), int(6 * s), s)
    plum_blossom(foot, int(fx + 6 * s), int(fy + 24 * s), int(5 * s), s)
    pine_fan(foot, (int(fx - 12 * s), int(fy + 18 * s)), math.radians(210),
             math.radians(56), 5, 26 * s, s, sign=-1)
    pine_fan(foot, (int(fx + 12 * s), int(fy + 18 * s)), math.radians(-30),
             math.radians(56), 5, 26 * s, s, sign=1)
    # dim the foot a step: a translucent ink wash masked to the foot's own alpha
    fmask = pygame.mask.from_surface(foot)
    wash = fmask.to_surface(setcolor=(*INK, 70), unsetcolor=(0, 0, 0, 0))
    foot.blit(wash, (0, 0))
    surf.blit(foot, (0, 0))
...
def main():
    ...
    sheet.blit(font_sm.render(
        ...
        "(coarse node-collars TILED = belly-scales) · reared cream OPEN SLANTED cut-MAW · jade-teal filament · round 4 (FINAL)",
    ...
    sheet.blit(font_sm.render(
        ...
        "1px grown outline.  R4 (FINAL, scoped): maw now the SINGLE brightest cream WEDGE at 32px — near-full CUT_HI sheen + near-white upper-LEFT lip-PEAK vs dark gullet · foot value knocked down a step.",
