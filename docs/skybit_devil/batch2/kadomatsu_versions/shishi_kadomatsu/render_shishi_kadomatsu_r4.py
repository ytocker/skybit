"""
Round-4 (FINAL, tightly scoped) concept renderer for SHISHI-KADOMATSU — the
bound-culm guardian lion (Kadomatsu brood, concept #5). Forked VERBATIM from
render_shishi_kadomatsu_r3.py; the ELEVATED-REALISM pipeline (SS=6 -> smoothscale;
ZERO gradients, hard stepped value bands only; ink keyline (28,22,30) + 1px grown
outline; radial glow for ACCENTS only) is unchanged.

Round 3 passed 3 of 4 blackout gates + the full 32px color creature read. This
pass lands ONLY the one residual and touches nothing else that passed:

  1. CROWN SCALLOP AS A SILHOUETTE FEATURE (the one unmet gate). Round 3 set a
     1px INK GAP between outer discs, but in pure black those 1px ink seams
     compress away and the crown still read as a smooth dome cap. Round 4 carves
     TRUE 2px (at final scale) SKY-COLORED NEGATIVE-SPACE gaps THROUGH the
     crown's outer contour — the same negative-space technique that won the four
     legs — between the topmost outer discs, so the BLACK silhouette contour
     itself goes bumpy/toothed and the pure-blackout crown reads as a NOTCHED
     ring of cut-tips, not a rounded dome. (`crown_notch()` carves after the
     discs are drawn, honoring the chip's `sky_slot` color / transparent erase.)
  2. The CENTER skirt slot is widened ~1px so the four black leg-posts read as
     evenly spaced rather than 2+2 fused.
  3. The two stray bright mid-body pixels (a face highlight + a disc highlight)
     that floated free of the silhouette at 1x are killed.

KEEP EXACTLY (these passed — untouched): the four-post negative-space skirt, the
low-wide komainu raft proportion, the killed center face-window, the cut-disc
crown being the brightest value, the green node-ringed leg-posts + straw anklets
+ plum/straw base, the indigo brow-cord, the 32px day+night creature read.
...
def bound_face(surf, cx, cy, r, s, lit=True):
    ...
    ROUND-4: the bright inner FACE inset is pulled IN a touch so its top-cream
    edge no longer pokes a free bright pixel above the face mass at 1x (one of
    the two stray-pixel kills)."""
    if lit:
    ...
    # WHY inset 0.84 (was 0.7): keeps the bright inner cream off the very top of
    # the face plane so no lone bright pixel floats free above the head at 1x.
...
# ══ SHISHI construction — round 4 (LAND THE SCALLOPED-CONTOUR BLACKOUT) ════════
...
# WHY a module-level record of where the TOPMOST outer crown discs land: the
# blackout-scallop notches must be carved AFTER the discs are painted (negative
# space, the proven survivor), so mane_ring() stashes the disc tip geometry here
# and draw_shishi() carves the inter-disc gaps with the chip's sky color (or a
# transparent erase for the pure mask). Cleared at the start of every figure.
_CROWN_DISCS = []
...
def mane_ring(surf, cx, cy, R, s, n=11, blue_cord=True):
    ...
    ROUND-4 blackout fix: the 1px ink seams from r3 compressed away in pure
    black, so the top contour still domed. mane_ring() now RECORDS the topmost
    outer-disc tip positions into _CROWN_DISCS; draw_shishi() then carves TRUE
    2px SKY-COLORED negative-space gaps between adjacent top discs THROUGH the
    crown contour, so the BLACK silhouette itself goes toothed — a notched ring
    of cut-tips, never a smooth dome cap.
    ...
    inner = R * 0.50
    # WHY the back fills are pulled IN to R*0.78: the old CUT_D ring out at R*1.04
    ...
    for (tx, ty, ca, sa, k) in disc_pts:
        ...
        diagonal_cut(surf, int(tx), int(ty), hw, s, lean=0.80, dominance=dom,
                     bright_lip=True)
    # === RECORD the topmost outer-disc tips so the negative-space scallop gaps
    # can be carved THROUGH the contour after the whole figure is laid down. We
    # take the discs whose tip sits in the upper arc of the crown (sa < ~-0.15)
    # and stash the MIDPOINT between each adjacent pair plus the local outward
    # normal — that midpoint is exactly the seam to cut sky through.
    _CROWN_DISCS.clear()
    upper = [(tx, ty, ca, sa) for (tx, ty, ca, sa, k) in disc_pts if sa < -0.12]
    # order them around the arc (left -> right across the top) by angle
    upper.sort(key=lambda d: math.atan2(d[3], d[2]))
    for i in range(len(upper) - 1):
        ax, ay, aca, asa = upper[i]
        bx, by, bca, bsa = upper[i + 1]
        mx, my = (ax + bx) * 0.5, (ay + by) * 0.5
        # outward normal at the seam = average of the two disc radial directions
        nca, nsa = (aca + bca), (asa + bsa)
        nl = math.hypot(nca, nsa) or 1.0
        nca, nsa = nca / nl, nsa / nl
        _CROWN_DISCS.append((mx, my, nca, nsa))
...
def crown_notch(surf, R, s, sky_slot):
    """ROUND-4 — the one unmet gate. Carve TRUE negative-space gaps between the
    topmost outer crown discs (recorded by mane_ring into _CROWN_DISCS) so the
    BLACK silhouette CONTOUR goes toothed in pure blackout — the same technique
    that won the four legs. Each seam is a short SKY-COLORED slot driven OUTWARD
    along the local crown normal from just inside the disc tips out past the
    outer lip, so it cuts clean THROUGH the crown's outer edge.
    ...
    is ~2px wide at the final 1x scale after the SS smoothscale."""
    # WHY ~3.2*s wide on the supersampled surface: it lands at ~2px true after
    # the /SS smoothscale of the chip — wide enough that pure black cannot
    # compress the seam shut (r3's 1px ink seam did), narrow enough that the
    # cut-tips on either side still read as separate teeth.
    half = max(2, int(1.6 * s))
    erase = (sky_slot is None)
    for (mx, my, nca, nsa) in _CROWN_DISCS:
        # carve from just inside the disc band outward past the bright lip
        x0 = mx + nca * (R * -0.20)
        y0 = my + nsa * (R * -0.20)
        x1 = mx + nca * (R * 0.30)
        y1 = my + nsa * (R * 0.30)
        ...
        if erase:
            pygame.draw.polygon(surf, (0, 0, 0, 0), quad)
            # SRCALPHA polygon fill does not erase to transparent reliably across
            # blends; punch it with an explicit clear via per-pixel min-alpha
            # rect fallback is unnecessary — fill with BLEND to zero alpha:
...
def draw_shishi(surf, cx, cy, s, sky_slot=None):
    ...
    ROUND-4 (scoped) — lands the one residual blackout gate, keeps all else:
      ...
      · the crown TOP CONTOUR is now CARVED toothed via crown_notch(): true 2px
        sky-colored negative-space gaps between the top discs, so the pure
        blackout reads a notched ring of cut-tips, not a smooth dome cap.
    ...
    # split the bottom into FOUR standing black posts. ROUND-4: the CENTER slot
    # is widened ~1px (true) so the four posts read EVENLY spaced rather than the
    # inner pair fusing into a 2+2 reading.
    slot_top = body_bot + int(2 * s)
    slot_bot = ground_y - int(6 * s)            # stop above the anklets/paws
    slot_w_outer = int(8 * s)
    slot_w_center = int(15 * s)                 # widened (was 13*s) — even spacing
    erase = (sky_slot is None)
    slot_specs = [
    ...
    # upper crown (discs + brow-cord); negative space is what survives the mask.
...
def main():
    ...
    sheet.blit(font_sm.render(
        "bound-culm GUARDIAN LION  ·  R4 FINAL (scoped) — CARVED scalloped/toothed crown contour (true 2px sky notches between top discs) · "
        "widened center skirt slot (even 4 posts) · stray bright pixels killed.  All r3 passes KEPT.",
    ...
    sheet.blit(font_sm.render("LOW FORWARD ring-crown head = cream cut-discs; TOP CONTOUR carved toothed; indigo brow-cord.", True, LABEL_DIM), (14, 622))
    ...
    sheet.blit(font_sm.render("32px day — slots + crown notches", True, LABEL), (panel_x + 20, day_y + 156))
    ...
    # of cut-tips — NOT a domed bell. The slots AND the crown notches are erased
    # (transparent), so the mask shows them as genuine negative space in the
    # CONTOUR itself.
    def silhouette32():
        big = pygame.Surface((96 * SS, 96 * SS), pygame.SRCALPHA)
    ...
    sheet.blit(font_sm.render("a LOW WIDE body + a CARVED TOOTHED crown", True, LABEL_DIM), (sx, sil_y + 132))
    sheet.blit(font_sm.render("CONTOUR (sky notches) — NOT a smooth dome.", True, LABEL_DIM), (sx, sil_y + 148))
    ...
    sheet.blit(font_sm.render(
        ...
        "1px grown outline · radial glow ACCENTS only.  R4 FINAL (scoped): CARVED toothed crown contour · widened center slot · stray pixels killed.",
