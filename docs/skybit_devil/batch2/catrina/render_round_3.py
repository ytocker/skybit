"""CATRINA — round 3 review sheet (Section 2 Skeletons, locked15 brief).

Round-2 critique gate (AD, ITERATE, final): the 4-plume bouquet cleared the
Mariachi sibling-drift — KEEP it. The ONLY remaining defect: the gown pinched
so hard under the brim that the silhouette nearly severed into a floating hat +
a detached body. At gap scale a colorblind player must read ONE figure.

Round-3 single fix — re-anchor the gown so the silhouette is ONE continuous
mass from hat to hem:
  1. The cinch (narrowest point) drops DOWN to the true waist (mid-gown). The
     shoulder/collar line directly under the brim is broadened so an unbroken
     visual column carries the hat into the body — no daylight gap, no
     sub-pole-width necking.
  2. A flared couture COLLAR (a triad-lit shawl fan at the shoulders) is added
     as the WIDEST point of the upper body — period-correct Catrina that also
     physically bridges skull/brim to gown so the join can't read severed.
  3. The bone neck is shortened + stays behind the collar; the gown top now
     meets the collar at full shoulder width rather than a sliver.
  4. Hat (brim+plume) re-verified still dominant over the widened shoulders —
     the body must not bulk up enough to rival the brim.

KEPT from round 2 (per critique #5): the 4-plume bouquet, the upswept
asymmetric brim, the side hatband marigolds, the simplified face tells, and the
ribbed parasol canopy + pointed ferrule prop/pillar mirror — left untouched.
...
Run headless (SDL_VIDEODRIVER=dummy). Writes round_3.png beside this script.
"""
import os
import math
...
def _build_catrina_big():
    ...
    # ----- GOWN: hourglass body (teal), drawn first (behind). The cinch now
    # sits at the TRUE WAIST (mid-gown), and the gown TOP meets the collar at
    # broad shoulder width so the silhouette never necks to a sliver under the
    # brim — hat and body stay one continuous mass. -----
    gown = new_surf(w, h)
    # shoulder line tucks right under the skull/collar; widest at the shoulders,
    # cinched at the waist below mid-gown, then flaring to the hem.
    shoulder_y = 0.575       # full-width top, directly beneath the collar
    waist_y = 0.785          # the hourglass cinch — DROPPED to the true waist
    hem_y = 0.985
    sh_hw = 0.150            # half-width at the shoulders (was 0.115 — broadened)
    wa_hw = 0.092            # half-width at the cinched waist
    hem_hw = 0.230
    gpoly = [
        P(0.5 - sh_hw, shoulder_y),
        P(0.5 - sh_hw * 0.86, (shoulder_y + waist_y) * 0.5),
        P(0.5 - wa_hw, waist_y),
        P(0.5 - hem_hw, hem_y),
        P(0.5 + hem_hw, hem_y),
        P(0.5 + wa_hw, waist_y),
    ...
    for fxp in (0.40, 0.46, 0.54, 0.60):
        ...
        pygame.draw.line(gown, TEAL_SH, (x, int(waist_y * h)),
                         (int((0.5 + (fxp - 0.5) * 2.1) * w), int(hem_y * h)), max(2, SS))
    pygame.draw.polygon(gown, GOLD, [
        P(0.5 - hem_hw, hem_y), P(0.5 + hem_hw, hem_y),
        P(0.5 + hem_hw - 0.015, hem_y - 0.045), P(0.5 - hem_hw + 0.015, hem_y - 0.045)])
    # waist sash + bloom mark the cinch as the deliberate hourglass focal
    pygame.draw.line(gown, GOLD, P(0.5 - wa_hw, waist_y), P(0.5 + wa_hw, waist_y), int(SS * 1.8))
    draw_marigold(gown, cx, waist_y * h, w * 0.052, base=PINK, shade=PINK_SH,
    ...
    # ----- SHORT bone NECK — kept slim but stubby so it can't read as the
    # severing pinch; the collar (below) overlaps its base. -----
    neck = new_surf(w, h)
    pygame.draw.rect(neck, BONE, (int(cx - w * 0.05), int(0.520 * h),
                                  int(w * 0.10), int(0.065 * h)))
    pygame.draw.line(neck, BONE_SH, (int(cx - w * 0.05), int(0.55 * h)),
                     (int(cx + w * 0.05), int(0.55 * h)), max(2, SS // 2))
    core_shade(neck, BONE_SH, 110)
    triad_sheen(neck, top_a=110, bot_a=40)
    s.blit(neck, (0, 0))

    # ----- flared couture COLLAR — a triad-lit shawl fan at the shoulders, the
    # WIDEST point of the upper body. It physically bridges skull/brim to gown
    # so the hat-to-body join is one unbroken silhouette, never severed. -----
    collar = new_surf(w, h)
    col_cy = 0.565
    col_hw = 0.205          # widest upper-body mass (still well under brim_w)
    # a scalloped shawl fan: lobes between points give a lace couture edge
    lobes = 7
    cpts = [P(0.5 - 0.05, 0.515)]
    for i in range(lobes * 2 + 1):
        t = i / (lobes * 2)
        ang = math.pi + math.pi * t
        rr = col_hw * (1.0 if i % 2 == 0 else 0.84)
        cpts.append((cx + math.cos(ang) * rr * w,
                     col_cy * h + abs(math.sin(ang)) * 0.045 * h
                     + (0.012 * h if i % 2 == 0 else 0.0)))
    cpts.append(P(0.5 + 0.05, 0.515))
    pygame.draw.polygon(collar, TEAL, cpts)
    # gold collar trim arc + a hot-pink brooch at the throat (couture focal)
    pygame.draw.arc(collar, GOLD, (int(cx - col_hw * w), int((col_cy - 0.05) * h),
                                   int(col_hw * 2 * w), int(0.12 * h)),
                    math.radians(182), math.radians(358), max(2, SS // 2 + 1))
    pygame.draw.circle(collar, PINK, (int(cx), int(0.545 * h)), int(w * 0.034))
    pygame.draw.circle(collar, _lite(PINK, 0.5),
                       (int(cx - w * 0.012), int(0.541 * h)), int(w * 0.014))
    core_shade(collar, TEAL_SH, 120)
    triad_sheen(collar, top_a=100, bot_a=34, ell=(0.16, 0.40, 0.40, 0.36))
    s.blit(collar, (0, 0))

    # ----- gloved bone HANDS holding a folding FAN -----
    hands = new_surf(w, h)
    fan_cx, fan_cy = cx + w * 0.13, h * 0.73
    r1 = w * 0.13
    ...
    pygame.draw.circle(hands, TEAL, (int(cx - w * 0.11), int(h * 0.735)), int(w * 0.03))
...
# header
sheet.blit(ftitle.render("CATRINA  —  round 3 (final)", True, TITLE), (28, 18))
sheet.blit(fbody.render(
    "Gown re-anchored: cinch dropped to the TRUE WAIST + a flared couture collar widens the "
    "shoulders, so hat+body are ONE continuous silhouette (no severed float).",
    True, SUB), (28, 54))
...
sheet.blit(ftiny.render("waist cinch + shawl collar bridge hat->body", True, (235, 242, 252)),
...
out_path = os.path.join(_OUT_DIR, "round_3.png")
