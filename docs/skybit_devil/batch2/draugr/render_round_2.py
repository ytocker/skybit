"""Round-2 concept sheet — DRAUGR, the barrow-mound viking bone-warrior.

Batch-2 SKELETON boss concept. Round-2 resolves the AD critique on round-1:
...
Run:  SDL_VIDEODRIVER=dummy python docs/skybit_devil/batch2/draugr/render_round_2.py
Out:  docs/skybit_devil/batch2/draugr/round_2.png
...
def _rime_cluster(ss, S, rnd, cx, cy, spread, n):
...
def build_draugr(size):
    ...
    pygame.draw.rect(ss, PAL["rust_hi"], (fx - ng_w // 2, ng[1], max(1, _SS), ng[3]))
    ...
    # frost rime — ONE restrained crystalline cluster on the bone CROWN / brow
    # only (AD fix #2). White-diamond facets are the non-hue cold tell.
    _rime_cluster(ss, S, rnd, fx - int(fw * 0.3), fy - int(fh * 0.7),
                  int(fw * 0.7), n=5)
...
def main():
    ...
    sheet.blit(f_title.render(
        "DRAUGR — barrow-mound viking bone-warrior  (Batch 2 / Skeletons, round 2)",
        True, (236, 242, 248)), (24, y))
    y += 34
    sheet.blit(f_small.render(
        "R2: helm shrunk to a LOW rust-iron brow-cap so the frost-bone skull "
        "dominates; ice-cyan disciplined to the jaw glow + ONE crown rime "
        "cluster; stubby thick horns; dim mail studs; moss up onto shoulders.",
