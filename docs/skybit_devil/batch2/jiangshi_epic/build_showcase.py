"""Assemble the Jiangshi-epic boss-set showcase.

Compositing-only: each source sheet's hero-creature region is cropped with a
per-sheet measured window (the top-left "hero" panel), then
fit-inside-scaled so every creature reads whole and undistorted at the same
tile size. The shipped Jiangshi anchors the row as a set-apart REFERENCE so a
reviewer can read each inspired-by boss against the established silhouette.

Vetala's sheet is wider (1280px) and its hero hangs INVERTED (head at bottom);
its window is measured to keep the whole upside-down figure intact.
"""
...
# Neutral mid-grey keeps the coloured bosses honest regardless of their own
...
# clear of each sheet's "hero ..." caption so only the creature carries in.
TILES = [
    {
        "file": "docs/skybit_devil/batch2/jiangshi/round_3.png",
        "crop": (62, 165, 228, 300),
        "name": "Jiangshi",
        "facet": "shipped (hopping corpse)",
        "reference": True,
    },
    {
        ...
        "crop": (90, 180, 215, 320),
        "name": "Citipati",
        "facet": "dancing skeleton-lord + khatvanga",
    },
    {
        "file": "docs/skybit_devil/batch2/jiangshi_epic/zhenmushou/round_3.png",
        "crop": (88, 90, 210, 375),
        "name": "Zhenmushou",
        "facet": "sancai tomb-beast + spine stela",
    },
    {
        "file": "docs/skybit_devil/batch2/jiangshi_epic/xinniang/round_2.png",
        "crop": (12, 128, 340, 375),
        "name": "Xinniang",
        "facet": "vermilion corpse-bride + dowry pole",
    },
    {
        "file": "docs/skybit_devil/batch2/jiangshi_epic/jangseung/round_2.png",
        "crop": (88, 70, 150, 440),
        "name": "Jangseung",
        "facet": "carved totem-post (creature=pillar)",
    },
    {
        "file": "docs/skybit_devil/batch2/jiangshi_epic/vetala/round_2.png",
        "crop": (85, 110, 215, 400),
        "name": "Vetala",
        "facet": "inverted bat-hung revenant + branch",
    },
]
...
def main():
    ...
    title = f_title.render("Jiangshi-epic — 5 inspired-by bosses", True, TITLE_COL)
