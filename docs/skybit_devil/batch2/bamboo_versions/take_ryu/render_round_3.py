"""
Round-3 (FINAL) concept renderer for TAKE-RYU — the grove-coiling bamboo dragon
...
WHY R3 vs R2 (AD verdict ITERATE — binding punch list; the head construction is
LOCKED — AD signed off on it, so the hero head internal anatomy is untouched).
R2's gate failure was the BLACKED-OUT 32px chip: it read as a gently wavy STACK,
not a snake; the head tip competed with a fin tip; the skull-cap thinned to a
wedge. R3 fixes exactly those, tuned in the blackout chip FIRST:
1. S-COIL amplitude pushed ~50% deeper in the OUTER silhouette. The sway plan
   now drives the body's LEFT edge out HARD on one node and the RIGHT edge out
   HARD two nodes down, so the outline carries at least TWO clear concave bites
   on ALTERNATING sides — a real snake outline at blackout, not a wavy column.
2. Snout rotated ~18 deg toward forward-DOWN so the head JUTS HORIZONTALLY out
   from the body mass (was up-and-back). In blackout it now reads as a head
   pointing AWAY from the column, clearly distinct from the leaf-fin tips.
3. Gap-edge skull-cap beefed ~20% (bigger head_r) and carries one stag-horn nub
   + the snout-jut into the cap silhouette, so the cap reads as "head," not an
   angled cut.
4. Leaf-fins re-shaped to LOW, BROAD, one-sided flares (raked DOWN-and-out, wide
   base, blunter tips) so no fin tip competes with the horns/snout for "which
   point is the head" at 32px.

KEEP LOCKED (AD said these pass — NOT reopened): the entire hero head
construction (snout, jaw, stag-horns, lidded almond gold eye, low-trailing
lacquer-red beaded whiskers) — only its ORIENTATION rotates per note 2, the
internal anatomy is unchanged; the node-segment repeat shaft + red-sheath/gold-
collar ring treatment; the jade (86,170,108)-vs-lacquer-red focal relationship
and the day/night value structure; the fat-under-head taper. No Kaguya
convergence — saturated jade held.
...
def leaf_fin(surf, root, ang, length, width, s):
    """One bamboo leaf-fin: a LOW, BROAD one-sided blade-flare swept along `ang`,
    triad-lit (dark-core lower half, flat jade, pale mint leading edge + a central
    rib). WHY leaf-fins off the joints (HARD-staggered, not paired): the brief
    pins asymmetric leaf-fin flares so the coil reads ALIVE and never a straight
    column — a big flare on the OUTSIDE of one swing, none at the next.

    WHY broad + blunt (R3): R2's fins were narrow lances whose sharp tips
    competed with the horns/snout for "which point is the head" at 32px. R3 makes
    them WIDE at the belly and BLUNT at the tip (the tip is a short two-point cap,
    not a needle), raked DOWN-and-out by the call sites, so a fin never reads as
    the head spike."""
    ca, sa = math.cos(ang), math.sin(ang)
    px, py = -sa, ca
    hw = width * 0.5
    # blunt the tip: pull the apex back and give it a short flat cap so the
    # silhouette terminates in a broad lobe, not a needle point.
    tipc = (root[0] + ca * length, root[1] + sa * length)
    tip_a = (tipc[0] + px * hw * 0.34, tipc[1] + py * hw * 0.34)
    tip_b = (tipc[0] - px * hw * 0.30, tipc[1] - py * hw * 0.30)
    belly = 0.46 * length
    bx = (root[0] + ca * belly, root[1] + sa * belly)
    tip = tip_a            # leading-edge anchor for the rib/mint strokes
    leaf = [
        (root[0] + px * hw * 0.40, root[1] + py * hw * 0.40),
        (bx[0]   + px * hw * 1.18, bx[1]   + py * hw * 1.18),   # broad belly
        tip_a,
        tip_b,
        (bx[0]   - px * hw * 0.95, bx[1]   - py * hw * 0.95),
        (root[0] - px * hw * 0.40, root[1] - py * hw * 0.40),
    ]
...
def dragon_head(surf, cx, cy, r, s, lit=True, face_dir=-1, tilt_deg=0.0):
    ...
    WHY `tilt_deg` (R3): the LOCKED anatomy must not be re-drawn point-by-point —
    so to rotate the snout forward-DOWN (the head must JUT HORIZONTALLY out, not
    up-and-back) the whole head is rendered onto a temp surface in its canonical
    pose, then the SURFACE is rotated about the head centre. Every internal line
    is byte-identical to the signed-off pose; only orientation changes. Positive
    tilt_deg drops the snout (forward-down) for a face_dir=-1 head."""
    if tilt_deg:
        # render canonical, then rotate the surface so the locked anatomy is
        # untouched and only the head's ORIENTATION changes (note 2).
        pad = int(r * 3.4)
        tmp = pygame.Surface((pad * 2, pad * 2), pygame.SRCALPHA)
        dragon_head(tmp, pad, pad, r, s, lit=lit, face_dir=face_dir, tilt_deg=0.0)
        # a left-facing snout drops by rotating CW (negative pygame angle)
        rot = pygame.transform.rotate(tmp, -face_dir * tilt_deg)
...
def draw_take_ryu(surf, cx, cy, s):
    ...
    WHY R3 deepens the coil (the AD gate): R2's swing was real but SHALLOW — at
    blackout it read as a gently wavy STACK. R3 pushes the swing amplitude ~50%
    deeper AND multiplies it by the taper so the WIDE upper nodes swing FARTHEST,
    driving the body's LEFT edge out hard on one node and the RIGHT edge out hard
    two nodes down — at least TWO clear concave BITES on ALTERNATING sides. The
    body still TAPERS (fat under the head, narrowing to the rooted base) so the
    blacked-out chip reads as a serpent, not a bead column. Drawn root->up->head."""

    seg_h = 25.0 * s
    seg_w0 = 34.0 * s      # widest segment (top, just under the head)
    n = 6
    # x-offsets carve a REAL left-right S. WHY this plan (bottom->top): node 1
    # swings HARD left, node 3 swings HARD right, node 4 stays right of axis —
    # that left-then-right pair is the two alternating concave bites the blackout
    # chip needs. Amplitude pushed ~50% past R2's via SWAY_K below.
    sway = (0.10, -1.20, -0.10, 1.30, 0.70, -0.35)   # bottom -> top
    SWAY_K = 0.92          # R2 used 0.62 — ~50% deeper swing into the silhouette
    ...
    for i in range(n):
        ...
        # wider (upper) nodes swing farther so the fat body carries the bites
        seg_cx = cx + int(sway[i] * seg_w0 * SWAY_K * taper[i])
    ...
    # HARD-staggered leaf-fins (behind the culm bulges). WHY raked DOWN-and-out
    # (R3): a fin that flares low and broad can never be mistaken for the head
    # spike at 32px. Both blades on a joint rake below horizontal (positive y),
    # wide and blunt, on the OUTSIDE of that node's swing.
    for (scx, scy, i, sw) in pts:
        ...
        # ~32deg below horizontal, out to the side — a low broad flare
        ang = math.radians(32 if side > 0 else 148)
        leaf_fin(surf, (scx + side * int(sw * 0.48), scy + int(seg_h * 0.02)),
                 ang, (38 * mag) * s, (19 * mag) * s, s)
        # a clustered second blade lower on the SAME joint (one-sided burst)
        leaf_fin(surf, (scx + side * int(sw * 0.46), scy + int(seg_h * 0.30)),
                 math.radians(58 if side > 0 else 122), (26 * mag) * s,
                 (13 * mag) * s, s)
    ...
    # head crowns the top segment, snout facing LEFT into motion. WHY pushed
    # further LEFT + tilted forward-DOWN (R3 note 2): the head must JUT
    # HORIZONTALLY off the body mass and read as a head pointing AWAY from the
    # column at blackout — not perched up-and-back atop the stack.
    neck_cx, neck_cy, _, neck_w = pts[-1]
    head_r = int(30 * s)
    head_cx = neck_cx - int(head_r * 0.34)
    head_cy = neck_cy - int(seg_h * 0.5) - int(head_r * 0.52)
...
def draw_pillar(surf, cx, top, bot, s, cap="bottom"):
    ...
    # WHY head_r beefed ~20% (R3 note 3): R2's cap thinned to a wedge + whisker
    # lines at cap scale. A bigger cranial mass + the forward snout-jut (tilt)
    # carries a real "head" silhouette (one stag-horn nub + the snout) into the
    # cap, so it reads as HEAD, not an angled cut.
    head_r = int(26 * s)
    if cap == "bottom":
        ...
        dragon_head(tmp, cx, surf.get_height() - cap_cy, head_r, s,
                    lit=True, face_dir=-1, tilt_deg=18.0)
...
def main():
    ...
    sheet.blit(font_sm.render(
        "grove-coiling bamboo dragon  ·  CRANIAL forward-DOWN snout HEAD finial (faces left) · DEEP left-right S-COIL · "
        "LOW broad leaf-fins · low trailing red whiskers · round 3 (FINAL)",
    ...
    sheet.blit(font_sm.render("CRANIAL wedge-skull head TILTED forward-DOWN so the snout JUTS horizontally off", True, LABEL_DIM), (14, 590))
    sheet.blit(font_sm.render("the body mass (locked anatomy, rotated orientation) crowning a DEEP left-right", True, LABEL_DIM), (14, 606))
    sheet.blit(font_sm.render("S-COIL — two alternating concave bites, LOW broad fins, tapered body.", True, LABEL_DIM), (14, 622))
