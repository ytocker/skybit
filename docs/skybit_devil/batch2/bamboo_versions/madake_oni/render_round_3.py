...
=== ROUND-3 REVISIONS (final pass; AD verdict ITERATE on round 2) ======
Round 2's pose + ember + pillar all landed (LOCKED -- untouched). The fixes are
the two GATES + the soft-shading regression, in the AD's priority order:

1. GATE -- ONE gold glint SURVIVES 32px. Round 2's gold cracks vanished entirely
   on both 32px chips (body read as a uniform straw blob). Now ONE dominant CHEST
   facet is a chunky 2px-stepped HERO chip (gold_chip) sized in absolute px so it
   stays a visible light chip after downscale -- a hard stepped facet, not a
   hairline. The other cracks stay fine grain detail. Confirmed present on the
   DAY chip. This is the locked "HARD METALLIC, survives small" foil to Kaguya.
2. GATE -- DE-FACE the root-ball. Removed the gold CROSS-crack from the root-ball
   (it built a muzzle) and dropped the two symmetric collar studs that read as
   eyes (the collar now uses an ASYMMETRIC stud cadence). The knob is now made
   NOTICEABLY BIGGER + lumpier than the head: irregular gnarl bumps break its
   outline so it is not the head's polygon -- it reads as KNOTTED WOOD, not a
   face. Confirmed the night chip no longer scans two heads.
3. REGRESSION -- body shading recut to FLAT HARD STEPS. Deleted the soft diagonal
   streak-gradients on torso/thighs/upper arm (airbrushed banding, a house-style
   violation). Each form now has 2-3 FLAT value planes + ONE hard top-left sheen
   edge, matching the crisp step quality on the horns.
4. Brow-bar nudged darker + squarer (separates head from club at 32px) without
   over-fixing -- the chibi charm reads well and stays.

LOCKED (AD: the concept's spine -- untouched): diagonal kanabo pose + blackout
club-lobe read; single cyclopean ember in its dark socket; pillar node-band/stud
cadence + tileable gap-cap root-ball finial; warm desaturated straw lane; warm
husk-bone horn tips.
"""
import math
import os
...
STRAW_M   = (184, 146,  78)   # mid plane -- the FLAT step between base + shade
...
def flat_form(surf, outline_pts, base, lit_pts, sheen=None, sheen_pts=None,
              shade=None, ow=2):
    """A form built from FLAT discrete value PLANES + one HARD top-left sheen
    edge -- NO lerp blends. WHY (AD regression fix): round 2's torso/thigh/arm
    used lerped diagonal slivers that smoothscale antialiased into soft airbrush
    banding (a house-style violation). Here the whole silhouette fills with the
    SHADE plane, a crisp lit polygon carves the BASE plane off it, and a thin
    SHEEN wedge sits on the light side -- every edge a SOLID palette step, the
    same hard quality the horns read with. Painted dark->light so the lit edge
    sits on top."""
    pygame.draw.polygon(surf, INK, outline_pts)
    pygame.draw.polygon(surf, shade if shade else base, outline_pts)  # shade plane
    pygame.draw.polygon(surf, base, lit_pts)                          # lit base plane
    if sheen_pts and sheen:
        pygame.draw.polygon(surf, sheen, sheen_pts)                   # hard sheen edge
    pygame.draw.polygon(surf, INK, outline_pts, ow)
...
# -- the ONE dominant gold HERO chip that must survive the 32px downscale -------
def gold_chip(surf, x, y, w_units, ss, ang=math.radians(96)):
    """A single chunky 2-tone STEPPED gold facet on the chest -- the one glint
    pinned to survive at 32px. WHY sized in absolute px (not figure-units): the
    AD GATE is that round 2's hairline cracks vanished on the chip, so this facet
    is sized so its bright core is at least ~2 device px AFTER the SS downscale.
    A wide dark ink shelf gives the gold a hard step to snap against (struck
    metal), then a fat bright gold-leaf core, then a hot catch-pip -- a hard
    stepped facet, deliberately the foil to Kaguya's soft moon-halo."""
    ca, sa = math.cos(ang), math.sin(ang)
    px, py = -sa, ca
    # device-px floors so the chip outlives the smoothscale at gameplay size
    L = max(w_units, 9.0 * ss)            # facet length
    hw = max(w_units * 0.30, 3.2 * ss)    # facet half-width -> a CHUNK, not a line
    tip = (x + ca * L, y + sa * L)
    # 1) the ink shelf -- the hard dark step the gold sits against (widened)
    shelf = [(int(x + px * hw * 1.55), int(y + py * hw * 1.55)),
             (int(x - px * hw * 1.15), int(y - py * hw * 1.15)),
             (int(tip[0] - px * hw * 0.7), int(tip[1] - py * hw * 0.7)),
             (int(tip[0] + px * hw * 1.0), int(tip[1] + py * hw * 1.0))]
    pygame.draw.polygon(surf, INK, shelf)
    # 2) a burnt-amber mid step so the value climbs in HARD planes, not a blur
    mid = [(int(x + px * hw * 1.05), int(y + py * hw * 1.05)),
           (int(x - px * hw * 0.55), int(y - py * hw * 0.55)),
           (int(tip[0] - px * hw * 0.4), int(tip[1] - py * hw * 0.4)),
           (int(tip[0] + px * hw * 0.65), int(tip[1] + py * hw * 0.65))]
    pygame.draw.polygon(surf, STRAW_DD, mid)
    # 3) the FAT bright gold-leaf core -- the load-bearing light chip. Kept WIDE
    # + SOLID so a cluster of near-pure-gold pixels survives the SS downscale
    # (a thin core averages into the straw and vanishes -- the round-2 failure).
    core = [(int(x + px * hw * 0.95), int(y + py * hw * 0.95)),
            (int(x - px * hw * 0.30), int(y - py * hw * 0.30)),
            (int(tip[0] - ca * L * 0.10), int(tip[1] - sa * L * 0.10)),
            (int(tip[0] + px * hw * 0.55), int(tip[1] + py * hw * 0.55))]
    pygame.draw.polygon(surf, GOLD, core)
    # 4) a FAT solid hot catch-pip at the head of the facet -> the metallic glint.
    # A solid GOLD_HI disc this size leaves a few hot pixels after downscale so
    # the glint actually reads as struck metal at 32px, not a soft light smudge.
    pygame.draw.circle(surf, GOLD_HI,
                       (int(x + ca * L * 0.34 + px * hw * 0.1),
                        int(y + sa * L * 0.34 + py * hw * 0.1)),
                       max(3, int(hw * 0.85)))
...
# an ASYMMETRIC stud cadence for the root-ball collar so two studs never land
# symmetric and read as eyes; the pillar/shaft keep the locked symmetric STUD_KS.
STUD_KS_ASYM = (-0.92, -0.30, 0.18, 0.64, 1.0)
...
def node_band(surf, cx, y, half_w, s, color=CLUB, shade=CLUB_D, studs=True,
    ...
    if studs:
        for k in (stud_ks if stud_ks is not None else STUD_KS):
...
def root_ball(surf, cx, cy, r, s, collar=True):
    """The gnarled root-ball striking head: a BIG lumpy culm-root knob with an
    ASYMMETRIC stud-ring collar banding up to it. WHY this geometry (AD GATE 2):
    round 2's knob was the same ~size heptagon as the ogre head AND wore a gold
    cross-crack + two symmetric collar studs -> it scanned as a second face. Now
    the knob is NOTICEABLY BIGGER, its outline is BROKEN by irregular gnarl bumps
    (so it is not the head's polygon), the collar studs run an ASYMMETRIC cadence
    (no eye-pair), and there is NO gold cross-crack across it -- it must read as
    KNOTTED WOOD, never a muzzle."""
    # the stud-ring collar -- ASYMMETRIC cadence so no two studs pair as eyes
    if collar:
        node_band(surf, cx, cy + int(r * 0.92), int(r * 0.62), s, studs=True,
                  stud_ks=STUD_KS_ASYM)
    # an IRREGULAR knotted outline: large lumpy bosses pushed OUT past the radius
    # at uneven angles + uneven amplitude so the silhouette is gnarled, not a
    # clean head polygon. More vertices + asymmetric bulge = reads as root-burl.
    knob = []
    bumps = (1.14, 0.96, 1.20, 0.88, 1.05, 1.18, 0.92, 1.10,
             0.86, 1.16, 0.95, 1.08, 0.90, 1.13)
    n = len(bumps)
    for i, bm in enumerate(bumps):
        # a small per-vertex angle jitter so bosses aren't evenly spaced (gnarl)
        jit = (0.18 if i % 2 == 0 else -0.12) * (math.pi / n)
        a = math.pi * 2 * i / n - math.pi / 2 + jit
    ...
    triad_blob(surf, CLUB, knob,
               ...
               ow=max(1, int(1.9 * s)))
    # knotted root gouges -- placed asymmetrically (NOT an eye-pair) so the lump
    # reads as burl knots, never as facial features
    for dx_f, dy_f, rr in ((-0.40, -0.20, 0.20), (0.30, -0.34, 0.13),
                           (0.36, 0.18, 0.24), (-0.10, 0.44, 0.15),
                           (-0.02, -0.04, 0.10)):
        ...
        pygame.draw.circle(surf, CLUB,
                           ...
                           max(1, int(r * rr * 0.5)))
    # a single fine gold grain-glint LOW-RIGHT only (no CROSS-crack -> no muzzle):
    # keeps the metallic sparkle lane on the wood without building a face.
    gold_crack(surf, cx + int(r * 0.30), cy + int(r * 0.30), r * 0.58,
               math.radians(-48), s, -1)
...
def oni_face(surf, cx, cy, r, s):
    ...
    # heavy horizontal BROW-BAR -- a DARKER, SQUARER block-ridge with a hard
    # centre notch. WHY darker+squarer (AD note 4): a heavier rectangular scowl
    # reads as the head's brow at 32px and pulls the head further apart from the
    # round knotted club -- without over-fixing the chibi charm.
    brow = [(cx - int(r * 0.92), cy - int(r * 0.40)),
            (cx - int(r * 0.30), cy - int(r * 0.48)),
            (cx - int(r * 0.10), cy - int(r * 0.24)),  # hard centre notch (frown)
            (cx + int(r * 0.10), cy - int(r * 0.24)),
            (cx + int(r * 0.30), cy - int(r * 0.48)),
            (cx + int(r * 0.92), cy - int(r * 0.40)),
            (cx + int(r * 0.86), cy - int(r * 0.04)),
            (cx, cy - int(r * 0.02)),
            (cx - int(r * 0.86), cy - int(r * 0.04))]
    triad_blob(surf, STRAW_DD, brow,
               sheen_pts=[(cx - int(r * 0.86), cy - int(r * 0.36)),
                          (cx - int(r * 0.40), cy - int(r * 0.42)),
                          (cx - int(r * 0.34), cy - int(r * 0.20)),
                          (cx - int(r * 0.74), cy - int(r * 0.14))],
               ow=max(1, int(1.4 * s)))
...
def draw_madake_oni(surf, cx, cy, s):
    ...
    # bigger than r2 so the knob is plainly LARGER + lumpier than the ogre head
    # (head r = 26 units) -> the two lobes can never read as the same polygon
    root_ball(csurf, ccx, rb_y, int(26 * s), s, collar=True)
    ...
    body = [(cx - int(34 * s), cy - int(30 * s)),
            ...
            (cx - int(30 * s), cy + int(54 * s)),
            (cx - int(40 * s), cy + int(20 * s))]
    # FLAT planes: STRAW_M shade fills the whole barrel, a crisp STRAW lit plane
    # carves the lit left+centre off it, a hard SHEEN wedge on the top-left.
    flat_form(surf, body, STRAW,
              lit_pts=[(cx - int(40 * s), cy + int(20 * s)),
                       (cx - int(34 * s), cy - int(30 * s)),
                       (cx + int(14 * s), cy - int(30 * s)),
                       (cx + int(10 * s), cy + int(20 * s)),
                       (cx + int(6 * s), cy + int(54 * s)),
                       (cx - int(30 * s), cy + int(54 * s))],
              sheen=SHEEN,
              sheen_pts=[(cx - int(34 * s), cy - int(30 * s)),
                         (cx - int(8 * s), cy - int(30 * s)),
                         (cx - int(14 * s), cy + int(2 * s)),
                         (cx - int(38 * s), cy + int(4 * s))],
              shade=STRAW_M, ow=max(1, int(1.9 * s)))
    ...
    # the ONE dominant chest HERO chip -- sized in device-px (via SS) so its
    # bright core survives the smoothscale and reads as a light chip at 32px
    # (AD GATE 1). The fine grain cracks stay as secondary detail only.
    gold_chip(surf, cx - int(14 * s), cy - int(20 * s), 22 * s, SS, ang=math.radians(98))
    gold_crack(surf, cx + int(12 * s), cy + int(6 * s), 20 * s, math.radians(76), s, -1)
    gold_crack(surf, cx - int(4 * s), cy + int(32 * s), 16 * s, math.radians(92), s, 1)
    ...
    for sgn in (-1, 1):
        ...
        leg = [(lx - int(12 * s), cy + int(48 * s)),
               ...
               (lx - int(14 * s), cy + int(78 * s))]
        flat_form(surf, leg, STRAW,
                  lit_pts=[(lx - int(12 * s), cy + int(48 * s)),
                           (lx + int(4 * s), cy + int(48 * s)),
                           (lx + int(5 * s), cy + int(78 * s)),
                           (lx - int(14 * s), cy + int(78 * s))],
                  sheen=SHEEN,
                  sheen_pts=[(lx - int(12 * s), cy + int(48 * s)),
                             (lx - int(5 * s), cy + int(48 * s)),
                             (lx - int(7 * s), cy + int(78 * s)),
                             (lx - int(14 * s), cy + int(78 * s))],
                  shade=STRAW_M, ow=max(1, int(1.6 * s)))
    ...
    for sgn, dy in ((-1, int(6 * s)), (1, -int(12 * s))):
        ...
        # FLAT planes on the arm: STRAW_M shade fills it, a crisp STRAW lit plane
        # rides the upper edge, a thin SHEEN sliver on the lit shoulder.
        flat_form(surf, arm, STRAW,
                  lit_pts=[(sx - int(10 * s), sy - int(8 * s)),
                           (sx + int(10 * s), sy - int(2 * s)),
                           (gx + int(8 * s), gy + int(2 * s)),
                           (gx + int(2 * s), gy + int(6 * s)),
                           (sx - int(6 * s), sy + int(2 * s))],
                  sheen=SHEEN,
                  sheen_pts=[(sx - int(10 * s), sy - int(8 * s)),
                             (sx + int(4 * s), sy - int(5 * s)),
                             (sx + int(1 * s), sy - int(1 * s)),
                             (sx - int(9 * s), sy - int(3 * s))],
                  shade=STRAW_M, ow=max(1, int(1.6 * s)))
...
def main():
    ...
    sheet.blit(font_sm.render(
        "dry-timber ogre + diagonal culm KANABO  ·  ONE hero gold chip @32px · de-faced knotted club · FLAT-step body · ROUND 3",
    ...
    sheet.blit(font_sm.render("32px day -- gold chip visible", True, LABEL), (panel_x + 20, day_y + 156))

    night_y = day_y + 184
    vgrad(sheet, (panel_x + 20, night_y, 150, 150), NIGHT_T, NIGHT_B)
    pygame.draw.rect(sheet, INK, (panel_x + 20, night_y, 150, 150), 1)
    sheet.blit(chip, (panel_x + 20 + 27, night_y + 27))
    sheet.blit(font_sm.render("32px night -- RED + gold chip, ONE head", True, LABEL_DIM), (panel_x + 20, night_y + 156))
    ...
    sheet.blit(font_sm.render(
        "ELEVATED pipeline: SS=6 supersample -> smoothscale.  R3 (final): ONE chunky chest gold CHIP survives 32px · DE-FACED root-ball "
        "(bigger+lumpier, asym collar studs, no cross-crack) · FLAT-step torso/leg/arm shading · darker squarer brow.  LOCKED: pose, ember, pillar.",
