"""Assemble the bamboo epic-boss versions showcase.
...
tile size. Bamboo is a fresh theme with no shipped reference, so all five are
NEW versions — there is no REFERENCE anchor tile. They are deliberately five
DISTINCT silhouette KINDs (coiling-dragon / club-ogre / slim-noble-stack /
snow-bowed-cap / split-culm-aperture); the matched tile size proves none of
them collapse back into one generic bamboo stalk.

Two of the five never earned a ship sign-off, so an amber MATURED chip flags
them against the green SHIP-READY chips — the un-blessed work stays visually
...
# Neutral mid-grey keeps the jade/straw/lacquer/snow/pearl palettes honest
# regardless of each source sheet's own panel backdrop.
BG = (96, 98, 104)
FRAME = (150, 152, 160)
...
CHIP_MATURED = (236, 184, 78)  # amber: matured but never blessed

# Per-sheet hero windows (x, y, w, h) in source pixels, measured from each
# sheet's actual left-panel hero ink bounding box plus a 14px background margin
# on every side so no extremity clips — the dragon's full snout+coil base, the
# ogre's raised culm-club lobe, the slim noble's coronet+hakama hem, the snow
# cap's full rightward BOW, the moon-child's whole split-culm vesica. Generous
# bg is fine; fit_inside normalizes to the matched tile.
TILES = [
    {
        "file": "docs/skybit_devil/batch2/bamboo_versions/take_ryu/round_3.png",
        "crop": (98, 95, 204, 392),
        "name": "Take-Ryu",
        "facet": "grove-coiling bamboo dragon",
        "status": "MATURED (r3, no ship sign-off)",
        "ship": False,
    },
    {
        "file": "docs/skybit_devil/batch2/bamboo_versions/madake_oni/round_3.png",
        "crop": (98, 161, 286, 348),
        "name": "Madake-Oni",
        "facet": "dry-timber ogre + culm club",
        "status": "MATURED (r3, no ship sign-off)",
        "ship": False,
    },
    {
        "file": "docs/skybit_devil/batch2/bamboo_versions/kurochiku_nushi/round_2.png",
        "crop": (126, 177, 130, 298),
        "name": "Kurochiku-Nushi",
        "facet": "blackened-bamboo lacquer noble",
        "status": "SHIP-READY",
        "ship": True,
    },
    {
        "file": "docs/skybit_devil/batch2/bamboo_versions/yukitake/round_2.png",
        "crop": (98, 131, 288, 380),
        "name": "Yukitake",
        "facet": "snow-bowed bamboo spirit",
        "status": "SHIP-READY",
        "ship": True,
    },
    {
        "file": "docs/skybit_devil/batch2/bamboo_versions/kaguya/round_2.png",
        "crop": (122, 187, 138, 284),
        "name": "Kaguya",
        "facet": "moon-child in the split culm",
        "status": "SHIP-READY",
        "ship": True,
    },
]
...
def main():
    ...
    title = f_title.render(
        "Bamboo versions — epic bamboo bosses (5 distinct silhouette KINDs)",
        True, TITLE_COL,
    )
    ...
    for i, t in enumerate(TILES):
        ...
        # Top-left chip: green SHIP-READY vs amber MATURED so the two un-blessed
        # concepts are flagged at a glance.
        chip_text = "SHIP-READY" if t["ship"] else "MATURED"
        chip_col = CHIP_SHIP if t["ship"] else CHIP_MATURED
        ...
        # Caption band: name + thesis + full status line.
        cap_y = top + TILE + 8
        ...
        status_col = CHIP_SHIP if t["ship"] else CHIP_MATURED
        status = f_status.render(t["status"], True, status_col)
