...
def draw_asthi_dakini(surf, cx, cy, s):
    ...
    # the HERO gem — a faceted cyan cut-stone in the HEART of the necklace, the single
    # brightest pixel (hot core on, no ink seat). Made SMALLER than the prior pass.
    cyan_gem(surf, (rc_cx, rc_cy + int(14 * s)), int(7 * s), s, focal=True)
    ...
    # THIRD EYE — the earlier darker-skin round's concentric-RING iris shape (gold ring
    # + cyan disc), clean (no blue aura) and a step dimmer than the necklace hero gem.
    tex, tey = head_c[0], head_c[1] - int(hr * 0.36)
    ring_eye(surf, (tex, tey), int(hr * 0.30), s)
    # nose triangle
...
def main():
    ...
    sheet.blit(font_big.render("ASTHI — RING THIRD-EYE (no aura) + smaller necklace-heart gem", True, LABEL), (24, 13))
