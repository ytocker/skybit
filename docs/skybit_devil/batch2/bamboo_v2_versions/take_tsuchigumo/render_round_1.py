"""
Round-1 concept renderer for TAKE-TSUCHIGUMO — the grove spider-yokai whose
...
WHY this set is a DELIBERATE DEPARTURE from the chibi/scary-cute house grammar:
bamboo v2 is REALISTIC and botanically accurate — real tall-thin culm
proportions, the two-ring node (sheath-ring + stem-ring), the Phyllostachys
sulcus groove, papery sheath-collars. So the shading here is NOT the smooth
dark-core->fill->sheen triad of the skeleton broods; it is HARD STEPPED VALUE
BANDS (4-6 discrete facets per form, NO gradients) for a sculpted near-
volumetric read that survives the true-32px downscale. Radial glow is for the
venom-amber eye/fang ACCENTS only.
...
many-limbed star. The top read is a compact bristled CARAPACE body with a
cluster of long jointed CULM legs arcing OUT and DOWN, fangs below — each leg a
real bamboo culm whose NODE-RINGS segment the leg-joints (the node IS the leg-
knuckle: botany and anatomy fuse), a dusty-rose sheath-collar at each joint.
...
values on the figure and the FRONT leg-pairs are rendered largest so the radial
STAR reads as a star (not a blob) at 32px. The whole figure leans HARD into the
WARM amber/dusty-rose ambush-predator temperature — the gap-widener against #3's
COOL snow-white winter mound.

WHY the single culm-LEG is the pillar: a jointed leg IS a natural bamboo culm —
node-segment leg = the repeat band (each node a leg-knuckle); the fanged head-
carapace with eye-glints = the detachable gap-edge cap; a curled leg-tip claw +
a wisp of web = the lower mirror. Slim, on-axis, bottom-rooted.
...
# ── PINNED PALETTE (locked brief, lane #7) ───────────────────────────────────
# Deep forest-grove green-black is the dominant culm mass; the venom-amber
# eye/fang accent is the BRIGHTEST value + readability anchor; dusty-rose marks
# the papery sheath-collars at each leg-joint.
...
# derived stepped bands kept INSIDE the pinned families (no new hues) — these
# are the discrete VALUE steps, never blended between.
GROVE_HI   = ( 96, 136,  92)   # lit culm facet (top-left band)
GROVE_HHI  = (138, 176, 124)   # hottest culm rim band (a thin top edge only)
NODE_HI    = (190, 158, 100)   # lit node-knuckle band
...
def culm_segment(surf, a, b, w_root, w_tip, s, collar=True, sulcus=True):
    ...
    if collar:
        ...
        stepped_blob(surf, node, [
            (NODE_D, 1.0), (NODE_TAN, 0.82), (NODE_HI, 0.5),
        ], ow=max(1, int(w_tip * 0.12)))
...
# ── a whole jointed CULM LEG — 3 internodes arcing OUT and DOWN, claw tip ────
def culm_leg(surf, root, ang0, reach, w0, s, droop=0.55, tip_claw=True):
    ...
    than the last, the leg ARCING progressively DOWNWARD (the `droop` adds to the
    angle per joint) so it reads as a long jointed limb planted toward the
    ground, not a straight radial spoke. Node-knuckles + rose collars segment
    every joint. Ends in a small curled culm-tip CLAW (a sharp dark hook). WHY
    front-largest construction lives in the caller: the radial star must read at
    32px, so the caller renders the FRONT pairs with bigger `reach`/`w0`."""
    ...
    for i, (lf, wf) in enumerate(segs):
        ...
        p = nxp
        w = wt
        a += droop                       # each joint angles further DOWN
    if tip_claw:
...
def carapace(surf, cx, cy, rw, rh, s):
    ...
    only — top-left lit facet stepped, never blended."""
    ...
    stepped_blob(surf, ab, [
        (GROVE_D, 1.0), (GROVE, 0.82), (GROVE_HI, 0.46),
    ], ow=max(1, int(1.4 * s)))
    ...
    stepped_blob(surf, ceph, [
        (GROVE_D, 1.0), (GROVE, 0.84), (GROVE_HI, 0.5),
    ], ow=max(1, int(1.6 * s)))
...
def draw_tsuchigumo(surf, cx, cy, s, leg_glow=True):
    """The grove spider-yokai in a radial STAR: a compact bristled carapace hub
    with EIGHT jointed culm-legs arcing OUT and DOWN, the FRONT pairs largest so
    the star reads at 32px, venom-amber eye-cluster + fangs as the bright focal.
    `s` ~ unit scale around a ~120-unit figure. Legs drawn first/behind so the
    carapace overlaps the roots (legs anchored INTO the body)."""
    rw = int(20 * s)
    rh = int(17 * s)
    root = (cx, cy + int(2 * s))

    # EIGHT legs, four per side, arcing out-and-down. FRONT pairs (nearer the
    # horizontal-forward) get the longest reach + widest root so the radial star
    # holds its points at 32px; rear legs are shorter so they don't muddy it.
    # angles measured from +x (right); we mirror across the vertical axis.
    # (base_angle_deg_from_down, reach_units, root_width_units, droop)
    leg_specs = [
        (118, 56, 8.5, 0.40),   # FRONT (most forward/up-out) — LARGEST
        (150, 50, 7.5, 0.50),   # front-mid
        (182, 44, 6.5, 0.58),   # rear-mid
        (210, 38, 5.5, 0.64),   # REAR (most back/down) — smallest
    ]
    # draw rear legs first (behind), front legs last (in front), per side
    order = sorted(range(4), key=lambda i: -leg_specs[i][1])  # longest drawn last? no
    # draw shortest (rear) first so longest front legs overlap them
    order = sorted(range(4), key=lambda i: leg_specs[i][1])
    for i in order:
        base_deg, reach, w0, droop = leg_specs[i]
        for sgn in (-1, 1):
            # convert: down is +y. base_deg measured so 90 = straight down,
            # 180 = straight left. We want legs going UP-and-OUT then drooping.
            # Use angle from +x: left side = (180 - (base_deg-90)) sort of —
            # simpler: define on the LEFT then mirror.
            ang = math.radians(base_deg)        # left-side launch (up-left-ish)
            if sgn > 0:
                ang = math.radians(180) - ang   # mirror to the right side
            # root offset along the carapace rim so legs fan from the hub edge
            rx = root[0] + sgn * int(rw * 0.7)
            ry = root[1] - int(rh * 0.2) + (i - 1) * int(rh * 0.28)
...
def draw_pillar(surf, cx, top, bot, s, cap="bottom"):
    ...
    bottom-rooted. `cap` names the END that faces the GAP. Verifies the leg reads
    as bamboo on its own."""
    w_root = int(13 * s)
    w_tip = int(11 * s)
    cap_room = int(46 * s)
    if cap == "bottom":
    ...
    if cap == "bottom":
        ...
        carapace(surf, cx, cap_cy, int(15 * s), int(13 * s), s)
        culm_leg(surf, (cx - int(10 * s), cap_cy), math.radians(150),
                 26 * s, 6 * s, s, droop=0.5)
        culm_leg(surf, (cx + int(10 * s), cap_cy), math.radians(30),
                 26 * s, 6 * s, s, droop=0.5)
        eye_cluster(surf, cx, cap_cy - int(2 * s), int(13 * s), s, glow=True)
    else:
        # lower mirror: a curled leg-tip CLAW + a wisp of web, mirrored vertically
        tmp = pygame.Surface(surf.get_size(), pygame.SRCALPHA)
        ty = surf.get_height() - (top + int(24 * s))
        # a curled culm-tip claw hooking toward the gap
        p, a = culm_leg(tmp, (cx, ty - int(20 * s)), math.radians(90),
                        34 * s, 9 * s, s, droop=0.42)
        # a wisp of web — three faint dragline threads fanning from the claw
        for dth in (-0.5, 0.0, 0.5):
            wx = p[0] + math.cos(a + dth) * 30 * s
            wy = p[1] + math.sin(a + dth) * 30 * s
            pygame.draw.line(tmp, ROSE_HI, p, (wx, wy), max(1, int(0.8 * s)))
...
def main():
    ...
    sheet.blit(font_sm.render(
        "grove spider-yokai · RADIAL multi-leg · culm-segment legs (node = leg-knuckle) · venom-amber focal · dusty-rose collars · round 1",
        True, LABEL_DIM), (320, 26))

    # === (a) BIG HERO =========================================================
    def render_hero():
        big = pygame.Surface((360 * SS, 470 * SS), pygame.SRCALPHA)
        draw_tsuchigumo(big, 178 * SS, 210 * SS, 2.05 * SS)
        small = pygame.transform.smoothscale(big, (360, 470))
        return grow_outline(small, INK + (255,), 1)

    sheet.blit(render_hero(), (14, 92))
    sheet.blit(font.render("Creature — hero", True, LABEL), (110, 566))
    sheet.blit(font_sm.render("RADIAL spider: bristled carapace hub + EIGHT jointed CULM legs arcing out/down.", True, LABEL_DIM), (14, 590))
    sheet.blit(font_sm.render("Each leg-joint = a bamboo NODE-knuckle (muddy-tan) + dusty-rose sheath-collar; sulcus groove.", True, LABEL_DIM), (14, 606))
    sheet.blit(font_sm.render("Venom-amber 8-eye cluster + fangs = brightest values + readability anchor. HARD stepped bands, no gradients.", True, LABEL_DIM), (14, 622))
    ...
    sheet.blit(font_sm.render("leg-knuckle); fanged head-carapace w/ eye-glints", True, LABEL_DIM), (pcx - 4, 730))
    sheet.blit(font_sm.render("= gap cap; curled claw + web wisp = lower mirror.", True, LABEL_DIM), (pcx - 4, 746))
    ...
    def chip32():
        ...
        draw_tsuchigumo(big, 48 * SS, 46 * SS, (32 / 122.0) * SS)
    ...
    # a blob: the front-largest leg arc has to hold its points.
    def silhouette32():
        big = pygame.Surface((96 * SS, 96 * SS), pygame.SRCALPHA)
        draw_tsuchigumo(big, 48 * SS, 46 * SS, (32 / 122.0) * SS, leg_glow=False)
        small = pygame.transform.smoothscale(big, (96, 96))
    ...
    sheet.blit(font_sm.render(
        ...
        "hard ink keyline (28,22,30) + 1px grown outline · botanically-real culm legs (two-ring node + sulcus + sheath-collar) · WARM ambush temperature.",
