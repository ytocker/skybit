...
ROUND 2 — focused 32px legibility pass on the round-1 master. The concept,
palette, and prop were signed off; the only failure was that at 32px the two
bodies fused into one brown-grey blob and the rearing-horse + rider KIND did
not survive. This round forces the two-body grand-boss silhouette:

  1. INK CHANNEL — a 1-2px negative-space gap (at 32px) carved between the
     rider mass and the horse's back, so the stacked-body read is bought by
     dark air, not by adjacent same-value fills.
  2. STEEPER REAR — the neck->foreleg diagonal is pushed to a ~35deg lean and
     the forelegs are lifted clearly off the baseline, so the black shape reads
     unmistakably as a REAR, not a lumpy quadruped.
  3. SOMBRERO-ARM SPIKE — the rider's up-thrust hat breaks the outline as a
     distinct thin-arm + disc lollipop, separated from the skull by sky.
  4. A DELIBERATE 32px CONSTRUCTION (not a downscale of the 160px): rib banding,
     jacket buttons and reins are dropped small; the horse barrel is one bone
     mass with a single dark-core trough; the rider is a skull dot + torso block
     + arm spike. The full incident detail lives only on the large render.
  5. VALUE SPLIT — the horse barrel carries a deeper tan-shade core so the
     lighter bone rider pops off it (~15-20% value contrast sells "rider on
     mount").

House style unchanged: CHIBI proportions, FLAT saturated fills + hard 1-2px ink
keylines, form via dark-core -> flat-fill -> top-left rim-sheen TRIAD,
silhouette POP via a 1px outline grown from the alpha mask, supersample -> scale.

Red-split pin held exactly: rust in the desaturated BRICK/LEATHER lane
(176,70,56), riding with ochre + turquoise as the full house triad. Prop->pillar
mirror kept essentially as-is (the AD called it correct): slim lance shaft +
compact on-axis horse-skull finial, single-mass and symmetric.

Run headless:  SDL_VIDEODRIVER=dummy python render_round_2.py
"""
import math
import os
...
# rider reads as a separate body riding ON it (AD note #6) — still bone-family,
# just the shaded mid-tone, never a new hue.
...
def _ink_channel(surf, pts, w):
    ...
    most important fix: dark air between rider mass and horse back so the two
    bodies cannot fuse into one fill at 32px."""
...
def draw_jinete(target_size):
    ...
    high, one bone arm thrust UP as a hat-lollipop spike. A hard ink channel
    separates the two bodies. The grandest, two-body boss silhouette of the
    family."""
    S = target_size * SS
    ...
    # Pushed to the lower-right corner so the WHOLE body tilts back along a steep
    # up-left diagonal — the rear reads in the gross silhouette, not just a pose.
    ...
    # Barrel sits a value-step darker than the rider's bone (HORSE_BODY) so the
    # rider pops off it.
    ...
    triad_ellipse(surf, chest_x, chest_y, S * 0.150, S * 0.140,
                  HORSE_CORE, HORSE_BODY, HORSE_SHEEN)
    # Rib bands — large-render only (dropped at 32px per AD #2).
    for i in range(4):
    ...
    # Lifted well clear of the ground so the black shape reads as a REAR.
    ...
    zar = [
        (chest_x + S * 0.04, chest_y - S * 0.05),
        (haunch_x + S * 0.06, haunch_y - S * 0.12),
        (haunch_x + S * 0.085, haunch_y + S * 0.04),
        (chest_x + S * 0.08, chest_y + S * 0.115),
    ]
    ...
    # ── INK CHANNEL — carve dark air between horse back and rider (AD #1) ─────
    # Runs along the top of the barrel/haunch where the rider sits, BEFORE the
    # rider is drawn, so the rider's lighter bone mass lands above a hard dark
    # gap and the two-body read is forced by negative space.
    rid_x = haunch_x - S * 0.03
    seat_y = haunch_y - S * 0.135
    _ink_channel(surf, [
        (chest_x + S * 0.05, chest_y - S * 0.085),
        (rid_x - S * 0.02, seat_y + S * 0.05),
        (haunch_x + S * 0.07, haunch_y - S * 0.10),
    ], int(S * 0.05))

    # ── RIDER: tiny charro skeleton seated high (bone — pops off darker barrel)
    bone_limb(surf, (rid_x + S * 0.005, seat_y + S * 0.03),
              (rid_x + S * 0.07, seat_y + S * 0.085),
              (rid_x + S * 0.045, seat_y + S * 0.15), S * 0.032)
    bone_limb(surf, (rid_x - S * 0.005, seat_y + S * 0.03),
              (rid_x - S * 0.075, seat_y + S * 0.08),
              (rid_x - S * 0.05, seat_y + S * 0.15), S * 0.032)
    jacket = [
        (rid_x - S * 0.072, seat_y - S * 0.085),
        (rid_x + S * 0.072, seat_y - S * 0.085),
        (rid_x + S * 0.06, seat_y + S * 0.045),
        (rid_x - S * 0.06, seat_y + S * 0.045),
    ]
    triad_poly(surf, jacket, RUST_CORE, RUST, RUST_SHEEN,
               sheen_pts=[(rid_x - S * 0.058, seat_y - S * 0.07),
                          (rid_x - S * 0.01, seat_y - S * 0.075),
                          (rid_x - S * 0.025, seat_y + S * 0.02),
                          (rid_x - S * 0.058, seat_y + S * 0.02)])
    for side in (-1, 1):
        for i in range(3):
            byp = seat_y - S * 0.055 + i * S * 0.035
            bxp = rid_x + side * S * 0.038
            triad_ellipse(surf, bxp, byp, S * 0.013, S * 0.013,
                          OCHRE_CORE, OCHRE, OCHRE_SHEEN)
    pygame.draw.arc(surf, TURQ,
                    (int(rid_x - S * 0.068), int(seat_y - S * 0.07),
                     int(S * 0.04), int(S * 0.09)),
                    math.pi * 1.4, math.pi * 2.1, max(1, int(SS)))

    # Rein arm (low, player-left) — large-render only.
    bone_limb(surf, (rid_x - S * 0.05, seat_y - S * 0.05),
              (rid_x - S * 0.11, seat_y + S * 0.0),
              (rid_x - S * 0.17, seat_y + S * 0.05), S * 0.024)
    pygame.draw.line(surf, TURQ,
                     (rid_x - S * 0.17, seat_y + S * 0.05),
                     (sk_x + S * 0.04, sk_y + S * 0.02), max(1, int(SS * 1.2)))

    # RIDER SKULL (small, painted).
    rh_y = seat_y - S * 0.155
    triad_ellipse(surf, rid_x, rh_y, S * 0.066, S * 0.072, BONE_CORE, BONE, SHEEN)
    triad_ellipse(surf, rid_x, rh_y + S * 0.048, S * 0.05, S * 0.032,
                  BONE_CORE, BONE, SHEEN)
    for ex in (rid_x - S * 0.025, rid_x + S * 0.025):
        triad_ellipse(surf, ex, rh_y - S * 0.004, S * 0.017, S * 0.019,
                      SOCKET_CORE, SOCKET_FILL, SOCKET_SHEEN, outline=None)
        pygame.draw.circle(surf, INK, (int(ex), int(rh_y - S * 0.004)),
                           int(S * 0.008))
    pygame.draw.polygon(surf, SOCKET_CORE, [
        (rid_x, rh_y + S * 0.018),
        (rid_x - S * 0.008, rh_y + S * 0.034),
        (rid_x + S * 0.008, rh_y + S * 0.034)])
    sm_y = rh_y + S * 0.044
    pygame.draw.arc(surf, INK,
                    (int(rid_x - S * 0.034), int(sm_y - S * 0.022),
                     int(S * 0.068), int(S * 0.044)),
    ...
    # ── WAVING ARM thrust UP as a hat-lollipop SPIKE (AD #3) ──────────────────
    # Thin arm rising clear of the skull into open sky, capped by a sombrero
    # disc — a distinct lollipop that breaks the outline, the motion tell that
    # must survive at 32px. Separated from the head so it never buries.
    hand = (rid_x + S * 0.135, rh_y - S * 0.175)
...
# ── DEDICATED 32px CONSTRUCTION (AD #2/#4/#5 — not a downscale) ───────────────
...
def draw_jinete_tiny(target_size):
    ...
    the baseline. The rider is a skull dot + a brick torso block sitting above a
    HARD INK CHANNEL, with a thin arm + ochre disc lollipop spiking into the sky.
    No rib banding, no buttons, no reins, no fringe — those are large-render only.
    """
    S = target_size * SS
    surf = pygame.Surface((S, S), pygame.SRCALPHA)
    ...
    # Steep-rear geometry (mirrors the large pose but stripped).
    haunch_x, haunch_y = cx + S * 0.18, S * 0.62
    chest_x, chest_y = cx - S * 0.08, S * 0.42

    # Hind legs — two planted, the diagonal base of the rear.
    bone_limb(surf, (haunch_x, haunch_y),
              (cx + S * 0.10, S * 0.80), (cx + S * 0.08, S * 0.96),
              legw, body=HORSE_BODY, core=HORSE_CORE)
    bone_limb(surf, (haunch_x + S * 0.04, haunch_y),
              (cx + S * 0.27, S * 0.79), (cx + S * 0.30, S * 0.96),
              legw, body=HORSE_BODY, core=HORSE_CORE)
    for hx in (cx + S * 0.08, cx + S * 0.30):
        triad_ellipse(surf, hx, S * 0.955, S * 0.055, S * 0.04,
                      RUST_CORE, RUST, RUST_SHEEN)
    ...
    sk_x, sk_y = cx - S * 0.30, S * 0.21
    neck = [
        (chest_x + S * 0.02, chest_y - S * 0.11),
        (chest_x - S * 0.10, chest_y - S * 0.10),
        (sk_x + S * 0.02, sk_y + S * 0.10),
        (sk_x + S * 0.12, sk_y + S * 0.07),
    ]
    triad_poly(surf, neck, HORSE_CORE, HORSE_BODY, HORSE_SHEEN)

    # Horse skull-muzzle — cranium + a clear forward snout wedge (the head read).
    ...
    # Zarape — a single rust wedge band (no stripes at this size, just the color).
    rid_x = haunch_x - S * 0.04
    seat_y = haunch_y - S * 0.15
    zar = [(chest_x + S * 0.06, chest_y - S * 0.04),
           (haunch_x + S * 0.05, haunch_y - S * 0.10),
           (haunch_x + S * 0.07, haunch_y + S * 0.04),
           (chest_x + S * 0.10, chest_y + S * 0.10)]
    triad_poly(surf, zar, RUST_CORE, RUST, RUST_SHEEN, inset_amt=0.10)

    # HARD INK CHANNEL between horse back and the rider — drawn before the rider.
    _ink_channel(surf, [
        (chest_x + S * 0.06, chest_y - S * 0.10),
        (rid_x - S * 0.03, seat_y + S * 0.07),
        (haunch_x + S * 0.06, haunch_y - S * 0.10),
    ], int(S * 0.075))

    # Rider — skull dot + brick torso block + arm spike. That is all it can hold.
    block = [(rid_x - S * 0.085, seat_y - S * 0.085),
             (rid_x + S * 0.085, seat_y - S * 0.085),
             (rid_x + S * 0.07, seat_y + S * 0.06),
             (rid_x - S * 0.07, seat_y + S * 0.06)]
    ...
    # Two socket dots so the skull reads even tiny.
    ...
    pygame.draw.line(surf, INK, (rid_x + S * 0.06, seat_y - S * 0.07),
                     hand, int(legw + SS * 1.4))
    pygame.draw.line(surf, BONE, (rid_x + S * 0.06, seat_y - S * 0.07),
                     hand, int(legw * 0.55))
...
def build_sheet():
    ...
    label("EL CHARRO JINETE — rearing bone-horse + charro rider  ·  round 2",
          18, 12, (252, 224, 150))
    caption("ROUND 2 — forces the two-body grand-boss read at 32px: ink channel "
            "between rider+horse · steeper rear · sombrero-arm spike · dedicated "
            "32px build · darker barrel",
            18, 36)

    # Large creature.
    big = draw_jinete(320)
    sheet.blit(big, (20, 58))
    caption("creature · large (steep rear + raised-hat spike)", 20, 382)
    ...
    # Pure-black silhouette read — the AD ship test, large + 32px.
    ...
    caption("read: reared horse + rider, hat-arm spike", 192, 554)

    # 32px on OCHRE DAY sky and on NIGHT sky (AD #7).
    sky_crop(sheet, 352, 408, 128, tiny,
             (236, 196, 120), (210, 158, 92), small, caption,
             "32px @4x · ochre DAY sky", "brick-rust stays brown vs warm")
    sky_crop(sheet, 500, 408, 128, tiny,
             (40, 44, 84), (22, 26, 54), small, caption,
             "32px @4x · NIGHT sky", "bone bodies still carry the read")
    ...
    swatches = [
        ...
        ("barrel", HORSE_BODY), ("tan", BONE_SHADE), ("marigold", MARIGOLD),
        ("ink", INK), ("sheen", SHEEN),
    ]
