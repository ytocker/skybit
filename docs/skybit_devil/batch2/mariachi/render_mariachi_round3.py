...
Round 3 (final pass) lands the one remaining blocker from the round-2 critique:
the held guitarron must separate from the ribcage at TRUE 32px instead of fusing
into one warm lump. Surgical fix only — a 1px INK GAP between the disc rim and
the jacket, a darker deep-rosewood disc value, and a bright rim-sheen arc so the
disc curve survives the 1x alpha-outline. Rosette spark lifted one step; warm
ochre-sky frame added to verify the brim-underside band keeps the face legible.
Everything else (off-axis tilt, splayed kick leg, flat crown, palette, pillar)
is held exactly as round 2.

Run headless:  SDL_VIDEODRIVER=dummy python render_mariachi_round3.py
"""
import math
import os
...
# Guitarron pushed one value-step DARKER than round 2's rosewood so the held
# disc reads as a distinct deeper-rust mass against the warm-bone ribcage at
# TRUE 32px — at 1x the old wood and the jacket fused into one warm lump.
GUITAR_WOOD = (128,  70,  40)   # guitarron deep-rosewood body (darker than r2)
GUITAR_CORE = ( 78,  42,  26)
GUITAR_SHEEN= (214, 156, 100)   # kept bright so the rim still catches light
...
def draw_mariachi(target_size):
    ...
    # Hard INK GAP so the wood disc never merges with the rust jacket at TRUE
    # 32px. Widened to ~SS*3.4 (≈1px after the /SS smoothscale) so a real dark
    # seam survives the 1x alpha-outline between the two warm masses — the round-2
    # halo was too thin and the disc edge dissolved into the ribcage at 1x.
    _ell(surf, INK, gx, gy, grx + SS * 3.4, gry + SS * 3.4)
    triad_ellipse(surf, gx, gy, grx, gry, GUITAR_CORE, GUITAR_WOOD, GUITAR_SHEEN)
    # Bright top-left rim sheen arc so the disc CURVE catches light and the held
    # round mass reads as a second rounded shape, not a flat continuation of the
    # torso — the value contrast carries the cue once the ink gap separates them.
    pygame.draw.arc(surf, GUITAR_SHEEN,
                    (int(gx - grx), int(gy - gry),
                     int(grx * 2), int(gry * 2)),
                    math.pi * 0.55, math.pi * 1.15, max(2, int(SS * 1.6)))
    pygame.draw.arc(surf, SHEEN,
                    (int(gx - grx + SS), int(gy - gry + SS),
                     int(grx * 2 - SS * 2), int(gry * 2 - SS * 2)),
                    math.pi * 0.62, math.pi * 1.02, max(1, int(SS)))
    ...
    # Rosette spark lifted one step: 1px larger ring + a brighter turquoise so it
    # reads as a deliberate sound-hole point (the cheapest "instrument not shield"
    # cue at 1x) rather than stray noise once the disc edge is separated.
    ROSE = (96, 214, 202)
    pygame.draw.circle(surf, ROSE, (int(gx - S * 0.006), int(gy - S * 0.008)),
                       int(S * 0.078), int(SS * 2.0))
    pygame.draw.circle(surf, SHEEN, (int(gx - S * 0.006), int(gy - S * 0.008)),
                       int(S * 0.078), int(SS * 0.5))
...
def build_sheet():
    ...
    label("MARIACHI — strumming charro skeleton musician  ·  round 3 (final)", 18, 12,
          (252, 224, 150))
    ...
    # Warm OCHRE-DAY-SKY verification (critique fix #3): confirm the brim-underside
    # ink band keeps the face disc-edge legible on a desert biome, not only on the
    # dark review mat — and that the new guitarron ink gap reads on warm ground too.
    tiny_w = draw_mariachi(32)
    for (panel_x, sky_top, sky_bot, name) in (
        (350, (236, 196, 120), (210, 158, 92), "ochre day sky"),
    ):
        for yy in range(128):
            t = yy / 128.0
        ...
        caption("32px @4x on " + name, panel_x, 538)
        caption("brim band + disc edge legible", panel_x, 554)
