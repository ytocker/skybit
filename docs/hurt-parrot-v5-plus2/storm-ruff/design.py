"""
`storm-ruff` — field-dressed-plus with a blown-back collar of dressing.

Same body treatment as field-dressed-plus, plus a gauze crescent anchored at
the nape and flared over the crown into the empty rows above the head. The
collar reads as wind-caught rather than applied, so the bird looks like it flew
out of the weather that hurt it instead of merely being patched up afterwards.
"""
import math
import os, sys
