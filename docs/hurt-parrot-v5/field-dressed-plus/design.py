"""
`field-dressed-plus` — field-dressed with two additional adhesive bandaid strips.

Same thesis as field-dressed (one main chest pad, one wound, one cross), but
two small rectangular plasters are added to the body: one on the left flank
below the lower claw-cut, one on the lower-right body. They read as the medic
grabbed whatever was in the kit — the big pad for the serious chest wound, two
plasters for the incidental nicks.
"""
import math
import os, sys
...
if __name__ == "__main__":
    ...
    lbl = font.render("field-dressed-plus — round 2   (4x / 2x / 1x day + night)",
                      True, (225, 225, 245))
