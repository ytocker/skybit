...
def _name_engraved_strike(big, name, cx, cy, max_w):
    ...
    for char, adv in zip(name, advances):
        ...
        # Recessed floor: slightly darker than the band (subtractive tint)
        recess = tile.copy()
        recess.fill((9, 8, 18, 200), special_flags=pygame.BLEND_RGBA_MULT)
        big.blit(recess, (dst_x, dst_y))

        # Shadow pass: pewter at (+1,+1) bottom-right — the receding cut wall
        shadow = tile.copy()
        shadow.fill((40, 36, 52, 220), special_flags=pygame.BLEND_RGBA_MULT)
        big.blit(shadow, (dst_x + 1, dst_y + 1))

        # Catch-light: CARD_RING_BRIGHT at (-1,-1) top-left — raking light on cut edge
        catch = tile.copy()
        catch.fill((*CARD_RING_BRIGHT, 255), special_flags=pygame.BLEND_RGBA_MULT)
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v4 — engraved-strike — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v4/engraved-strike/round_1.png"
