"""Headless 10x12 exploration sheet for the round-12 CALM sky concepts.

Renders the 10 `tools.sky_concepts_calm.CONCEPTS` (rows) across 12 day-phase
samples in natural day order (columns), sky-only via the smoother Catmull-Rom
`paint_sky` (stars kept on), into `docs/biome_redesign/round_12.png`.

This is a copy of `tools/preview_sky_concepts.py` pointed at the calm concept
set; it inherits the band-free Catmull gradient bake so the restrained pastels
read clean (no contour lines, no muddy plateaus). Dev aid only — the game never
imports this; the live `ACTIVE_SKY_DESIGN` is untouched.

    python tools/preview_sky_concepts_calm.py
"""
import os
import sys
...
def main():
    ...
    title = f_title.render("Skybit sky concepts — round 17 (calm day/night, colourful sunrise+sunset)", True, (245, 246, 250))
    ...
    out = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                       "docs", "biome_redesign", "round_17.png")
