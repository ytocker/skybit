"""Generate 5 pill-colour variants of the polished v3 menu.

The user rejected the deep-wine pill body. This produces 5 different
button colour directions on the SAME polished v3 base, so the only
variable is the pill body colour. Border + divider + panel accent
stay gold across every variant (consistent baseline). Title is
unchanged (gold-on-red signature). Background, stars, mountains,
panels are identical across all five.

Output (720x1280 each):
  v3_btn_1_navy.png      — midnight navy body
  v3_btn_2_scarlet.png   — refined scarlet body
  v3_btn_3_plum.png      — royal plum body
  v3_btn_4_charcoal.png  — neutral charcoal slate body
  v3_btn_5_amber.png     — warm burnt-amber body
"""
import math
import os
import random
...
# ── Background composition (identical across every variant) ─────────────────
...
# ── Parametric pill — the ONLY thing that changes across variants ───────────

def pill(surf, center, text, btn_top, btn_bot, text_color, text_shadow,
         size=22, min_w=240, h=48, primary=False):
    h *= SCALE
    cx, cy = center
    f = font(size, True)
    ...
    if primary:
        # Gold outer halo — same across all variants for consistency
    ...
    # Gold border (constant across variants)
...
# ── BEST + TOP 10 panels (identical across variants — gold accent system) ───
...
def render_variant(name, btn_top, btn_bot, text_color, text_shadow):
    ...
    pill(s, (W // 2, primary_y), "TAP TO START",
         btn_top, btn_bot, text_color, text_shadow,
...
# Variant palettes — pill body top/bot + text colour + text shadow.
# Border + accents stay gold across every variant.
VARIANTS = [
    # 1. Midnight Navy — body matches the BEST/TOP 10 panel family
    ("v3_btn_1_navy.png",
     ( 52,  40,  96),
     ( 22,  14,  56),
     CREAM,
     (8, 4, 24)),
    # 2. Refined Scarlet — deep red that ties to the title's red outline
    ("v3_btn_2_scarlet.png",
     (172,  36,  28),
     ( 96,  16,  10),
     CREAM,
     (40, 6, 4)),
    # 3. Royal Plum — leans into the night sky's purple undertones
    ("v3_btn_3_plum.png",
     ( 96,  38, 102),
     ( 50,  18,  68),
     CREAM,
     (30, 10, 40)),
    # 4. Charcoal Slate — neutral premium minimal
    ("v3_btn_4_charcoal.png",
     ( 56,  62,  78),
     ( 26,  30,  44),
     (250, 248, 240),
     (8, 10, 18)),
    # 5. Burnt Amber — warm copper without the bright-orange-red plastic
    ("v3_btn_5_amber.png",
     (172, 104,  36),
     (108,  60,  16),
     CREAM,
...
if __name__ == "__main__":
    print(f"Generating 5 pill-colour variants at {W}x{H}...")
    for v in VARIANTS:
        render_variant(*v)
    print("Done.")
