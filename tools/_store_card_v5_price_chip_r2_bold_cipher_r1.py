"""v5 price-chip concept: BOLD-CIPHER — round 1 (Lever: T — typography).

The premise: strip the chip back to a matte near-black slab and let the
numerals BE the identity. Gold body (the current PRICE_STOPS chip) is the
BEFORE the user wants to move away from — a busy amber pill competing with its
own digits. Here the body recedes to a matte graphite slab with a single thin
cool-steel bevel, and the price is pushed hard: 1.2x taller glyphs, wide m(2)
tracking, a near-white cool-foot gradient fill, and a 1px dark keyline to kill
any halo against the dark slab. The digits do all the talking.

Round-1 sheet: skin_mummy EPIC affordable + can't-afford heroes, a 4x detail
strip, and a true-1x swatch row that includes the "100,000" worst-case so the
oversized glyph width is verified against the widest price the store can show.

Output: docs/store_card_v5_price_chip_r2/bold-cipher/round_1.png
"""
import os, sys
...
def _draw_numerals(surf, text, f, nw, center, top_col, foot_col):
    """Near-white glyph with a faint cool-blue foot, over a 1px dark keyline.

    The keyline is a black stamp ringed one step out (kw) so a hairline of dark
    survives around the glyph on the matte slab — it kills the soft halo that a
    bright glyph would otherwise bleed on a near-black ground. The face is a
    white glyph master multiplied by a top->foot cool ramp, so the digits read
    as one lit block of type rather than a flat fill."""
...
def bold_cipher_chip(surf, cx, cy, text, h, affordable=True):
    ...
    body is a near-black graphite slab (affordable) or cool slate (locked) with
    ONE thin cool-steel bevel and minimal gloss/AO, so nothing competes with the
    type. Digits are 1.2x the stock height, m(2)-tracked, near-white with a cool
    foot, keylined against the slab. Locked cools the whole slab + tints the
    coin toward slate so it reads locked on value, not hue alone."""
    ...
    if affordable:
        # matte graphite slab — the BEFORE gold pill is deliberately abandoned
        sc.chip_body_stops(surf, r, rad,
                           stops=[(0.0, (14, 14, 24)), (1.0, (8, 8, 16))],
                           rim_dark=(8, 8, 14), rim_bright=(130, 142, 162),
                           gloss=14, gamma=1.03)
        top_col, foot_col = (255, 255, 255), (200, 215, 235)
        cool_coin = None
    else:
        # locked: whole slab lifts to cool slate (value delta, colourblind-safe)
        sc.chip_body_stops(surf, r, rad,
                           stops=[(0.0, (80, 86, 108)), (1.0, (52, 56, 76))],
                           rim_dark=(8, 8, 14), rim_bright=(168, 176, 198),
                           gloss=14, gamma=1.03)
        top_col, foot_col = (228, 232, 240), (190, 196, 212)
...
# width report to confirm against the brief's ~109 dev px stamped / w ~203 est.
_h = sc.m(20)
...
def main():
    ...
    text("v5 bold-cipher r1 — matte slab, numerals ARE the identity (Lever: T)",
    ...
    text("AFFORDABLE", lbl_sm, (cx_yes + PANEL_W // 2, y_cardlbl + 12),
         (150, 220, 255))
    text("CAN'T AFFORD", lbl_sm, (cx_no + PANEL_W // 2, y_cardlbl + 12),
         (150, 158, 172))
    ...
    text("matte graphite slab  ·  1.2x oversized numerals, m(2) tracking  ·  "
         "cool near-white fill + 1px keyline  ·  locked slab cools whole",
         lbl_xs, (canvas_w // 2, canvas_h - MARGIN - FOOTER_H // 2),
         (120, 118, 140))

    out = os.path.join(os.path.dirname(__file__), "..", "docs",
                       "store_card_v5_price_chip_r2", "bold-cipher", "round_1.png")
