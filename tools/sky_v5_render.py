"""ALPINE_HAZE fine-tune v5_early_night — night rises from the ground up.

Only sky_bot and horizon differ from live ALPINE_HAZE at phases 0.27–0.52.

    python tools/sky_v5_render.py

Output: docs/sky_transition/v5_early_night/round_1.png
"""
import os
import sys
...
# Surgical: every other palette channel (sky_top, sky_mid, star_alpha) and
# every phase outside the sunset arc is inherited from live ALPINE_HAZE, so
# the only variable under review is how early the ground-level band goes cold.
_OVERRIDES = {
    0.27: dict(sky_bot=(242, 158, 146), horizon=(255, 180, 134)),
    0.31: dict(sky_bot=(240, 134, 145), horizon=(255, 152, 118)),
    0.37: dict(sky_bot=(120, 82, 142), horizon=(214, 84, 96)),
    0.42: dict(sky_bot=(86, 58, 112), horizon=(172, 68, 92)),
    0.47: dict(sky_bot=(59, 42, 86), horizon=(116, 54, 80)),
    0.52: dict(sky_bot=(41, 31, 63), horizon=(72, 44, 72)),
}
...
SPEC = BiomeSpec(
    name='alpine_haze_v5_early_night',
    note='v5 early_night - indigo rises from ground while dome still in sunset',
    keyframes=_KF,
    sky=ALPINE_HAZE.sky,
)
...
def main():
    ...
    title = f_title.render("v5_early_night — indigo rises from the ground up", True, TEXT_HI)
    ...
    out = os.path.join(_ROOT, "docs", "sky_transition", "v5_early_night", "round_1.png")
