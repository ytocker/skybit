"""Round-1 render for tl9 numeral concept: stroke_rects.

Fully procedural digit construction — the font is dropped for the amount
numeral entirely. Each digit is built from filled rectangles on a
seven-segment topology: thick verticals plus thinner horizontals give the
mark a flat-brush proportion without any glyph mask or font measure. The
coin token, finishing-stroke rule, and tag geometry are inherited from the
brushwork template. Review sheet only — not wired live.
"""
import os
...
# Cell: 10x16 device px per digit. Wv=3 (vert stroke width), Wh=2 (horiz stroke
# height). Thick stems + thin bars read as a flat-brush numeral at tag scale.
_CELL_W, _CELL_H = 10, 16
_WV, _WH = 3, 2   # vertical stem width, horizontal bar height


def _seg_rects(cell_w=_CELL_W, cell_h=_CELL_H, wv=_WV, wh=_WH):
    mid = cell_h // 2
    return {
        'T':  (wv,         0,          cell_w - 2*wv, wh),
        'TL': (0,          wh,         wv,            mid - wh),
        'TR': (cell_w-wv,  wh,         wv,            mid - wh),
        'M':  (wv,         mid,        cell_w - 2*wv, wh),
        'BL': (0,          mid+wh,     wv,            cell_h - mid - 2*wh),
        'BR': (cell_w-wv,  mid+wh,     wv,            cell_h - mid - 2*wh),
        'B':  (wv,         cell_h-wh,  cell_w - 2*wv, wh),
    }
...
def _draw_digit(surf, x, y, ch, ink):
    """Draw one digit character at offset (x,y) using rect segments."""
    segs = _seg_rects()
    if ch in _DIGIT_SEGS:
        ...
        # Small comma tick below baseline
        pygame.draw.rect(surf, ink, (x + 1, _CELL_H - 4, 2, 3))
    elif ch == '.':
        pygame.draw.rect(surf, ink, (x + 2, _CELL_H - 3, 2, 2))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 STROKE_RECTS digit treatment — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9/stroke_rects/round_1.png"
