#!/usr/bin/env python3
"""vault-jewel · confirm_purchase_v8 · card-frame-v1 · round_2

Anime/Gacha SSR armoured vault plate. The distinguishing move vs a bleed-to-edge
chromatic full-art is CONTAINMENT: a desaturated gunmetal plate holds a circular
porthole hero FIRMLY enclosed by a heavy gold bezel, bolted with steel rivets.
The TIER colour is one accent hue that lives in the corner nebula, the inner
accent line, and the Zone-B ribbon + gems — never in the neutral gold/grey lane
(porthole bezel, price numeral, rivet steel). LEGENDARY therefore reads
"crimson-tier in a gold vault," matching RARE (blue) and EPIC (purple).
"""
import os, sys, math
...
# The ONE LEGENDARY tier-accent: oxblood (richer + darker than a bright red) so
# the rarity reads crimson-in-a-gold-vault without ever collapsing into the
# neutral gold the price + bezel already own.
LEGENDARY_ACCENT = {"gem": (170, 45, 45), "glow": (150, 32, 32), "deep": (88, 20, 20)}
...
# Fixed starfield so the scatter is deterministic across renders. Mixed grade:
# a few bright 2px stars (a=200) among fainter 1px sparks (a=120), all in the
# plate margins clear of the porthole and copy lanes.
_STARS = [
    (45, 160, 2, 200), (226, 250, 2, 200), (118, 292, 2, 200), (182, 404, 2, 200),
    (72, 235, 1, 120), (32, 300, 1, 120), (56, 372, 1, 120), (208, 165, 1, 120),
    (200, 322, 1, 120), (230, 388, 1, 120), (162, 352, 1, 120), (96, 406, 1, 120),
]


# ── plate body ──────────────────────────────────────────────────────────────
def _nebula(big, rect, rad, pal):
    """Corner-anchored tier haze + starfield, hard-clipped to the plate interior
    so the colour is CONTAINED by the armour. Auras sit in the upper corners,
    clear of the r=59 bezel, so the porthole reads as a contained window rather
    than a source of glow."""
    inner = pygame.Surface((rect.w, rect.h), pygame.SRCALPHA)
    ox, oy = rect.x, rect.y
    sc._alpha_aura(inner, m(45) - ox, m(175) - oy, m(55), pal["glow"], peak=32, layers=12)
    sc._alpha_aura(inner, m(215) - ox, m(195) - oy, m(45), pal["glow"], peak=28, layers=10)
    for sx, sy, ssz, sa in _STARS:
        pygame.draw.circle(inner, (255, 255, 255, sa), (m(sx) - ox, m(sy) - oy), max(1, m(ssz)))
...
def _rivets(big, rect):
    """Bolted steel rivets — the mechanical read that sells the armoured plate.
    Four corners plus a mid-edge pair at the plate seam."""
    inset = m(15)
    pts = [(rect.left + inset, rect.top + inset), (rect.right - inset, rect.top + inset),
           (rect.left + inset, rect.bottom - inset), (rect.right - inset, rect.bottom - inset),
           (rect.left + inset, m(235)), (rect.right - inset, m(235))]
    for px, py in pts:
        pygame.draw.circle(big, (0, 0, 0, 155), (px, py), m(6))
        pygame.draw.circle(big, (150, 155, 170), (px, py), m(5))
        pygame.draw.circle(big, (188, 193, 205), (px - m(1), py - m(1)), max(1, m(2.4)))
        # top-left glint pixel so each bolt catches the plate's light source
        pygame.draw.circle(big, (200, 205, 210), (px - m(2), py - m(2)), max(1, m(1)))


def card_body(big, accent):
    rect = pygame.Rect(m(CARD_X), m(CARD_TOP), m(CARD_W), m(CARD_H)); rad = m(CARD_RAD)
    sc.drop_shadow(big, rect, rad, blur=m(8), alpha=165, dy=m(4))
    # desaturated gunmetal/steel — deliberately NOT the indigo of the shipping
    # CONSTELLATION store cards, so the confirm plate reads as its own object.
    big.blit(vgrad_stops(rect.w, rect.h, rad,
                         [(0.0, (30, 33, 44)), (0.5, (18, 20, 30)), (1.0, (26, 29, 40))],
                         255, gamma=1.12), rect.topleft)
    sc.top_sheen(big, rect, rad, m(30), peak=40)
    _nebula(big, rect, rad, accent)
    # steel bevel over a crisp dark keyline, then a hairline TIER accent inside
    # it so the armour edge is defined but carries the rarity hue.
    pygame.draw.rect(big, (4, 5, 16), rect, width=max(1, m(2)), border_radius=rad)
    sc.bevel_rim(big, rect, rad, (40, 45, 90), (180, 190, 220), w=max(1, m(3)))
    acc = rect.inflate(-m(9), -m(9))
    pygame.draw.rect(big, (*accent["gem"], 150), acc, width=max(1, m(1)), border_radius=rad - m(4))
    _rivets(big, rect)
...
def zone_a(big, price):
    """Raised gunmetal lozenge carrying the coin + PRICE_STOPS gold numeral,
    the coin+numeral GROUP centred on CX."""
    r = pygame.Rect(0, 0, m(160), m(28)); r.center = (m(CX), m(ZA_CY))
    sc._dark_chip_body(big, r, m(11),
                       [(0.0, (52, 56, 92)), (0.5, (38, 42, 78)), (1.0, (54, 58, 94))],
                       (60, 65, 110), (120, 130, 180))
    # measure the numeral first, then centre coin(d=m(24)) + gap(m(6)) + numeral
    # as one group on CX.
    nf = font(20)
    ...
    sc.coin_glyph(big, coin_cx, m(ZA_CY), m(12))
    center = (num_left + bw // 2, m(ZA_CY))
...
def buttons(big):
    """BUY is the primary CTA — a warm-gold 'spend gold' button with a bright
    gold bevel. CANCEL stays muted blue-steel, clearly subordinate."""
    br = m(BTN_RAD)
    for cx_b, lbl, stops, lab_c, kl_c, pk, rw in [
        (m(BUY_CX), "BUY",
         [(0.0, (200, 165, 70)), (1.0, (180, 140, 50))], (54, 34, 8), (255, 236, 188), 62, m(2.2)),
        (m(CAN_CX), "CANCEL",
        ...
        plain_text(big, lbl, font(14 if lbl == "BUY" else 13), r.center, lab_c,
                   shadow_a=90, weight=m(0.9), keyline=kl_c, kw=m(0.9))
...
def hero(big, sid, pal):
    ...
    ONLY BLEND_ADD source at this centre; no aura stacks here. The bezel stays
    NEUTRAL gold on every tier."""
...
def render_popup(tier_word, sid, price, pal):
    ...
    # ONE tier-accent hue drives the nebula, the inner accent line, and the
    # Zone-B ribbon + gems. LEGENDARY swaps to oxblood so it reads crimson-tier
    # in a gold vault, matching RARE/EPIC's tier-in-a-gold-vault read.
    accent = LEGENDARY_ACCENT if tier_word == "LEGENDARY" else pal
    card_body(big, accent)
    name_text(big, NAMES[tier_word])
    zone_a(big, price)
    buttons(big)
    zone_b(big, tier_word, accent)
    hero(big, sid, pal)
...
idr.text((MARGIN, 20), "vault-jewel  ·  confirm_purchase_v8  ·  card-frame-v1  ·  round_2",
         fill=(232, 226, 208))
