"""Round-1 render for tl8 numeral concept: warm_sepia_letterpress.

Letterpress plate deboss: a solid medium-weight numeral printed once, with a
single 1px shadow stamp of the SAME mask offset (0, +1) filled a darker sepia
BEFORE the main blit, so the impression reads as a plate pressed into paper.
No gradients, no halos, no 8-way ring — just the one downward offset. Review
sheet only — not wired live.
"""
import os
...
def _draw_price(face, text, center, affordable):
    """Letterpress deboss: a single darker-sepia shadow of each mask stamped at
    (0, +1) BEFORE the main opaque fill, reading as a plate pressed into paper.
    No gradients, halos, or 8-way rings — only value contrast and the one
    downward offset survive the 2x->1x smoothscale."""
    cx = center[0]
    # Only solid fills + value contrast survive at final card size; the darker
    # sepia shadow gives the deboss its pressed-in depth without any alpha.
    if affordable:
        ink = (58, 38, 22)
        ink_sh = (38, 24, 14)
        coin_col = (200, 156, 50)
    ...
    # line. Same deboss recipe: darker stamp first at (0, +1), then main on top.
    ...
    coin_shadow = coin_mask.copy()
    coin_shadow.fill((*coin_sh, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(coin_shadow, (coin_r.x, coin_r.y + 1))
    coin_fill = coin_mask.copy()
    coin_fill.fill((*coin_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(coin_fill, coin_r)

    # Amount line — same deboss: darker sepia shadow stamped one pixel down
    # before the medium-weight ink fill lands on top.
    amt_mask = _price_glyph(text)
    cy_amount = int(FACE_H * 0.62)
    amt_r = amt_mask.get_rect(center=(cx, cy_amount))
    amt_shadow = amt_mask.copy()
    amt_shadow.fill((*ink_sh, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(amt_shadow, (amt_r.x, amt_r.y + 1))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("WARM-SEPIA-LETTERPRESS price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/warm_sepia_letterpress/round_1.png"
