"""Round-1 render for the `rubber-stamp` tl6 numeral treatment.

Hand-stamped boutique label: warm black-brown ink (38,30,24) with a soft bleed halo,
deterministic interior void punches, and the assembled price rotated +2 degrees off
the tag's own tilt. All tag structure identical to tl2 r2.
"""
import os
...
def _draw_price(face, text, center, affordable):
    ink = (38, 30, 24) if affordable else (72, 76, 90)
    ...
    # Pass 1: bleed halo — expanded glyph at low alpha
    bleed = sc._stamp_bold(mask, sc.m(0.8))
    halo = bleed.copy()
    halo.fill((*ink, 28), special_flags=pygame.BLEND_RGBA_MULT)
    price_surf.blit(halo, (mx, my))

    # Pass 2: body fill
    body = mask.copy()
    body.fill((*ink, 235), special_flags=pygame.BLEND_RGBA_MULT)
    price_surf.blit(body, (mx, my))

    # Pass 3: void punches (deterministic per price text)
    rng = random.Random(hash(text) & 0xFFFFFFFF)
    for vy in range(my, my + mh):
        for vx in range(mx, mx + mw):
            _, _, _, a = price_surf.get_at((vx, vy))
            if a > 100 and rng.random() < 0.08:
                price_surf.set_at((vx, vy), (0, 0, 0, 0))

    # Rotate +2 degrees extra (tag itself will rotate -7 degrees later)
    price_rot = pygame.transform.rotate(price_surf, 2.0)
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("RUBBER-STAMP — tl6 hang-tag round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/rubber_stamp/round_1.png"
