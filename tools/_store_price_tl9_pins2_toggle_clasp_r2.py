"""Round-2 render for tl9 pins2: toggle_clasp (T crossbar + O-ring, stem threads through)"""
import os
...
def _attachment(card, affordable):
    import math
    col = (170, 130, 42) if affordable else (96, 102, 116)
    hi  = (214, 180, 90) if affordable else (140, 146, 160)
    kl  = (70, 50, 16) if affordable else (30, 32, 40)
    # T crossbar at top: bar is the horizontal part
    pygame.draw.rect(card, kl, (8, 6, 16, 5))
    pygame.draw.rect(card, col, (9, 7, 14, 3))
    pygame.draw.line(card, hi, (9, 7), (22, 7), 1)
    # stem hanging down from crossbar center
    pygame.draw.line(card, kl, (15, 11), (15, 19), 4)
    pygame.draw.line(card, col, (15, 11), (15, 19), 2)
    # O-ring threaded by stem: bottom arc first (behind), then stem overdraw, then top arc in front
    ring_rect = pygame.Rect(10, 16, 11, 9)
    pygame.draw.arc(card, kl, ring_rect.inflate(2, 2), math.radians(180), math.radians(360), 3)
    pygame.draw.arc(card, col, ring_rect, math.radians(180), math.radians(360), 2)
    # stem redrawn over ring interior (creates threading illusion)
    pygame.draw.line(card, kl, (15, 16), (15, 21), 4)
    pygame.draw.line(card, col, (15, 16), (15, 21), 2)
    # top arc of ring drawn last → in front of stem
    pygame.draw.arc(card, kl, ring_rect.inflate(2, 2), math.radians(0), math.radians(180), 3)
    pygame.draw.arc(card, col, ring_rect, math.radians(0), math.radians(180), 2)
    pygame.draw.arc(card, hi, ring_rect, math.radians(120), math.radians(180), 1)
...
fh = hud_font(18)
fl = hud_font(12)
title = fh.render(f"tl9 pins2: {SLUG} — round 2", True, (240, 224, 180))
...
out = f"docs/store_price_tl9_pins2/{SLUG}/round_2.png"
