"""Stitch the 5 matured "ACHIEVEMENT EARNED!" list designs into one labeled
comparison figure: docs/achievements/unlock_notice/award_list/showcase.png.

Scratch tooling only; nothing here is imported by the game.

    python tools/build_award_list_showcase.py
"""
import os
...
BASE = os.path.join(_ROOT, "docs", "achievements", "unlock_notice", "award_list")

# (slug, final-round file, one-line thesis) in figure order.
CONCEPTS = [
    ("dispatch-manifest", "round_2.png",
     "Earned items as check-stamped line-items on a torn courier slip."),
    ("postmark-routing-map", "round_1.png",
     "Badges as stops along a completed courier flight-path."),
    ("commendation-card-stack", "round_1.png",
     "A premium vertical stack of gilt-framed achievement cards."),
    ("trophy-cabinet-shelves", "round_2.png",
     "Badges seated in the lit niches of a glass trophy cabinet."),
    ("airmail-filmstrip", "round_2.png",
     "A franked-stamp ribbon header over a clean 3-row list."),
]
...
def main():
    ...
    title = _font(36, True).render(
        "\"ACHIEVEMENT EARNED!\"  ·  5 LIST DESIGNS", True, _GOLD_BRIGHT)
