#!/usr/bin/env python3
"""gem-facet · confirm_purchase_v8 · swap-round-1"""
import os, sys, math
...
def zone_a_chip(big, price_str, pal):
    # Cut-crystal price seat: a low, wide faceted lens. Triangular facets fan
    # from one centre so a single light source value-steps crown->edge, and the
    # bright central facets give the dark numeral its contrast ground.
    cx, cy = m(CX), m(CHIP_CY)
    hw, hh = m(80), m(14)
    gem, glow, deep = pal["gem"], pal["glow"], pal["deep"]
    # sparkle halo bleeds a hair past the girdle so the seat reads as crystal
    sc._alpha_aura(big, cx, cy, m(94), glow, peak=35, layers=8)

    def facet_col(d):                        # d: 0 at centre .. 1 at outer edge
        if d < 0.5:
            return lerp_color(gem, glow, d / 0.5)
        return lerp_color(glow, deep, (d - 0.5) / 0.5)

    # tips sit at mid-height so the silhouette is a stretched lozenge, not a box
    top = [(cx - hw, cy), (cx - hw*0.66, cy - hh*0.7), (cx - hw*0.33, cy - hh),
           (cx, cy - hh), (cx + hw*0.33, cy - hh), (cx + hw*0.66, cy - hh*0.7),
           (cx + hw, cy)]
    bot = [(cx - hw, cy), (cx - hw*0.66, cy + hh*0.7), (cx - hw*0.33, cy + hh),
           (cx, cy + hh), (cx + hw*0.33, cy + hh), (cx + hw*0.66, cy + hh*0.7),
           (cx + hw, cy)]
    C = (cx, cy)
    edge = (8, 6, 18); ew = max(1, m(1)); n = 6
    # bottom half rides one value-step deeper so the lens catches top-down light
    for pts, sink in ((top, 0.0), (bot, 0.26)):
        for i in range(n):
            d = min(1.0, abs((i + 0.5) - n/2) / (n/2) + sink)
            tri = [C, pts[i], pts[i+1]]
            pygame.draw.polygon(big, facet_col(d), tri)
            pygame.draw.polygon(big, edge, tri, ew)

    # coin + numeral engraved across the bright crown facets
    ...
    plain_text(big, price_str, nf, (int(sx + coin_d + gap + num_w/2), int(ty)),
               (14, 12, 26), shadow_a=70, weight=m(0.9))

def zone_b_banner(big, tier_word, pal):
    # Faceted crystal bar spanning the two terminal gems: five value-stepped
    # facet bands with chevron seams so the light reads as cut prism, not a
    # flat gradient. The bottom gems (drawn after) cap it as the same setting.
    ...
    pygame.draw.line(big, lerp_color(gem, WHITE, 0.4), (x0, ytop), (x1, ytop), max(1, m(1)))
...
idr=ImageDraw.Draw(strip); idr.text((MARGIN,18),"gem-facet · swap-round-1",fill=(232,226,208))
...
import pathlib; pathlib.Path("/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/gem-facet").mkdir(parents=True,exist_ok=True)
OUT="/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/gem-facet/round_1.png"
