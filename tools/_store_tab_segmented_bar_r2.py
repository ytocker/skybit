...
def my_draw_tabs(self, surf):
    ...
    the active tab raised as a brighter carved segment so it reads as the most
    forward element, and inactive tabs as dimmer flat labels on the trough."""
    f = _font(12, True)
    pad, gap = 11, 6
    ...
    surf.blit(_vgrad_panel(trough.w, trough.h, 6, (42, 30, 12), (22, 15, 7), 200),
              trough.topleft)
    # Raised border alpha ~170 so the rail holds definition on the real store panel.
    pygame.draw.rect(surf, (*_GOLD_DEEP, 170), trough, width=1, border_radius=6)
    ...
    for i, (label, _g) in enumerate(_TABS):
        ...
        if active:
            ...
            # Brightened fill ensures the active cap out-values every inactive label —
            # the eye should land here, not on the dimmer unselected text.
            seg = pygame.Rect(r.x + 2, _TAB_Y - 10, r.w - 4, 20)
            _drop_shadow(surf, seg, 8, blur=3, alpha=70)
            surf.blit(_vgrad_panel(seg.w, seg.h, 8, (168, 124, 52), (108, 76, 30), 245),
                      seg.topleft)
            pygame.draw.rect(surf, (*_GOLD_BRIGHT, 210), seg, width=1, border_radius=8)
            # Inset bevel highlight 2px below the rim so it reads as a carved ledge
            # rather than just a top-line artefact; alpha 90 makes it visible.
            pygame.draw.line(surf, (*_GOLD_PALE, 90),
                             (seg.x + 3, seg.y + 2), (seg.right - 4, seg.y + 2))
            timg = f.render(label, True, NEAR_BLACK)
            surf.blit(timg, timg.get_rect(center=r.center))
        else:
            timg = f.render(label, True, _GOLD_PALE)
            # alpha ~150 creates a clean three-step ladder:
            # bright active block → dim inactive labels → dark trough.
            timg.set_alpha(150)
            surf.blit(timg, timg.get_rect(center=r.center))

        cx += w + gap

    # Faint 1-px vertical divider ticks between consecutive inactive tabs;
    # the 6-px gap absorbs them without crowding.
    cx = 0
    active_tab = self.tab
    for i, (_, _g) in enumerate(_TABS[:-1]):
        w = widths[i]
        next_inactive = (i != active_tab) and ((i + 1) != active_tab)
        if next_inactive:
            tick_x = round(vp.x + cx + w + gap // 2 - self.tab_scroll)
            s = pygame.Surface((1, trough.h - 4), pygame.SRCALPHA)
            s.fill((255, 220, 120, 40))
            surf.blit(s, (tick_x, trough.y + 2))
        cx += w + gap
...
lf = _font(11, True)
lt = lf.render("SEGMENTED-BAR  round_2", True, (255, 220, 80))
...
out = "/home/user/skybit/docs/store_tab_strip_redesign/segmented-bar/round_2.png"
