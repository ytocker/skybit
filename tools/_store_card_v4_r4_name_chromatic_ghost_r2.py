...
# Stylized baked chromatic-aberration ghost. The fringes lerp hard toward
# fixed warm/cool poles so they stay saturated regardless of the tier gem
# (a soft lerp lets a blue/gold gem swallow one fringe). Tier identity lives
# in a faintly gem-tinted core; the offset runs predominantly horizontal so
# the split reads as a holo lens rather than a doubled drop shadow.
def _name_chromatic_ghost(big, name, cx, cy, max_w, pal):
    offset = max(1, m(1.5))  # 3 device px — survives collapse to 1x display
    voff = round(offset * 0.4)

    # Cap the whole treatment (glyph + both halo lobes) to ~90% of the band
    # interior so the fringes never kiss the bevel rim on long names.
    interior_w = max_w + m(22)
    eff_max = min(max_w, int(0.90 * interior_w) - 2 * offset)

    sz = 13.5
    f = font(sz)
    tracking = m(0.4)
    while _glyph_base(name, f, tracking).get_width() > eff_max and sz > 9.0:
        sz -= 0.5
        f = font(sz)
    base = _stamp_bold(_glyph_base(name, f, tracking), m(0.6))
    bw, bh = base.get_size()

    # Fringes lerp hard toward fixed poles to keep both lobes chromatic even
    # when the gem already sits on the warm/cool axis (RARE blue, LEGENDARY gold).
    warm_col = lerp_color(pal["gem"], (255, 168, 80), 0.83)
    cool_col = lerp_color(pal["gem"], (80, 178, 255), 0.83)

    def tinted(col, alpha):
        img = base.copy()
    ...
    # Cyan reads dimmer than orange on ADD over the dark band, so the cool
    # lobe carries more alpha to weigh equally against the warm one.
    warm_img = tinted(warm_col, 195)
    big.blit(warm_img, base.get_rect(center=(cx - offset, cy - voff)),
             special_flags=pygame.BLEND_ADD)

    cool_img = tinted(cool_col, 215)
    big.blit(cool_img, base.get_rect(center=(cx + offset, cy + voff)),
             special_flags=pygame.BLEND_ADD)

    # Core carries the tier signature: a whisper of gem tint gives RARE a
    # cool-white core and LEGENDARY a warm-white one without muddying the halo.
    core_col = lerp_color((250, 250, 255), pal["gem"], 0.12)
    core = base.copy()
    core.fill((*core_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
...
htxt = hfont.render("store_card_v4_r4_name — chromatic-ghost — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name/chromatic-ghost/round_2.png"
