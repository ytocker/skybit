"""v5 itemdown r1 — item thumbnail nudged down inside the dome.

Everything else is locked (dome position, ring, glass, ribbon, name chip).
Only the item Y changes. Steps are in logical px (2 device px each).

Panels:
  #0  CURRENT    item at cy - _ITEM_DY           (baseline)
  #1  +1 lx      item 1 logical px lower
  #2  +2 lx      item 2 logical px lower
  #3  +3 lx      item 3 logical px lower
  #4  +4 lx      item 4 logical px lower

Output: docs/store_card_v5_card_item/itemdown_r1.png
"""
import os, sys
...
def draw_card_itemdown(surf, sid, rect, extra_dy):
    """draw_card() with item shifted extra_dy device px further down."""
    ...
    sc.blit_thumb(surf, sid, cx, cy - sc._ITEM_DY + extra_dy, sc._BOX_PX)
...
def render_panel(extra_dy):
    ...
    draw_card_itemdown(big, SID, rect, extra_dy)
...
PANELS = [
    (0,  "CURRENT",  "baseline"),
    (2,  "+1 lx",    "1 logical px down"),
    (4,  "+2 lx",    "2 logical px down"),
    (6,  "+3 lx",    "3 logical px down"),
    (8,  "+4 lx",    "4 logical px down"),
]
...
hf   = hud_font(18, True)
htxt = hf.render(
    f"v5 itemdown r1  —  {SID}  —  item nudged down (ring + glass fixed)",
    True, (210, 206, 224))
...
for col, (extra_dy, title, subtitle) in enumerate(PANELS):
    x = MARGIN + col * (PANEL_W + GAP)
    canvas.blit(render_panel(extra_dy), (x, panel_y))
...
out = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_card_v5_card_item", "itemdown_r1.png")
