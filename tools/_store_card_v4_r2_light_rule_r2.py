"""light-rule — store_card_v4_r2 concept, round 2 (final) headless render.
...
Round 2 folds in the art-director's final notes:

  * NO STRIKETHROUGH. In round 1 the rule's hot cream core was drawn AFTER the
    price capsule and speared straight through the coin + digits. The core is
    now GAPPED under the capsule footprint and drawn BEFORE it: only the soft
    additive bloom continues behind the pill, so the capsule reads as CAPPING
    the rule's right terminus rather than being skewered by it.
  * OPEN NEGATIVE SPACE. The disc is lifted (cy = rect.y + m(27)) and its
    radius trimmed to R=30 so the disc TOP stays tangent to the top frame
    exactly as before (no breach) while the disc BOTTOM lifts clear — opening a
    genuine ~10px field of open indigo between the disc and the floating name,
    which the "airy minimal" thesis requires.
  * NO HIDDEN PLATE. The disc's under-glow peak is eased so the body below the
    disc stays open air, not a softly lit frosted band.
  * PRICE-WARD RAMP. The rule brightens toward its capped (price) end and fades
    left, tying the gem + price focal together without reading as a border.
...
store; writes docs/store_card_v4_r2/light-rule/round_2.png.
"""
import os
...
# Hero disc. Trimmed from R=34 -> 30 so the disc can be lifted (cy=m(27)) to open
# the indigo body below it WITHOUT its top bezel breaching the card frame: with
# cy - m(R) held at the same value as round 1, the disc top stays tangent to the
# top edge exactly as before while the bottom lifts clear.
R = 30
...
def _luminous_rule(surf, x0, x1, y, bright, core_x1=None, ramp=True):
    """The baseline itself: a crisp bright core line wrapped in a soft ADDITIVE
    bloom so it reads as emissive light, not a drawn border.

    The bloom spans the FULL width (x0..x1) so the emission reads continuous
    even behind the price capsule that caps the right terminus. The hard bright
    CORE, however, stops at `core_x1` (the capsule's left edge) so the hot cream
    filament never spears the coin + digits — only the bloom bleeds behind them.
    The core also ramps brighter toward its capped (price) end and fades left,
...
def render_card(sid):
    ...
    rad = m(CARD_RAD)
    cx, cy = rect.centerx, rect.y + m(27)          # disc high; open body below it
    ...
    # ── HERO DISC ──
    # Under-glow eased (peak 34 -> 26) so its lower falloff can't pool below the
    # lifted disc and read as a frosted plate; the body below stays open air.
    soft_glow(big, cx, cy, m(R + 2), pal["glow"], 26, layers=9)
    ...
    # ── PRICE cell geometry (glow-capsule straddling the rule's RIGHT END, below
    #    the gem). Sized first so the rule knows where to cap its core. ──
    ...
    cap_h = m(13)
    # capsule sits a hair ABOVE the rule so it reads as capping the terminus, not
    # bisected by it.
    cap_cy = rule_y - m(2)
    cap = pygame.Rect(rule_x1 - cap_w, cap_cy - cap_h // 2, cap_w, cap_h)

    # ── LUMINOUS BASELINE RULE — bloom full width, but the bright core stops at
    #    the capsule's left edge so it never strikes through the price. Drawn
    #    BEFORE the capsule so the pill caps the rule's right terminus. ──
    _luminous_rule(big, rule_x0, rule_x1, rule_y, CARD_RING_BRIGHT,
                   core_x1=cap.left - m(4))

    # ── PRICE capsule ON TOP of the rule terminus ──
...
ffont = _font(18, True)
sfont = _font(15, True)
htxt = hfont.render("store_card_v4_r2 — light-rule — round 2", True,
                    (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r2/light-rule/round_2.png"
os.makedirs(os.path.dirname(out), exist_ok=True)
pygame.image.save(sheet, out)
print("saved", out, sheet.get_size())


# ── L* probes (no image display) — confirm the AD fixes numerically at authored
#    SS: strikethrough gone, real gap below the disc, no hidden plate. ──
def _lstar(rgb):
    def lin(c):
        c /= 255.0
...
# geometry: disc bottom, name top, and the gap of open indigo between them.
disc_bottom_y = cy + m(R)
# name top = topmost cream (L*>60) pixel in the name band/columns.
name_top_y = None
for yy in range(rect.bottom - m(24), rect.bottom - m(4)):
    for xx in range(rect.x + m(10), cx + m(30)):
        if _lstar(epic.get_at((xx, yy))) > 60:
            name_top_y = yy
            break
    if name_top_y is not None:
        break
