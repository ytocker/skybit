...
# the glyph silhouette falling in temperature from the stroke edge outward
# (warm-white -> warm-ivory -> ivory). Normal-alpha (not additive): compositing
# displaces the navy's blue toward the warm hue so the fixed palette can read
# both warm and bright at once, and the tightening/brightening toward the stroke
# makes each letter read as cast from glowing metal with a lit bevel edge.
def _name_rim_forge(big, name, cx, cy, max_w):
    ...
    for char, adv in zip(name, advances):
        ...
        # Layer 1: modest atmospheric champagne halo (baseline-modest, not escalated)
        halo = tile.copy()
        halo.fill((218, 208, 182, 55), special_flags=pygame.BLEND_RGBA_MULT)
        steps = max(16, m(6) * 4)
        ...
        # Layer 2: 3-ring contour bevel — the escalation (warm-white -> warm-ivory -> ivory)
        # These trace the glyph silhouette tightly, reading as a glowing bevel edge.
        BEVEL = [
            (m(3), (250, 244, 225), 120),   # ivory outer bevel ring
            (m(2), (252, 250, 238), 180),   # warm-ivory mid bevel ring
            (m(1), (255, 255, 252), 235),   # warm-white inner bevel (hot-core, escalated)
        ]
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v6 — rim-forge — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v6/rim-forge/round_1.png"
