"""Round-1 render sheet for the kite-cut gem badge concept.

Monkey-patches store_cards.facet_gem with an ASYMMETRIC kite stone — the belly
is pushed high (near the top) with a long lower spike, so it reads as a
top-heavy falling shard rather than the symmetric rhombus of diamond-cut. Four
kite crown facets tile the ring around a proportionally-biased table, shaded off
one top-left key with a warm lower-right reflected return. Renders it in-context
on two full v5 cards plus an 8x zoomed 4-tier gem strip. Review-only tooling —
...
def my_facet_gem(surf, cx, cy, r, base, deep, mystery=False):
    """Kite-cut: an asymmetric kite whose wide belly is lifted high toward the
    apex and whose lower point drops into a long spike. The short upper segment
    and the long lower sliver make the stone read as a top-heavy falling shard —
    the whole differentiator from diamond-cut's symmetric rhombus. Four kite
    crown facets tile the ring to a proportional table; each is shaded by its
    outer-edge normal off one top-left key, with a warm lower-right reflected
    secondary keeping the SE-facing facets alive."""
    if mystery:
    ...
    # Low-sun bounce, not just a pale wash, so cool shadow facets pick up a
    # believable amber return.
    ...
    # ── girdle + table corners (asymmetric kite, belly lifted HIGH) ──
    N = (cx,                  cy - int(r * 1.00))   # top apex
    E = (cx + int(r * 0.82),  cy - int(r * 0.22))   # right belly, above centre
    W = (cx - int(r * 0.82),  cy - int(r * 0.22))   # left belly, above centre
    S = (cx,                  cy + int(r * 0.98))   # long lower spike

    tN = (cx,                  cy - int(r * 0.30))
    tE = (cx + int(r * 0.20),  cy - int(r * 0.06))
    tW = (cx - int(r * 0.20),  cy - int(r * 0.06))
    tS = (cx,                  cy + int(r * 0.34))
    ...
    # Four kite crown facets fully tiling the crown ring. Upper pair are short
    # and lit; lower pair are long slivers running down the spike, darkest.
    kites = [
        ([W, N, tN, tW], unit_mid(W, N)),   # upper-left, most lit
        ([N, E, tE, tN], unit_mid(N, E)),   # upper-right, mid-lit
        ([S, W, tW, tS], unit_mid(S, W)),   # lower-left sliver
        ([E, S, tS, tE], unit_mid(E, S)),   # lower-right sliver, darkest
    ]
    ...
    # ── split specular seated in the upper-left facet: a soft lens (wider than
    # tall) plus a distinct hot pip above it — both additive so they read as
    # glass. No centred/circular highlight, so the asymmetry stays legible. ──
    lx, ly = cx - int(r * 0.24), cy - int(r * 0.30)
    ...
    px, py = cx - int(r * 0.32), cy - int(r * 0.38)
...
def main():
    ...
    header = hf.render("gem badge — kite-cut r1", True, (236, 232, 250))
