"""Round-1 render for tl8 numeral concept: gilded-showcard.

Signwriter's gold-leaf show-card lettering: a fat dark-sepia keyline underlay
is stamped first, then a plain (weight-0) bright-gold face is overlaid at the
exact same rect centre, so the bold underlay peeks as a solid dark ring around
a gilt glyph. Pure value + stroke-weight contrast, zero alpha trickery — the
only thing that survives the 2x->1x smoothscale at tag size. Review sheet only
— not wired live.
"""
import os
...
def _price_glyph(text):
    """Returns (fat_mask, plain_mask) for keyline+face compositing."""
    for fs in (13, 12, 11, 10, 9, 8):
        fat = sc._stamp_bold(sc._glyph_base(text, sc.font(fs), 0), sc.m(1.5))
        if fat.get_width() <= 66:
...
def _draw_price(face, text, center, affordable):
    """Gold-leaf show-card stack: fat sepia keyline underlay stamped first,
    plain bright-gold face overlaid at the same centre so the underlay reads as
    a solid dark ring. G coin token high, gilt amount mid-face, ruled underline.
    """
    cx = center[0]
    # Face is the gilt fill; keyline is the fat dark underlay that rings it.
    # Silver/steel face + near-black keyline for the locked state.
    face_col = (214, 168, 74) if affordable else (176, 180, 196)
    key_col = (58, 40, 24) if affordable else (48, 50, 64)
    rule_col = (58, 40, 24) if affordable else (48, 50, 64)

    # Coin token — small "G", high but under the grommet's clear line. Same
    # keyline-under, gold-over technique as the amount.
    cy_token = int(FACE_H * 0.40)
    tok_fat = sc._stamp_bold(sc._glyph_base("G", sc.font(8), 0), sc.m(1.5))
    tok_plain = sc._glyph_base("G", sc.font(8), 0)
    tok_r = tok_fat.get_rect(center=(cx, cy_token))
    tok_key = tok_fat.copy()
    tok_key.fill((*key_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(tok_key, tok_r)
    tok_face = tok_plain.copy()
    tok_face.fill((*face_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(tok_face, tok_plain.get_rect(center=(cx, cy_token)))

    # Amount line — fat keyline underlay, then plain gold face centred on the
    # same point so the dark ring hugs the gilt glyph.
    fat_mask, plain_mask = _price_glyph(text)
    cy_amount = int(FACE_H * 0.62)
    key_fill = fat_mask.copy()
    key_fill.fill((*key_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(key_fill, fat_mask.get_rect(center=(cx, cy_amount)))
    face_fill = plain_mask.copy()
    face_fill.fill((*face_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(face_fill, plain_mask.get_rect(center=(cx, cy_amount)))
    amt_h = fat_mask.get_height()

    # Ledger underline directly beneath the amount, ruled in the keyline sepia.
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("GILDED-SHOWCARD price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/gilded_showcard/round_1.png"
