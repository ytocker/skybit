"""Promenade STREET FIXTURES / PROPS variety — round 3 candidate-sheet generator.
...
Round-3 revision (art-director ITERATE — NARROW night-cap clamp fix ONLY):

  The 15-design roster, all silhouettes, the layout and the palette are LOCKED and
  UNCHANGED from round 2. The single remaining defect: round 2 still measured a
  190.5-luma hottest lit prop (168 px over the 150 cap). The cause was the
  COMPOSITE: round 2 clamped the lit CORE (<=142) and clamped the additive halo
  FIELD (<=ADD_BUDGET) independently, but where the warm additive halo OVERLAPS the
  lit globe/coal/censer core the two SUM on the deck to ~190.

  FIX (guaranteed, lighting-compositing path only): each lit prop now renders its
  ENTIRE drawing — including the core glow faces, the pulsing coals AND the additive
  _warm_halo — onto its OWN small SRCALPHA layer. BEFORE that layer is blitted to
  the deck it gets a per-pixel hue-preserving luma clamp to <=LIT_COMPOSITE_CEIL
  (146): any pixel whose luma exceeds 146 has its rgb scaled by 146/luma. Because
  the layer already contains the core+halo SUM, clamping the layer captures exactly
  the overlap that produced the 190. The single BLEND_RGB_ADD blit now feeds that
  PRE-CLAMPED layer, never the deck. Result: hottest lit prop <=148 (target ~135-146),
  0 px over the 150 cap, coin (~230) the sole brightest element. No geometry, no
  roster, no day appearance, no layout change — only the night-lighting composite.
...
def _measure_night_cap():
    ...
    prints. Lantern globes, embers, lit pennant cloth AND the additive halos are all
    composited (now through the round-3 per-prop composite luma clamp) onto the night
    BG before measuring, so the scan sees the true core+halo SUM on the deck."""
    night = 0.95
...
def render():
    ...
    y = PAD
    _text(sheet, "SKYBIT PROMENADE — STREET FIXTURES / PROPS (round 3): variety POOLS for the recurring fixtures, grouped by TYPE — shared per-type drawer fed DATA rows",
          PAD, y, 16, (250, 246, 236), bold=True)
    y += 20
    _text(sheet, "Round-3 ITERATE — NARROW night-cap fix ONLY (roster / silhouettes / layout / palette / day appearance all LOCKED from round 2): round 2 still measured a 190.5-luma lit prop because the lit CORE (<=142) and the additive HALO were "
                 "clamped separately, so where the halo OVERLAPS the globe/coal/censer core the two SUMMED to ~190 on the deck. FIX: each lit prop now renders core glow + coals + the additive halo onto its OWN layer, and that whole COMPOSITE layer is "
                 "luma-clamped to <=146 (hue-preserving) BEFORE blitting — capturing the overlap the separate clamps missed. The single BLEND_RGB_ADD feeds the pre-clamped layer, never the deck. Footer audit re-measured on RENDERED pixels: PASS.",
          PAD, y, 9, (188, 186, 200))
    y += title_h - 20
    ...
    y = _true_band(sheet, y, "A.  NIGHT MIX — lit props (lanterns / pennant / braziers / censer) + a bench + crate  [NIGHT]  (every lit px <=146 via the composite clamp, coin sole-brightest)",
                   night_mix, 0.95)
    ...
    for is_night in (False, True):
        ...
        _text(sheet, "NIGHT  (whole lit prop composited then luma-clamped <=146: core+halo overlap can't sum past the cap; cloth cools toward (54,64,96); nothing self-lit past the coin)" if is_night else "DAY",
    ...
    msg = (f"NIGHT-CAP AUDIT (measured on RENDERED deck pixels across t-phases — the true core+halo SUM, after the round-3 per-prop COMPOSITE luma clamp <=146): "
           f"hottest LIT-PROP px luma = {hottest:.1f}  ·  px over {NIGHT_GLOW_CAP} = {over}  "
           f"·  gold-coin core luma = {coin_l:.1f}. "
           f"{'PASS — composite clamp holds every lit prop px <= cap with margin; coin is the SOLE brightest.' if coin_sole else 'FAIL — '+str(over)+' px breach the cap / coin not sole-brightest.'}")
    _text(sheet, msg, PAD, total_h - 16, 9,
          (170, 200, 180) if coin_sole else (220, 140, 130))

    out = "/home/user/skybit/docs/sidewalk_overhaul/props/round_3.png"
