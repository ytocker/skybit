"""Round-1 render for tl9 numeral concept: axis_crush.

Flat-brush width compression. The amount is stamped bold, then squeezed to 78%
of its natural width so the vertical strokes dominate — a condensed, calligraphic
flat-brush proportion where the tall axis of each numeral reads first. The ledger
rule stays the brushwork finishing stroke (a height-tapering wedge) and the 'G'
token stays a lighter burnt-copper stamp so the crushed amount is the dominant
mark. Review sheet only — not wired live.
"""
import os
...
def _price_glyph(text):
    """Flat-brush proportion: crush width to 78%, tall strokes dominate."""
    for fs in (15, 14, 13, 12, 11, 10, 9, 8):
        ...
        crushed = pygame.transform.smoothscale(
            raw, (max(1, int(raw.get_width() * 0.78)), raw.get_height()))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 AXIS_CRUSH digit treatment — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9/axis_crush/round_1.png"
