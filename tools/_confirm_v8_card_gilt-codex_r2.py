#!/usr/bin/env python3
"""gilt-codex · confirm_purchase_v8 · card-frame-v1 · round_2
...
round_2 seats the hero disc DOWN inside the arch window (the arch is the frame,
the disc is the portrait) so the gothic niche finally reads; caps the visible
crown with the keystone gem; warms the cabochon to aged umber; seats the coin
and darkens CANCEL for separation on parchment; fills the mid-leaf void with an
engraved double-rule; and restrains the wax-seal aura so the oxblood halo never
tints the surrounding vellum.
"""
import os, sys, math
...
# Hero disc pulled DOWN into the card body so it sits INSIDE the arch window
# (top=126 just clears the arch crown at 132) — the niche now shoulders up
# around the portrait instead of the disc covering it.
DISC_CY, DISC_R = 170, 44
ZA_CY = 247; ZB_CY = 402
...
# Aged-vellum umber for the cabochon well — a cold indigo disc fought the warm
# parchment, so the glass body is deep warm umber that reads as the same leaf in
# shadow. The skin sits inside it, the glass dome + gold bezel land on top.
CABO_UMBER_LO, CABO_UMBER_HI = (40, 30, 18), (20, 14, 8)
...
def gothic_arch(big):
    """Beveled gold arch that FRAMES the portrait: dark seat arc, bright gold,
    pale inner glint, plus short jamb feet at the springline. Drawn BEFORE the
    disc — the arch is the niche, the disc is the portrait seated inside it. The
    dome crowns at y≈131 and both jambs spring from y=200, so the visible gothic
    window rises above and around the hero."""
    seat = pygame.Rect(m(29), m(131), m(202), m(138))
    bright = pygame.Rect(m(30), m(132), m(200), m(136))
    glint = pygame.Rect(m(32), m(134), m(196), m(132))
...
def hero(big, sid, pal):
    ...
    _alpha_aura(big, cx, cy, m(30), pal["glow"], peak=22, layers=10)
    # Warm umber glass well instead of the cold indigo cabochon so the disc reads
    # as parchment-in-shadow, not a foreign blue jewel on the vellum.
    sc.cabochon(big, cx, cy, r, CABO_UMBER_LO, CABO_UMBER_HI, ring=pal["gem"], ring_a=50)
    ...
    # Keystone gem caps the arch crown at its true apex (now on visible parchment,
    # since the disc sits down in the window) — the classic capstone of the niche.
    facet_gem(big, m(130), m(132), m(11), pal["gem"], pal["deep"])


def nameplate(big, name):
    plain_text(big, name, font(18), (m(CX), m(230)), INK, shadow_a=70, weight=m(0.8))
...
def price_ledger(big):
    ...
    coin_cx = m(CX - 44)
    # Dark seat well behind the coin: gold-on-gold turned the glyph to mush, so a
    # slightly larger umber disc sinks the coin into the ledger with a shadow rim.
    seat = pygame.Surface((m(34), m(34)), pygame.SRCALPHA)
    pygame.draw.circle(seat, (30, 20, 10, 180), (m(17), m(17)), m(16))
    big.blit(seat, (coin_cx - m(17), m(ZA_CY) - m(17)))
    coin_glyph(big, coin_cx, m(ZA_CY), m(11))
    plain_text(big, PRICE, font(19), (m(CX + 22), m(ZA_CY)), (40, 25, 10),
               shadow_a=0, weight=m(0.7))


def engraved_rule(big, pal):
    """Fills the mid-leaf void between ledger and buttons with an engraved
    double-rule + gem termini — a manuscript flourish so the parchment never
    reads as a blank gap."""
    rule = pygame.Surface(big.get_size(), pygame.SRCALPHA)
    for yy in (m(289), m(291)):
        pygame.draw.line(rule, (120, 95, 35, 160), (m(30), yy), (m(230), yy), max(1, m(1)))
    big.blit(rule, (0, 0))
    for tx in (m(30), m(230)):
        facet_gem(big, tx, m(290), m(4), pal["deep"], (20, 12, 6))
...
def buttons(big):
    ...
    # Muted dark taupe "closed book" so CANCEL separates from the warm parchment
    # instead of near-vanishing into it; pale label carries the read.
    sc.chip_body_stops(big, can, m(BTN_RAD),
                       [(0.0, (120, 110, 95)), (1.0, (80, 72, 60))],
                       (44, 38, 30), (150, 140, 122), gloss=60)
    plain_text(big, "CANCEL", font(13), can.center, (222, 214, 194), shadow_a=90,
               weight=m(0.8))


def wax_seal_footer(big, tier_word, pal):
    zb = OXBLOOD if tier_word == "LEGENDARY" else pal
    # Restrained aura peak so the oxblood halo stays a soft seal glow and never
    # bleeds crimson into the surrounding vellum.
    for gx in (m(BOT_GEM_L_X), m(BOT_GEM_R_X)):
        _alpha_aura(big, gx, m(ZB_CY), m(16), zb["glow"], peak=35, layers=14)
...
idr.text((MARGIN, 18), "gilt-codex · card-frame-v1 · round_2", fill=(236, 220, 176))
