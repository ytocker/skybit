"""Round-2 render for tl6 numeral concept: tinted-deboss.

Three-pass relief: shadow groove shifted up-left, highlight face shifted
down-right, cocoa fill centered. Same cream tones as the tag body but
offset enough to read as a pressed-in letterform. Works entirely within
the tag palette — no external colour import.

Locked fill darkened to the low end of the grey family so the numeral
clears contrast against the mid-gradient body, and both relief passes
run stronger so the wall offsets — not fill contrast alone — carry the
read. Offset stays at one virtual pixel: strength comes from alpha, not
distance, so it presses rather than blurs.
...
def _draw_price(face, text, center, affordable):
    """Three-pass tinted deboss: shadow groove (-o,-o), highlight rim
    (+o,+o), cocoa fill at center. The offset pair reads as the side
    walls of a letterform pressed into the cream stock. Locked fill sits
    at the dark end of the grey family so it clears the mid-gradient."""
    ink_fill = (96, 72, 54) if affordable else (64, 68, 84)
    highlight = (250, 242, 218) if affordable else (210, 214, 226)
    shadow = (70, 50, 36) if affordable else (54, 58, 72)
    ...
    o = max(1, sc.m(1))
    sh = mask.copy()
    sh.fill((*shadow, 205), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(sh, (r.x - o, r.y - o))
    hi = mask.copy()
    hi.fill((*highlight, 205), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(hi, (r.x + o, r.y + o))
    fill = mask.copy()
    fill.fill((*ink_fill, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(fill, r)
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("TINTED-DEBOSS price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/tinted_deboss/round_2.png"
