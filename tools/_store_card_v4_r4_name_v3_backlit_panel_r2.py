...
def _name_backlit_panel(big, name, cx, cy, max_w):
    sz = 13.5
    f  = font(sz)
    while _glyph_base(name, f, 0).get_width() + m(28) > max_w and sz > 9:
        sz -= 0.5
        f   = font(sz)
    ...
    # Start at champagne-range fill (not white) so the additive center bloom
    # can visibly lift the hot-spot above the edges — yielding a real lightbox
    # reading instead of a flat plaque.  Edges ~215, centre will hit ~255.
    panel = vgrad_stops(pw, ph, rad,
                        [(0.0, (228, 218, 188)), (1.0, (210, 200, 168))], 255)

    # Central lamp: peak_alpha=200, multiple layers → centre rises to near-white
    # while edges stay champagne — the eye reads emitted light.
    soft_glow(panel, pw // 2, ph // 2, m(12), (255, 255, 252), 200, layers=14)
    ...
    big.blit(panel, prect.topleft)

    # Directional bevel: top-left warm-white (lit face), bottom-right deep pewter
    # (shadow undercut) — one clear light source gives the bezel physical relief.
    bevel_rim(big, prect, rad, (88, 82, 66), (255, 255, 252), w=max(1, m(1.5)))

    # Letter faces in champagne/pewter (205,190,150) so they separate tonally
    # from the near-white panel centre — "embossed metal on a lit lightbox."
    # Near-black keyline anchors each glyph against the bright field.
    plain_text(big, name, f, (cx, cy), (205, 190, 150), shadow_a=0,
               weight=m(0.9), keyline=(8, 8, 20), kw=m(1.0))
...
def render_card(sid):
    ...
    _name_backlit_panel(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26))
    return big
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v3 — backlit-panel — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v3/backlit-panel/round_2.png"
