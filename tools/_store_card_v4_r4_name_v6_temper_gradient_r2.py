...
def _name_temper_gradient(big, name, cx, cy, max_w):
    sz = 13.5
    f = font(sz)
    ...
    # DISCRETE TEMPERATURE STOPS: outer→inner, all normal-alpha (never BLEND_ADD),
    # each glyph haloed INDEPENDENTLY so the navy between letters survives. Three
    # legible bands only — gold outer stop / champagne / white core. The near-
    # duplicate warm-white m(1) rim is dropped (invisible against ivory at 1x).
    #
    # Radii are held tight (<= half the tightest ~5px 1x gap, i.e. <= m(2.5)
    # device) so the bright rings can't bridge inter-glyph gaps from both sides
    # and wash them milky grey. The gold stop stays a discrete OUTERMOST band,
    # but slim — its reach is one step past champagne, not a bloom.
    #
    # DARK_STEP is a 1px navy keyline blitted BETWEEN gold and champagne so the
    # gold reads as an intentional temperature stop, not a smear into champagne.
    RINGS = [
        (m(2.5), (236, 202, 116), 66),   # CARD_RING_BRIGHT — discrete gold outer stop
        (m(2.0), (12, 10, 26), 150),     # dark micro-step keyline (navy)
        (m(1.5), (228, 204, 158), 92),   # champagne (warmed: R up / B down)
        (m(1.0), (250, 244, 225), 180),  # ivory white-core rim
    ]
...
def render_card(sid):
    ...
    _name_temper_gradient(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26))
    return big
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v6 — temper-gradient — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v6/temper-gradient/round_2.png"
