...
def _name_arcade_cap(big, name, cx, cy, max_w):
    # Poster-flat cap: shrink to fit, then a single warm-white top lip under a
    # flat cream fill + crisp dark keyline. No metal, gradient, or glow — the
    # highest-legibility read in the set, so the treatment stays deliberately bare.
    sz = 13.0
    while sz > 9.0:
        f = font(sz)
    ...
    # ONE top-edge bevel lip — a warm-white glyph nudged up only, so the cap
    # catches light along its top rim without a second offset or a ramp.
    base = _glyph_base(name, f, 0)
    base = _stamp_bold(base, m(1.1))
    base.fill((255, 252, 244, 255), special_flags=pygame.BLEND_RGBA_MULT)
    lift = max(1, m(0.8))
    big.blit(base, (cx - base.get_width() // 2, cy - base.get_height() // 2 - lift))

    plain_text(big, name, f, (cx, cy), (250, 248, 240), shadow_a=120,
...
htxt = hfont.render("store_card_v4_r4_name_v2 — arcade-cap — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/arcade-cap/round_1.png"
