"""Promenade PLANTS & GREENERY — round 6 candidate-sheet generator.
...
THE 5 NEW SPECIES (structural/foliage — distinct silhouettes, distinct from all
15 existing AND from each other):
  P16 LOTUS  — flat round lily-pad discs floating + 1-2 upright pink lotus
               blooms/buds on tall stems, in a wide shallow WATER basin (a NEW
               low water-vessel). The flat round pads are the signature.
  P17 FERN   — an arching shuttlecock rosette of finely-divided feathery fronds
               (arched blades with pinnae notches), cool deep green, in a stone
               trough. Lacy arched silhouette, no flowers.
  P18 CYCAD  — a stiff symmetric crown of rigid radiating spear-fronds from a
               fat scaly trunk-knob, architectural + prehistoric, in a glazed
               urn. A stiff radial STAR vs the fern's soft arch.
  P19 BANANA — 2-3 huge paddle leaves (big flat blades, strong central midrib +
               a torn/notched edge), tropical, in a wooden tub. The oversized
               broad paddles are the read — the biggest leaf-mass in the family.
  P20 SCHOLAR'S ROCK (gongshi) — a tall pierced/eroded grey Taihu limestone rock
               standing in a low tray with a small moss/fern tuft at its base.
               The literati-garden OBJECT — a NON-plant-dominant design (the ROCK
               is the silhouette). Chosen over the kokedama (see note below).

#5 CHOICE — ROCK, not kokedama: the family already carries many rounded green
domes (shrub, two topiary balls, kumquat, peony, mum, vine), so a green moss
SPHERE on a stand would read as "yet another green dome" and would be near-
indistinguishable from the topiary ball at true far size. The Taihu rock's tall,
asymmetric, PIERCED grey monolith — with sky-holes punched clean through it — is
unlike any silhouette in the 15 and is the only non-plant-dominant object, which
is exactly the structural contrast this batch wants. So the gongshi reads more
distinctly and ships.
...
Sheet layout MIRRORS round 5: true far-lane DAY + NIGHT bands with an adult +
...
shipped siblings + human cast + a stall + the coin, and the _measure_night_cap()
audit footer (PASS only when hottest greenery <=150 and the coin ~230 stays
sole-brightest).
"""
...
def _sp_lotus(surf, cx, rim_y, v, night, t):
    """LOTUS (lianhua) — flat round lily-pad DISCS floating on a wide shallow
    water basin, with 1-2 upright pink lotus blooms/buds on tall stems rising
    clear above the pads. The flat round PADS are the signature read (a cluster
    of overlapping low ellipses sitting AT water level, each with a radial vein
    notch + a paler rim), distinct from every dome canopy in the family because
    the mass is HORIZONTAL, not a vertical bush. The bloom is a layered pink
    petal cup on a slim green stem; one open flower + one closed bud so the
    pink reads as the focal without flooding the water. The basin water-glaze
    is the lowest, coolest surface — held under the pads so the pink pops."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    ripple = math.sin(t * 1.1) * 0.5
    # the floating pads: wide flat overlapping discs at water level. Each is a low
    # ellipse (pad), a paler inner ring (lit upper face), and a thin radial vein
    # notch cut to the rim so the round pad reads as a leaf, not a puddle.
    pads = ((-7, 1, 7, 3), (6, 1, 6, 3), (-1, 0, 8, 3), (8, 2, 4, 2), (-9, 2, 4, 2))
    for dx, dy, rw, rh in pads:
        px = cx + dx + int(ripple * (dy <= 0))
        py = base + dy
        pygame.draw.ellipse(surf, dark, (px - rw, py - rh, rw * 2, rh * 2))
        pygame.draw.ellipse(surf, mid, (px - rw + 1, py - rh + 1, rw * 2 - 2, rh * 2 - 1))
        pygame.draw.ellipse(surf, top, (px - rw + 2, py - rh + 1, rw * 2 - 4, max(1, rh - 1)))
        # the signature radial vein notch — a thin dark slit from centre to rim
        pygame.draw.line(surf, dark, (px, py), (px - rw + 1, py), 1)
    # tall slim stems lifting one open bloom + one bud clear above the pads
    ...
    # OPEN bloom on the taller stem (slight nod with the ripple)
    obx = cx - 4
    oby = base - 16 + int(ripple)
    pygame.draw.line(surf, _shade(stem, -16), (obx, base - 1), (obx, oby + 2), 2)
    pygame.draw.line(surf, _shade(stem, 18), (obx, base - 1), (obx, oby + 2), 1)
    # a layered tulip-cup of pink petals: dark seat, three petal lobes, lit edge
    pygame.draw.polygon(surf, pet_dk, [
        (obx - 4, oby + 2), (obx, oby - 5), (obx + 4, oby + 2), (obx, oby + 1)])
    for sx, hgt in ((-3, 4), (0, 6), (3, 4)):
        tipx = obx + sx
    ...
    pygame.draw.circle(surf, pet_lt, (obx - 1, oby - 2), 0)
    # CLOSED bud on the shorter stem — a tight pink teardrop pointing up
    bbx = cx + 6
    bby = base - 12 - int(ripple)
    pygame.draw.line(surf, _shade(stem, -16), (bbx, base - 1), (bbx, bby + 1), 2)
    pygame.draw.line(surf, _shade(stem, 18), (bbx, base - 1), (bbx, bby + 1), 1)
    pygame.draw.polygon(surf, pet_dk, [(bbx - 2, bby + 2), (bbx, bby - 4), (bbx + 2, bby + 2)])
    pygame.draw.polygon(surf, pet, [(bbx - 1, bby + 1), (bbx, bby - 3), (bbx + 1, bby + 1)])
    pygame.draw.circle(surf, pet_lt, (bbx, bby - 1), 0)


def _sp_fern(surf, cx, rim_y, v, night, t):
    """FERN (Boston / sword fern) — an arching SHUTTLECOCK rosette of finely-
    divided fronds fountaining out + over from a central crown. Each frond is a
    feathery arched blade: a curved rachis (poly-line bowing outward then dropping
    at the tip) hung with short pinnae notches down both sides, so the silhouette
    reads LACY and ARCHED — soft, no flowers, no dome. Inner fronds stand more
    upright, outer fronds arch lower + droop at the tips (the fountain). Cool deep
    green in three tones (shadow / body / lit upper edge); the new tips uncurl
    pale. Distinct from the cycad (this ARCHES + is feathery) and from the bamboo
    (this is a tight ground rosette, not tall canes)."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    crown_x = cx
    ...
    # frond set: (launch angle, length, arch amount, +0/1 droop-tip). Symmetric
    # pairs fanning from the crown — steep inner, low arching outer (shuttlecock).
    fronds = (
        (2.55, 16, 0.9, 1), (0.59, 16, 0.9, 1),     # low outer, big arch + droop
        (2.25, 17, 0.7, 1), (0.89, 17, 0.7, 1),     # mid-low
        (1.95, 17, 0.5, 0), (1.19, 17, 0.5, 0),     # mid
        (1.71, 16, 0.35, 0), (1.43, 16, 0.35, 0),   # near-upright inner
        (1.5708, 18, 0.25, 0),                      # central spire
    )
    for ang0, ln, arch, droop in fronds:
        ...
        pygame.draw.lines(surf, col_main, False, pts, 1)
        # pinnae notches: short barbs alternating down both sides of the rachis
        for s in range(1, steps):
            (x0, y0) = pts[s]
            (x1, y1) = pts[s + 1]
            dxs, dys = x1 - x0, y1 - y0
            # perpendicular barb, length tapering toward the tip
            blen = max(1, 3 - s // 2)
            nx, ny = -dys, dxs
            nl = math.hypot(nx, ny) or 1.0
            nx, ny = nx / nl, ny / nl
            side = 1 if s % 2 == 0 else -1
            bx = x0 + int(round(nx * blen * side))
            by = y0 + int(round(ny * blen * side))
            pygame.draw.line(surf, dark, (x0, y0), (bx, by), 1)
            pygame.draw.line(surf, dark, (x0, y0),
                             (x0 - int(round(nx * blen * side)), y0 - int(round(ny * blen * side))), 1)
...
def _sp_cycad(surf, cx, rim_y, v, night, t):
    ...
    STARBURST — the opposite of the fern's soft droop and unlike any dome. The
    fat barrel trunk-knob at the base is cross-hatched with leaf-scar scales. Deep
    glossy green; lit comb-edges catch a cool highlight up top. Sits in a glazed
    urn so the heavy crown reads as borne on a solid vessel."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    ...
    # is a STRAIGHT line with comb-teeth; the topmost catch a lit edge.
    n = 11
    a_lo, a_hi = math.radians(8), math.radians(172)
    for i in range(n):
        ang = a_lo + (a_hi - a_lo) * (i / (n - 1)) + breathe * math.cos((i / (n - 1)) * math.pi)
        ln = 17 if (0.30 < (i / (n - 1)) < 0.70) else 14  # the central spears longest
        ex = crown_x + int(round(math.cos(ang) * ln))
        ey = crown_y - int(round(math.sin(ang) * ln))
        lit = (i / (n - 1)) > 0.30 and (i / (n - 1)) < 0.70
...
def _sp_banana(surf, cx, rim_y, v, night, t):
    """BANANA / broadleaf (Musa) — 2-3 HUGE paddle leaves: big flat blades with a
    strong central midrib + a torn / notched edge, the biggest leaf-mass in the
    family. Tropical. Each leaf is a long lozenge blade (two laminas meeting at a
    bold midrib), the windward edge cut with the species' characteristic tear-
    notches, splaying from a short trunk. One huge leaf leans each way + one
    upright furled spike (the unfurling new leaf) so the read is oversized
    paddles, not a bush of small leaves. Deep tropical green, lit upper lamina;
    the torn edges + the heavy midrib are the signature, distinct from the cycad's
    spiky star + the fern's lace."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
...
def _sp_rock(surf, cx, rim_y, v, night, t):
    """SCHOLAR'S ROCK (gongshi / Taihu stone) — a tall, asymmetric, pierced grey
    limestone monolith standing in a low tray, with a small moss/fern tuft at its
    base. The literati-garden OBJECT: the ROCK is the silhouette, NOT a plant.
    The stone is a craggy vertical mass (an irregular fang leaning slightly, with
    a knob shoulder), modelled in three cool greys (shadow / body / lit ridge),
    and PIERCED with two clean through-holes (the signature swiss-cheese
    dissolution pockets of Lake Tai limestone) that show the background through
    the rock. A little green moss skirt + a couple of tiny fern fronds at the foot
    ground it as a planting. Distinct from everything: a hard inorganic grey
    vertical, the only non-plant-dominant design in the family."""
    P, A = v.palette, v.attrs
    base = rim_y
...
def _draw_basin(surf, cx, base_y, v, night):
    ...
    whose mouth is filled with a cool water glaze (the pads float on it). The
    flattest, widest vessel; the water surface IS the rim the pads sit on. The
    rim band is lifted one value step so the basin survives the NIGHT ground
    band, and the water glaze is held cool + low (under the pink bloom)."""
    P, A = v.palette, v.attrs
    ...
    # the WATER glaze filling the mouth — a cool inset surface (held low/cool);
    # a couple of pale ripple ticks so it reads as water, not a flat lid.
    water = _hi(_retint(P.get("glaze", (70, 104, 150)), night), 8, night)
    water_lt = _hi(water, 18, night)
    pygame.draw.rect(surf, water, (cx - top_w // 2 + 2, ty + 1, top_w - 4, 2))
    for dxp in (-8, -1, 6):
        pygame.draw.line(surf, water_lt, (cx + dxp, ty + 1), (cx + dxp + 3, ty + 1), 1)
    return ty + 1
...
# cool deep-green fern bank (shade-loving, blue-green)
_FOL_FERN = dict(foliage_dark=(28, 70, 54), foliage_mid=(46, 102, 72), foliage_top=(96, 150, 110))
...
POOL = [
    ("P16 water-basin LOTUS (lianhua)", _row(
        _STONE, _FOL_LOTUS, dict(glaze=(70, 104, 150), stem=(96, 138, 84),
                                 accent=(228, 150, 178), accent2=(232, 206, 120)),
        vessel="basin", species="lotus", vlift=0.10, day_chroma=176),
     "vessel:basin(NEW wide shallow WATER trough, cool inset glaze + ripple ticks, rim lifted for night) species:lotus accent:pink petal-cup + accent2:gold seed-cup | flat round floating PADS (horizontal mass, radial vein notch) + 1 open bloom & 1 bud on tall stems — the pads are the signature, unlike any dome"),

    ("P17 stone-trough FERN (Boston)", _row(
        _STONE, _FOL_FERN, dict(vlift=0.10),
        vessel="trough", species="fern"),
     "vessel:trough(shipped stone) species:fern | an arching SHUTTLECOCK rosette of feathery fronds — curved rachis poly-lines hung with pinnae notches both sides, steep inner / low-arching drooping outer; lacy + soft + ARCHED, no flowers — distinct from the cycad's stiff star + the bamboo's tall canes"),

    ("P18 glazed-urn CYCAD (sago)", _row(
        _URN, _FOL_CYCAD, dict(trunk=(106, 86, 58)),
        vessel="urn", species="cycad"),
     "vessel:urn(blue-white belly+neck) species:cycad trunk:fat scaly leaf-scar KNOB | a stiff SYMMETRIC crown of rigid straight spear-fronds radiating in a flat STAR, each lined with hard comb-teeth + a spine tip; architectural / prehistoric — the hard radial opposite of the fern's soft arch"),

    ("P19 wooden-tub BANANA (Musa)", _row(
        _TUB, _FOL_BANANA, dict(stem=(150, 176, 96)),
        vessel="tub", species="banana"),
     "vessel:tub(staved barrel, iron hoops) species:banana | 2 HUGE arching paddle leaves (lozenge blades, bold central MIDRIB, torn/notched windward edge, lit upper lamina) + 1 upright furled new-leaf spike — the biggest, broadest leaf-mass in the family; tropical, no flowers"),
    ...
    ("P20 literati-tray SCHOLAR'S ROCK (gongshi)", _row(
     ...
     "vessel:tray(NEW low dark-wood plinth + foot-blocks — rock is MOUNTED, not potted) species:rock | a tall asymmetric PIERCED grey Taihu limestone fang (3 cool greys, fluted erosion grooves, 2 clean THROUGH-HOLES showing bg) with a moss skirt + tiny fern fronds at the foot — the literati OBJECT, the only non-plant silhouette (chosen over kokedama: a moss SPHERE would read as yet-another-dome)"),
]
...
# ════════════════════════════════════════════════════════════════════════════
# SHEET RENDERER  (mirrors tools/_greenery_round5.py house style)
# ════════════════════════════════════════════════════════════════════════════
...
def render():
    ...
    y = PAD
    _text(sheet, "SKYBIT PROMENADE — GREENERY POOL EXPANSION (round 6): batch 2 of 4 — FIVE NEW STRUCTURAL / FOLIAGE species (pool indices 15-19); the shipped 15 UNCHANGED",
          PAD, y, 17, (250, 246, 236), bold=True)
    y += 22
    _text(sheet, "Structural/foliage theme (contrast to batch 1's flowers): P16 LOTUS (flat floating pads + pink bloom in a NEW water BASIN) · P17 FERN (arching shuttlecock rosette, stone trough) · "
                 "P18 CYCAD (stiff radial spear-star + scaly knob, glazed urn) · P19 BANANA (huge torn-edge paddle leaves, wooden tub) · P20 SCHOLAR'S ROCK (pierced grey Taihu gongshi + moss foot on a NEW literati tray — chosen over kokedama). "
    ...
    _text(sheet, "C.  ON-STREET COMPOSITE — the 5 NEW species at true size INTERLEAVED with shipped siblings (refs) + human cast + a stall, with the coin reference  (DAY then NIGHT)",
          PAD, y, 12, (240, 220, 150), bold=True)
    y += 18
    for is_night in (False, True):
        night = 0.95 if is_night else 0.0
        ...
        # design index; NEW = a P16..P20 row). Siblings span flowers + form so the
        # new structural family reads beside the whole shipped cast.
        draw_greenery(strip, 40, base, POOL[0][1], night, 0.4)     # P16 lotus
        _shipped_ref(strip, 92, base, night, 1, 0.4)               # ref G2 conifer
        ...
        _text(strip, "(P-prefixed = NEW; others = shipped refs)", 80, 2, 8,
    ...
    out = "/home/user/skybit/docs/sidewalk_overhaul/greenery/round_6.png"
