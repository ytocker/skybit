"""tl5 `cream-ruled` hang-tag — round 2 render harness.

Round-2 refinements over r1: the pin ring is now shaded by angular
interpolation (smooth metal wrap instead of two flat fans), the specular pip
is relocated to the metal rim where it reads, the numeral gradient no longer
collapses into the keyline, the cartouche gains a stamped/recessed bevel, and
the cord is twisted from two offset strands. Headless render only.
"""
...
def _grommet(face, gx, gy, r_out, r_bore, r_void, col_metal, col_hi, col_shadow, col_bore):
    ...
    # the violet page backdrop through the eyelet. The ring is shaded by walking
    # 4° slices and lerping hi→metal→shadow by angular distance from the light
    # peak (135°), so the metal wraps smoothly instead of banding at two fans.
    for a_start in range(0, 360, 4):
        a_mid = a_start + 2
        dist = min(abs(a_mid - 135), 360 - abs(a_mid - 135))
        t = dist / 180.0
        if t < 0.5:
            t2 = 1.0 - 2.0 * t
            col = tuple(int(col_metal[c] + t2*(col_hi[c]-col_metal[c])) for c in range(3))
        else:
            t2 = (t - 0.5) * 2.0
            col = tuple(int(col_metal[c] + t2*(col_shadow[c]-col_metal[c])) for c in range(3))
        pts = [(gx, gy)]
        for a in range(a_start, a_start+5):
            pts.append((int(gx + r_out * _math.cos(_math.radians(a))),
...
def _cord_twist(surf, p0, p1, col_a, col_b, seg_dp, half_lw):
    dx, dy = p1[0]-p0[0], p1[1]-p0[1]
    length = _math.hypot(dx, dy)
    if length < 1: return
    nx, ny = -dy/length, dx/length
    n = max(1, int(length/seg_dp))
    for i in range(n):
        t0, t1 = i/n, (i+1)/n
        x0, y0 = p0[0]+dx*t0, p0[1]+dy*t0
        x1, y1 = p0[0]+dx*t1, p0[1]+dy*t1
        # alternate which strand is light vs dark so the pair reads as a twist
        colA = col_a if i%2==0 else col_b
        colB = col_b if i%2==0 else col_a
        pygame.draw.line(surf, colA,
                         (int(x0+nx*half_lw),int(y0+ny*half_lw)),
                         (int(x1+nx*half_lw),int(y1+ny*half_lw)), max(1,half_lw))
        pygame.draw.line(surf, colB,
                         (int(x0-nx*half_lw),int(y0-ny*half_lw)),
                         (int(x1-nx*half_lw),int(y1-ny*half_lw)), max(1,half_lw))
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    # ── cartouche (recessed / stamped panel) ─────────────────────────────────
    ...
    if affordable:
        # deboss: top inner edge falls into shadow, floor catches light at bottom
        pygame.draw.line(face, (150,120,80), (5,41), (FACE_W-5,41), 1)
        pygame.draw.line(face, (230,214,178), (5,77), (FACE_W-5,77), 1)

    # ── grommet ────────────────────────────────────────────────────────────
    if affordable:
        _grommet(face, sc.m(GX), sc.m(GY), sc.m(6.5), sc.m(3), sc.m(2),
                 (194,152,48), (244,212,88), (60,38,8), (20,14,4))
        # specular pip sits on the upper-left metal rim (not the bright highlight)
    ...
    if affordable:
        _draw_price(face, text, price_center,
                    col_key=(76,44,8),
                    col_fill_stops=[(0.0,(224,176,64)),(0.35,(190,144,40)),(0.7,(120,72,20)),(1.0,(110,66,20))],
                    fs=15)
    else:
        _draw_price(face, text, price_center,
    ...
    cord_a = (210,178,118) if affordable else (140,144,158)
    cord_b = (150,118,70) if affordable else (100,104,118)
    _cord_twist(surf, (gx_world, gy_world), knot, cord_a, cord_b, sc.m(3), sc.m(1))
    # knot nub
    pygame.draw.circle(surf, cord_a, (int(knot[0]), int(knot[1])), sc.m(2))
    pygame.draw.circle(surf, (min(255,cord_a[0]+40), min(255,cord_a[1]+40), min(255,cord_a[2]+40)),
...
ht = hf.render("CREAM-RULED — tl5 hang-tag round 2", True, (255,220,80))
...
out = "docs/store_price_tl5/cream_ruled/round_2.png"
