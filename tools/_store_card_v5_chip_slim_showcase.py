"""Chip-slim options: option B (h=36) locked as base, frame-only height reduction.

Option B (h=m(18)=36 device px) is the locked content size. This showcase
keeps all inner elements (coin diameter, font, numerals) sized from h_content=36
and varies only h_frame (the pill rect / border radius) independently, so the
chip frame gets slimmer without shrinking what's inside it.

Output: docs/store_card_v5_chip_slim/showcase.png
"""
import os, sys
...
_INSET    = sc._INSET
H_CONTENT = sc.m(18)            # option B — locked content size

docs_dir = os.path.join(os.path.dirname(__file__), "..",
                        "docs", "store_card_v5_chip_slim")


# ── split-cream chip with separate h_content / h_frame ───────────────────────
...
def price_chip_slim(surf, cx, cy, text, h_content, h_frame, affordable=True):
    """Split-cream chip: inner elements sized from h_content, pill from h_frame."""
    ...
    rad    = h_frame // 2                               # pill radius from frame height
...
def draw_card_slim(surf, sid, rect, affordable=True, h_frame=None):
    if h_frame is None:
        h_frame = H_CONTENT
    m   = sc.m
    ...
    price_chip_slim(surf, cx, rect.y + m(88) - sc._CHIP_DY, price,
                    H_CONTENT, h_frame, affordable=affordable)


def render_card(h_frame):
    ch   = sc.CARD_H * sc.SS
    surf = pygame.Surface((CARD_W, ch + 16), pygame.SRCALPHA)
    rect = pygame.Rect(sc.m(_INSET), sc.m(_INSET),
                       CARD_W - 2 * sc.m(_INSET),
                       ch - 2 * sc.m(_INSET))
    draw_card_slim(surf, SID, rect, affordable=True, h_frame=h_frame)
    return surf


# ── options ───────────────────────────────────────────────────────────────────
# BEFORE = option B locked (h_frame = h_content = 36); then frame-only reduction
OPTIONS = [
    ("B (base)", f"frame={sc.m(18)}px = content",  sc.m(18)),
    ("A",        f"frame={sc.m(17)}px (−1 each)", sc.m(17)),
    ("B",        f"frame={sc.m(16)}px (−2 each)", sc.m(16)),
    ("C",        f"frame={sc.m(15)}px (−3 each)", sc.m(15)),
    ("D",        f"frame={sc.m(14)}px (−4 each)", sc.m(14)),
]
...
hf   = hud_font(18, True)
htxt = hf.render(
    f"v5 store card — chip frame height (content locked at h={H_CONTENT}px) ({SID})",
    True, (210, 206, 224))
...
for col, (oid, sublabel, h_frame) in enumerate(OPTIONS):
    x    = MARGIN + col * (CARD_W + GAP)
    card = render_card(h_frame)
