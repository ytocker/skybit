#!/usr/bin/env python3
"""marquee-bulb · confirm_purchase_v8 · swap-round-1"""
import os, sys, math
...
def _pin_row(big, rect, n, inset):
    y = rect.top + m(4)
    x0, x1 = rect.left + inset, rect.right - inset
    for i in range(n):
        px = x0 if n == 1 else int(round(x0 + (x1 - x0) * i / (n - 1)))
        sc._alpha_aura(big, px, y, m(6), CARD_RING_BRIGHT, peak=40, layers=6)
        pygame.draw.circle(big, CARD_RING_BRIGHT, (px, y), m(3))
...
idr=ImageDraw.Draw(strip); idr.text((MARGIN,18),"marquee-bulb · swap-round-1",fill=(232,226,208))
...
OUT="/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/marquee-bulb/round_1.png"
