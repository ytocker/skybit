"""POISON VIAL — Round 4 micro-variants for the skull-and-crossbones label.

The user picked the conical erlenmeyer from Round 3 panel 4 and asked
that the red X be swapped for a skull-and-crossbones glyph. The
SKATEBOARD pickup already wears a Jolly Roger composition (bone-cream
skull, crossed SKATEBOARD DECKS, 96-px footprint), so this label has to
stay clearly subordinate to the vial silhouette: ~24-28 px tall inside
the 48-px footprint, with crossed cylindrical FEMURS (not decks) and a
slightly aged off-white bone tone so the two pickups don't share a
palette signature.

Four variants of just the label, on otherwise identical flasks:
- A Classic poison label   (universal pictogram)
- B Cartoon menace         (rounder skull, triangle nose)
- C Stark minimalist       (rounded-square skull, slab bones)
- D Tilted dynamic         (~10 deg tilt, slight grin, off-90 bones)

WHY this lives in tools/ — review-only sheet; production code is left
alone until the user picks a winner.
"""
...
# Palette — kit-matched to Round 3 vial
BLACK_DOME    = (10, 10, 18)
...
# Flask body — Round 3 panel 4, untouched except the label is now
# externally injected so each variant can paint a different skull.
def _draw_flask_body(big, cx, cy, pulse, paint_label):
    ...
    # Label sits centred on the cone between meniscus and base where the
    # glass is widest. Budget the height tight (~13 native px = ~26 of
    # the conical body's interior) so it stays a label, not a flag.
    label_cx = cx
    label_cy = (fill_top_y + base_y) // 2 - SS
    label_h  = 13 * SS         # total skull+bones bounding box height
    label_w  = 16 * SS         # max width across bones
...
def _label_classic(big, cx, cy, label_h, label_w):
    ...
    against which the others are tuned."""
    # Crossbones FIRST (behind skull)
    bone_len   = int(label_w * 0.55)
    bone_thick = int(SS * 1.4)
    knob_r     = int(SS * 1.9)
    _crossed_bones(big, cx, cy + int(SS * 1.5), bone_len, bone_thick, knob_r,
                   angle_a=math.radians(32),
                   angle_b=math.radians(-32))

    # Skull cranium
    skull_r = int(label_h * 0.34)
    pygame.draw.circle(big, BONE_OUTLINE, (cx, cy - SS), skull_r + SS)
    ...
    pygame.draw.circle(shade, (*BONE_SHADE, 130),
                       (skull_r + 2 + SS, skull_r + 2 + SS), skull_r - SS)
    big.blit(shade, (cx - skull_r - 2, cy - SS - skull_r - 2))

    # Square jaw block — slightly narrower than cranium
    jaw_w = int(skull_r * 1.15)
    jaw_h = int(skull_r * 0.55)
    jaw_rect = pygame.Rect(cx - jaw_w // 2, cy + skull_r - 2 * SS,
                           jaw_w, jaw_h)
    pygame.draw.rect(big, BONE_OUTLINE,
                     jaw_rect.inflate(2 * SS, SS),
                     border_radius=int(SS * 1.2))
    pygame.draw.rect(big, BONE, jaw_rect, border_radius=SS)
    # 2 jaw teeth slits
    for tx in (-jaw_w // 4, jaw_w // 4):
        pygame.draw.line(big, BONE_OUTLINE,
                         (cx + tx, jaw_rect.top + SS),
                         (cx + tx, jaw_rect.bottom - SS),
                         max(1, SS // 2))

    # Eye sockets — two simple dark circles, no nose
    eye_r = int(skull_r * 0.35)
    eye_y = cy - SS
    eye_dx = int(skull_r * 0.5)
    pygame.draw.circle(big, SOCKET, (cx - eye_dx, eye_y), eye_r)
    pygame.draw.circle(big, SOCKET, (cx + eye_dx, eye_y), eye_r)
    # Tiny 1-px catch-light inside each socket
    pygame.draw.circle(big, (220, 210, 195),
                       (cx - eye_dx + SS, eye_y - SS), max(1, SS // 2))
    pygame.draw.circle(big, (220, 210, 195),
                       (cx + eye_dx + SS, eye_y - SS), max(1, SS // 2))
...
def _label_cartoon(big, cx, cy, label_h, label_w):
    ...
    tiny triangular nose, crossed bones with rounded ends. Reads as
    "fun spooky" without sliding into corporate biohazard."""
    bone_len   = int(label_w * 0.58)
    bone_thick = int(SS * 1.55)
    knob_r     = int(SS * 2.2)        # fatter epiphyses = cartoonier
    _crossed_bones(big, cx, cy + int(SS * 1.5), bone_len, bone_thick, knob_r,
                   angle_a=math.radians(30),
                   angle_b=math.radians(-30))

    # Skull cranium — slightly oval (taller) for roundness
    skull_w = int(label_h * 0.78)
    skull_h = int(label_h * 0.72)
    cranium = pygame.Rect(0, 0, skull_w, skull_h)
    cranium.center = (cx, cy - SS)
    pygame.draw.ellipse(big, BONE_OUTLINE, cranium.inflate(2 * SS, 2 * SS))
    ...
    jaw_h = int(skull_h * 0.38)
    jaw_rect = pygame.Rect(cx - jaw_w // 2,
                           cranium.bottom - 2 * SS, jaw_w, jaw_h)
    pygame.draw.ellipse(big, BONE_OUTLINE, jaw_rect.inflate(2 * SS, 2 * SS))
    pygame.draw.ellipse(big, BONE, jaw_rect)

    # Bigger sockets
    eye_r = int(skull_w * 0.21)
    eye_y = cy - SS
    eye_dx = int(skull_w * 0.24)
    pygame.draw.circle(big, SOCKET, (cx - eye_dx, eye_y), eye_r)
    pygame.draw.circle(big, SOCKET, (cx + eye_dx, eye_y), eye_r)
    # Little white pupil dot — sells the "alive cartoon" read
    pygame.draw.circle(big, (240, 235, 220),
                       (cx - eye_dx + SS, eye_y - SS),
                       max(1, int(SS * 0.7)))
    pygame.draw.circle(big, (240, 235, 220),
                       (cx + eye_dx + SS, eye_y - SS),
                       max(1, int(SS * 0.7)))
    ...
    pygame.draw.polygon(big, SOCKET, nose_pts)

    # Subtle grin — short curved arc on the jaw block
    grin_rect = pygame.Rect(0, 0, jaw_w - 2 * SS, jaw_h)
    grin_rect.center = (cx, jaw_rect.centery + SS // 2)
    pygame.draw.arc(big, BONE_OUTLINE, grin_rect,
                    math.radians(200), math.radians(340),
                    max(1, SS // 2 + 1))
    # Two tooth slits down the grin
    for tx in (-SS * 2, SS * 2):
        pygame.draw.line(big, BONE_OUTLINE,
                         (cx + tx, jaw_rect.centery - SS),
                         (cx + tx, jaw_rect.centery + SS * 2),
                         max(1, SS // 2))
...
def _label_minimalist(big, cx, cy, label_h, label_w):
    ...
    bone_len   = int(label_w * 0.55)
    bone_thick = int(SS * 1.8)         # fatter slabs
    knob_r     = int(SS * 1.5)         # minimal knobs
    # Override femur to draw FLAT-CAP slabs — call lower-level helper
    for ang in (math.radians(34), math.radians(-34)):
        cosA, sinA = math.cos(ang), math.sin(ang)
        p0 = (cx - cosA * bone_len, cy + int(SS * 1.2) - sinA * bone_len)
        p1 = (cx + cosA * bone_len, cy + int(SS * 1.2) + sinA * bone_len)
    ...
    crm_w = int(label_h * 0.74)
    crm_h = int(label_h * 0.66)
    crm = pygame.Rect(0, 0, crm_w, crm_h)
    crm.center = (cx, cy - SS)
    pygame.draw.rect(big, BONE_OUTLINE,
                     crm.inflate(2 * SS, 2 * SS),
                     border_radius=int(SS * 1.8))
    pygame.draw.rect(big, BONE, crm, border_radius=int(SS * 1.3))

    # Single-pixel shade band across bottom
    shade_band = pygame.Rect(crm.left + SS, crm.bottom - 3 * SS,
                             crm.width - 2 * SS, 2 * SS)
    ...
    jaw.midtop = (cx, crm.bottom - SS)
    pygame.draw.rect(big, BONE_OUTLINE, jaw.inflate(2 * SS, 2 * SS),
                     border_radius=SS)
    pygame.draw.rect(big, BONE, jaw, border_radius=SS // 2 + 1)

    # Two big circle sockets
    eye_r = int(crm_w * 0.22)
    eye_y = cy - SS
    eye_dx = int(crm_w * 0.26)
    pygame.draw.circle(big, SOCKET, (cx - eye_dx, eye_y), eye_r)
    pygame.draw.circle(big, SOCKET, (cx + eye_dx, eye_y), eye_r)
...
def _label_tilted(big, cx, cy, label_h, label_w):
    ...
    so the tilt is clean."""
    buf_w = label_w + 8 * SS
    buf_h = label_h + 8 * SS
    buf = pygame.Surface((buf_w, buf_h), pygame.SRCALPHA)
    bx, by = buf_w // 2, buf_h // 2

    # Bones at off-90 angles (40 and -28)
    bone_len   = int(label_w * 0.56)
    bone_thick = int(SS * 1.45)
    knob_r     = int(SS * 1.9)
    _crossed_bones(buf, bx, by + int(SS * 1.5),
                   bone_len, bone_thick, knob_r,
                   angle_a=math.radians(40),
                   angle_b=math.radians(-26))

    # Skull cranium — slightly oval
    skull_w = int(label_h * 0.76)
    skull_h = int(label_h * 0.70)
    cranium = pygame.Rect(0, 0, skull_w, skull_h)
    cranium.center = (bx, by - SS)
    pygame.draw.ellipse(buf, BONE_OUTLINE, cranium.inflate(2 * SS, 2 * SS))
    ...
    # Jaw
    jaw_w = int(skull_w * 0.75)
    jaw_h = int(skull_h * 0.35)
    jaw_rect = pygame.Rect(bx - jaw_w // 2,
                           cranium.bottom - 2 * SS, jaw_w, jaw_h)
    pygame.draw.ellipse(buf, BONE_OUTLINE, jaw_rect.inflate(2 * SS, 2 * SS))
    pygame.draw.ellipse(buf, BONE, jaw_rect)

    # Eyes — slightly asymmetric (right eye a touch lower) for personality
    eye_r = int(skull_w * 0.20)
    eye_y = by - SS
    eye_dx = int(skull_w * 0.24)
    pygame.draw.circle(buf, SOCKET, (bx - eye_dx, eye_y), eye_r)
    pygame.draw.circle(buf, SOCKET, (bx + eye_dx, eye_y + SS), eye_r)
    pygame.draw.circle(buf, (240, 235, 220),
                       (bx - eye_dx + SS, eye_y - SS),
                       max(1, int(SS * 0.7)))
    pygame.draw.circle(buf, (240, 235, 220),
                       (bx + eye_dx + SS, eye_y),
                       max(1, int(SS * 0.7)))
    ...
    # Tiny nose
    nose_y = by + int(SS * 1.6)
    nose_pts = [
        (bx - SS, nose_y),
        (bx + SS, nose_y),
        (bx,      nose_y + int(SS * 1.6)),
    ]
    pygame.draw.polygon(buf, SOCKET, nose_pts)

    # Slight grin — angled line up to the right
    grin_y = jaw_rect.centery
    pygame.draw.line(buf, BONE_OUTLINE,
                     (bx - jaw_w // 3, grin_y + SS),
                     (bx + jaw_w // 3, grin_y - SS // 2),
                     max(1, SS // 2 + 1))
...
VARIANTS = [
    ...
    ("B",  "CARTOON",      "rounder skull + bigger sockets + tiny nose | Skybit-leaning",
    ...
    ("D",  "TILTED",       "skull tilted ~10 deg, off-90 bones, slight grin",
...
def build_sheet():
    ...
    _draw_label(sheet,
                "POISON VIAL  —  skull-and-crossbones label, 4 micro-variants",
                16, 12, font_title)
    _draw_label(sheet,
                "same Round-3 erlenmeyer | label ~13 px tall on the cone | "
                "aged bone (220,215,200) | cylindrical femurs, NOT skate decks",
                16, 42, font_s, color=DIM)
    ...
    footer = ("user-picked vial body from Round 3 panel 4 | label only changes | "
              "kept distinct from SKATEBOARD Jolly Roger (smaller, off-white, femurs)")
...
def main():
    ...
    sheet = build_sheet()
    out_path = os.path.join(out_dir, "round_4_vial_skull.png")
