"""ALPINE_HAZE fine-tune v7_ember_on_dark — banked coals smouldering at the base.

Only sky_bot and horizon differ from live ALPINE_HAZE at phases 0.27–0.52.
Note: at phases 0.27 and 0.31, sky_bot is in a moderate luminance zone
(~0.26–0.30), which was a deliberate design choice to avoid a jarring band
under the bright dome.

    python tools/sky_v7_render.py

Output: docs/sky_transition/v7_ember_on_dark/round_1.png
"""
import os
import sys
...
_OVERRIDES = {
    0.27: dict(sky_bot=(201, 133,  93), horizon=(222, 159,  89)),
    0.31: dict(sky_bot=(195, 119,  86), horizon=(217, 145,  82)),
    0.37: dict(sky_bot=(140,  67,  56), horizon=(168,  90,  57)),
    0.42: dict(sky_bot=(117,  54,  51), horizon=(150,  75,  57)),
    0.47: dict(sky_bot=( 88,  41,  44), horizon=(129,  61,  53)),
    0.52: dict(sky_bot=( 59,  32,  37), horizon=(103,  50,  51)),
}
...
SPEC = BiomeSpec(
    name='alpine_haze_v7_ember_on_dark',
    note='v7 ember_on_dark - dark base with thin warm ember sliver at horizon',
    keyframes=_KF,
    sky=ALPINE_HAZE.sky,
)
...
def main():
    ...
    title = f_title.render(
        "v7_ember_on_dark — banked coals, dark base with warm ember sliver",
        True, TEXT_HI)
