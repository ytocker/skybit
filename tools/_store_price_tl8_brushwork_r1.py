"""Round-1 render for tl8 numeral concept: brushwork.

East Asian ink brush (sumi). One maximal solid pass fills the amount at uniform
heavy weight so every stroke lands boldly at 5-8px. The ledger rule is upgraded
to the brush's finishing stroke: a thick bar that tapers via three stacked solid
rects of decreasing width. The 'G' token is a single max-bold ink stamp.
...
def _draw_price(face, text, center, affordable):
    """Sumi ledger: one maximal solid ink pass for the amount, a copper/grey 'G'
    token above it, and the underline upgraded to a brush finishing stroke that
    tapers via three stacked solid rects. No alpha layering — value and
    stroke-weight contrast alone carry the tag at 5-8px."""
    cx = center[0]
    ink = (40, 30, 26) if affordable else (66, 64, 78)
    # Copper-tinted token when affordable; a cool grey when locked keeps the two
    # states unmistakable on a glance.
    coin_col = (200, 156, 50) if affordable else (88, 92, 110)
    rule_base = (56, 42, 30) if affordable else (90, 88, 100)
    rule_taper = (80, 60, 42) if affordable else (120, 118, 128)

    # Coin token — one max-bold 'G' stamp in matching ink, no hairline overlay so
    # it reads at the same heavy weight as the amount.
    cy_token = int(FACE_H * 0.40)
    coin_mask = sc._stamp_bold(sc._glyph_base("G", sc.font(8), 0), sc.m(1.8))
    ...
    # Amount — single heavy ink pass, uniform bold weight, no lighter overlay.
    ...
    # Brush finishing stroke: a thick base bar, then two narrower lighter rects
    # stacked over its middle so the darker ends read as a tapered brush lift.
    rule_y = cy_amount + amt_h // 2 + sc.m(2)
    rule_w = min(amt_mask.get_width() + sc.m(4), FACE_W - sc.m(8))
    rule_h = max(2, sc.m(2))
    rule_x = FACE_W // 2 - rule_w // 2
    pygame.draw.rect(face, rule_base, (rule_x, rule_y, rule_w, rule_h))
    pygame.draw.rect(face, rule_taper,
                     (rule_x + sc.m(1), rule_y, rule_w - sc.m(2), rule_h))
    pygame.draw.rect(face, rule_taper,
                     (rule_x + sc.m(3), rule_y, rule_w - sc.m(6), rule_h))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("BRUSHWORK price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/brushwork/round_1.png"
