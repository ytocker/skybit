...
# Carve each glyph into its own near-black void column BEFORE painting the warm
# halo. The dark seat under each filament ring makes the champagne bleed pop far
# harder than over the neutral navy band — light/dark contrast is the concept, so
# the inter-glyph gaps must read near-black, not navy.
def _name_niche_void(big, name, cx, cy, max_w):
    ...
    # PASS 1: flat void columns — one per glyph, width = ink width (not full
    # advance) so gaps between letters stay dark. No bevel/emboss/highlight; the
    # column is a pure navy→near-black→navy vertical gradient seat.
    x = x0
    for char, adv in zip(name, advances):
        tile = _stamp_bold(_glyph_base(char, f, 0), m(0.8))
        tw = tile.get_width()
        ink_x = x + adv // 2 - tw // 2
        void_col = vgrad_stops(tw, band_h, 0,
                               [(0.0, (14, 12, 26)),
                                (0.5, (6,  6,  14)),
                                (1.0, (14, 12, 26))], 255)
        sy = cy - band_h // 2
        big.blit(void_col, (ink_x, sy))
        x += adv

    # PASS 2: per-glyph filament halo — champagne/ivory rings, normal-alpha. The
    # dark seat under each halo makes the warm bleed pop much harder.
    RINGS = [
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v6 — niche-void — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v6/niche-void/round_1.png"
