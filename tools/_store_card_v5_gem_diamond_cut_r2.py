"""Round-2 render sheet for the diamond-cut gem badge concept.
...
off one top-left light. R2 folds in the art-director notes: a stronger warm
reflected fill so shadow facets glow instead of going dead purple, a table
lifted to be the single brightest zone, and a split specular (soft lens + a
distinct hot pip, both correctly seated near the table's upper flank). Renders
...
def my_facet_gem(surf, cx, cy, r, base, deep, mystery=False):
    ...
    key light, then a warmer reflected secondary from the lower-right keeps
    the shadow facets alive rather than dead purple."""
    if mystery:
    ...
    # Warmer reflected tint than r1: a low-sun bounce, not just a pale wash, so
    # cool shadow facets pick up a believable amber return.
    ...
    def shade(nx, ny):
        ...
        # reflected secondary from lower-right, warmer + stronger than r1 so
        # the SE/S-facing shadow facets glow instead of reading dead purple
        d2 = nx * 0.5 + ny * 0.5
        f2 = max(0.0, (d2 + 1) / 2)
    ...
    lrx = max(1, int(r * (0.10 + 0.06 * gf)))
    lry = max(1, int(r * (0.08 + 0.04 * gf)))
    ...
    px, py = cx - int(r * 0.32), cy - int(r * 0.32)
...
def main():
    ...
    header = hf.render("gem badge — diamond-cut r2", True, (236, 232, 250))
