#!/usr/bin/env python3
"""
slot-marquee  ·  confirm_purchase_v8  ·  round 2

Fixes from art-director critique:
- Price numeral > coin: text_size 12→17, coin_r 10→7 (numeral dominates)
- Marquee window tightened to content: MARQUEE_W 216→134
- CANCEL capsule de-emphasised: bevel_rim bright → dim slate, top_sheen peak 14→6
"""
import os, sys, math
...
MARQUEE_CY, MARQUEE_W, MARQUEE_H = 284, 134, 36   # tightened 216→134
BUY_CY,     BUY_W,     BUY_H     = 342, 154, 46
CANCEL_CY,  CAN_W,     CAN_H     = 389, 154, 28
...
def coin_price_inline(big, cx, cy, price_str, coin_r=7, gap=6,
                      text_col=(234, 218, 168), text_size=17):
    """Price numeral larger than coin — numeral dominates hierarchy."""
    f   = font(text_size)
...
def marquee_window(big, price_str):
    """Dark recessed LED-credits window — tightened to content, numeral-first."""
    cx  = m(CX)
    cy  = m(MARQUEE_CY)
    w   = m(MARQUEE_W)   # tightened 216→134
    h   = m(MARQUEE_H)
    rad = m(7)
    x0  = cx - w // 2
    y0  = cy - h // 2
    ...
    # coin + price inline — coin_r=7, text_size=17 (numeral > coin)
    coin_price_inline(big, cx, cy, price_str,
                      coin_r=7, gap=6, text_col=(240, 218, 148), text_size=17)


def buy_plunger(big, pal):
    """Domed BUY plunger — warm gold, prominent."""
    cx  = m(CX)
    cy  = m(BUY_CY)
    w   = m(BUY_W)
    h   = m(BUY_H)
...
def cancel_capsule(big):
    """Ghost CANCEL capsule — dim outline only, no attention pull."""
    cx  = m(CX)
    cy  = m(CANCEL_CY)
    w   = m(CAN_W)
    h   = m(CAN_H)
    ...
    big.blit(vgrad_stops(w, h, rad,
                         [(0.0, (26, 24, 40)), (1.0, (16, 14, 28))], 255, gamma=1.0),
             rect.topleft)
    sc.top_sheen(big, rect, rad, m(8), peak=6)      # peak 14→6
    sc.bevel_rim(big, rect, rad, CARD_RING_DEEP, (120, 122, 150, 70),  # dim slate, not gold
                 w=max(1, m(1.2)))
    plain_text(big, "CANCEL", font(11), rect.center, (148, 144, 176),
               shadow_a=80, tracking=m(1.2), weight=m(0.7),
...
hdr  = _font(17, True).render("confirm_purchase_v8  ·  slot-marquee  ·  round 2", True, (232, 226, 208))
sub1 = _font(10, True).render("numeral-first marquee · domed BUY plunger · ghost CANCEL capsule", True, (140, 148, 168))
...
OUT = "/home/user/skybit/docs/confirm_purchase_v8/slot-marquee/round_2.png"
