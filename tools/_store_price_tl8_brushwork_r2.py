"""Round-2 render for tl8 numeral concept: brushwork.
...
to the brush's finishing stroke: a filled polygon that tapers in HEIGHT, heavy
on the left and flicking to a point at the right, so the brush reads as lifting
off the paper. The 'G' token is a lighter ink stamp in burnt copper so the
amount stays the dominant mark. Review sheet only — not wired live.
"""
import os
...
def _draw_price(face, text, center, affordable):
    """Sumi ledger: one maximal solid ink pass for the amount, a burnt-copper 'G'
    token above it at lighter weight, and the underline upgraded to a brush
    finishing stroke — a filled polygon that tapers in height and flicks to a
    point at the right. No alpha layering — value and stroke-weight contrast
    alone carry the tag at 5-8px."""
    cx = center[0]
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("BRUSHWORK price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/brushwork/round_2.png"
