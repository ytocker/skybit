...
def _name_marquee_glow(big, name, cx, cy, max_w):
    # Backlit theatre marquee: neutral lamp-white halo behind cream letters so
    # the word reads as lit from behind. Ivory is intentionally rarity-neutral
    # so the "lit lamp" reads the same on every tier.
    IVORY = (250, 244, 225)
    CREAM = (250, 248, 240)
    sz = 13.0
    while sz > 9.0:
    ...
    # Lay the halo FIRST (BLEND_ADD is safe on the opaque band) at several
    # x-positions spanning the glyph width so the whole word is backlit, not
    # just a hot centre. Endpoints pulled in a touch to keep the glow inside
    # the letters rather than bleeding past the ends.
    for fx in (cx - gw // 3, cx - gw // 9, cx + gw // 9, cx + gw // 3):
        soft_glow(big, fx, cy, m(11), IVORY, 80, layers=8)
    # No shadow — the halo is the atmosphere; tight dark keyline keeps the
    # cream type crisp against its own glow.
...
htxt = hfont.render("store_card_v4_r4_name_v2 — marquee-glow — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/marquee-glow/round_1.png"
