"""Day-cast variety overhaul — round 2 candidate-sheet generator.
...
Round 2 lands the art-director's gating notes on top of an approved structure:
- the V7 price board now obeys the same night cooling + <=150 luma cap as every
  other cloth prop, so it stops being the only daytime-warm prop that survives
  the night composite and out-pops everything but the coin.
- the hot KID props (balloon / candy / kite tail) are knocked ~15-20% in
  saturation+value so the gold coin stays unambiguously the brightest object on
  the day composite — props read as warm accents, not beacons.
- the two twin pairs are resolved: the running-kid duplicate collapses to ONE,
  re-posed as a LOW leaned-forward chase (distinct from a raised-arm kite kid);
  the weigh/ladle vendors split on upper-body ACTION (ladle goes two-handed +
  downward) since vendors read chest-up.
- vendors gain real body variety (a heavy-set and a lean/tall build, plus one
  bare-headed worker so the hat isn't the only head-shape note).
- elders trim 7->6, dropping the padded+cane figure that overlapped the cane and
  bent-posture elders; the birdcage and feeding-birds figures stay.
...
def _luma(c):
    # Rec. 601 perceived luminance — the brightness the coin yardstick is judged on.
    return 0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2]


def _cap_luma(col, cap=150.0):
    """Hold a prop under the night brightness ceiling without flattening its hue —
    scales the colour toward black only if it crosses the cap. Applied AFTER
    _retint so it polices whatever survives the cool, which is why the V7 board
    no longer out-pops the night street."""
    y = _luma(col)
    if y <= cap:
        return col
    k = cap / y
...
def _knock(col, amount=0.18):
    """Pull a hot accent toward a muted shan-shui tone — drops value and saturation
    by `amount` so the prop reads as a warm accent, never brighter than the coin."""
    g = _luma(col)
    desat = _mix(col, (g, g, g), amount * 0.7)     # toward its own grey -> lower sat
    return _shade(desat, -int(255 * amount * 0.55))  # and a touch darker -> lower value
...
def draw_kid(surf, cx, base_y, v, night, t):
    ...
    run = "run" in v.pose
    chase = "chase" in v.pose               # low leaned-forward sprint (distinct outline)
    carried = "carried" in v.pose
    # squat-play needs an unmistakable child read at the far end — exaggerate the
    # head-to-body ratio so it can't be mistaken for a crouched adult.
    head_bias = 0.10 if squat else 0.0
    head_r = max(2, int(total * (0.34 + head_bias - 0.06 * age)))   # toddler = bigger head
    ...
    if squat:
        ...
        # Hard sprint: torso pitched LOW toward the scroll dir, head out front,
        # legs thrown into a wide stride — a flatter, leaning silhouette that can't
        # be read as the upright raised-arm kite kid.
        chase_dx = -int(body_w * 0.9)               # whole body leans into the run
        body_y = body_y + int(body_h * 0.35)        # crouch the torso down
        body_bot = body_y + body_h
        # leaned torso as a tilted ellipse, long axis pointing forward
    ...
    # ── head + hair ── (chase pitches the head out ahead of the leaned torso)
    if chase:
    ...
    # Hot accent props are muted (_knock) and capped (_cap_luma) so on the DAY
    # composite none of them can rival the gold coin's brightness — and so night
    # cooling has something below the ceiling to cool.
    ax = cx - body_w * 0.6
    arm_y = body_y + 1 if not chase else body_y + body_h // 2
    bx = cx + chase_dx                          # the chase kid's arms travel with the lean
    if chase:
        # lead arm thrown forward+low, reaching after whatever it's chasing
    ...
    if "kite" in v.accessory:
        ...
        # the kite-tail line knocked too — it was a hot warm streak on the day comp
    ...
    if "stick" in v.accessory:
        ...
        # in a chase the stick trails low behind the lead arm, not raised overhead
        if chase:
            pygame.draw.line(surf, scol, (bx + body_w * 0.4, arm_y),
                             (bx + body_w * 1.8, arm_y - body_h * 0.2), 2)
        else:
            pygame.draw.line(surf, scol, (cx - body_w * 0.4, arm_y),
                             (cx - body_w * 2.4, arm_y - body_h * 0.6), 2)
    if "hoop" in v.accessory:         # a rolling hoop driven ahead — a low ring on the deck
        ...
        pygame.draw.line(surf, hcol, (bx, arm_y), (ring.centerx + 1, ring.centery), 1)  # driving stick
...
def draw_vendor(surf, cx, base_y, v, night, t, crop_y=None):
    ...
    # Build now carries genuine mass, not just a pose change: a heavy-set vendor
    # gets a wider belly/lower hem, a lean/tall one narrows toward the shoulders so
    # the pool isn't one apron body x N poses.
    heavy = build >= 1.18
    lean = build <= 0.92
    if heavy:
        # barrel torso — belly wider than shoulders
        ...
        # tapered torso — broader shoulders, narrow waist
    ...
    if pose == "call":                # one hand cupped to the mouth, calling out
    ...
    elif pose == "ladle":             # TWO-handed downward ladling into a low pot —
        # both arms swing down to a common point so the chest-up read is a stooped,
        # double-armed scoop, distinct from the weigher's single raised scale arm.
        px = cx - body_w * 1.3; py = arm_y + torso_h * 0.6
        for sgn, off in ((-1, 0.0), (1, 0.4)):
            pygame.draw.line(surf, shirt, (cx + sgn * body_w * 0.5, arm_y),
                             (px + off * body_w, py), max(2, body_w // 4))
        # the ladle shaft + bowl dipping below the hands into the pot
        ...
        # a low pot rim under the scoop to read the downward action
        ...
        if "skewers" in v.accessory:
            ...
            # The price board was the lone daytime-warm prop that survived night
            # cooling and out-popped the street. It now starts from a muted red,
            # rides _retint like all cloth, AND is held under the <=150 luma cap so
            # nothing but the coin reads bright on either composite.
    ...
    elif hat == "wrap":              # tied head-cloth — low band hugging the skull + a knot tail
        col = pf(P.get("hat_c", (150, 110, 96)))
        pygame.draw.circle(surf, hair, (hx, hy - head_r // 3), head_r)  # hair shows above the band
        band = pygame.Rect(hx - head_r, hy - head_r // 2, head_r * 2, max(2, head_r))
        pygame.draw.ellipse(surf, col, band)
        pygame.draw.circle(surf, _shade(col, -22), (hx + head_r, hy - head_r // 3), max(1, head_r // 2))  # knot
    else:                            # bare-headed — a clean round skull breaks the hat silhouettes
...
KIDS = [
    # K1 toddler-run KEPT as-is by the AD — tiny waddle, top tuft, no prop.
    ...
    # K2 = the SURVIVOR of the old K2/K7 run twins, re-posed as a LOW leaned-forward
    # chase driving a rolling hoop+stick ahead — a flat, pitched silhouette that
    # reads nothing like a raised-arm kite kid.
    V(dict(shirt=(90, 165, 220), pants=(58, 52, 56), hair=(46, 36, 30), hair_style="bowl", skin="fair", prop=(150, 110, 64)),
      ...
      note="roles: shirt+pants+hair+prop(hoop) | attrs: age0.9(h0.96) | pose: chase(low lean) | acc: hoop,stick"),
    # K3 squat-play — head-to-body ratio bumped so the child read is unmistakable
    # even alone at the far end of the composite.
    V(dict(shirt=(250, 200, 70), pants=(70, 56, 48), hair=(40, 32, 28), hair_style="bowl", skin="tan"),
      ...
      note="roles: shirt+pants+hair | attrs: age0.55(h0.83) | pose: squat | head ratio +0.10 (clear child)"),
    # K4 balloon girl — prop knocked ~18% sat/value so it's a warm accent, not a beacon.
    V(dict(shirt=(120, 200, 130), pants=(64, 58, 50), hair=(50, 40, 32), hair_style="buns", skin="warm", prop=(228, 84, 92)),
      ...
      note="roles: shirt+pants+hair+prop(balloon-knocked) | attrs: age0.7(h0.89) | pose: point | acc: balloon | buns"),
    # K5 candy+cap — tanghulu reds knocked the same so they sit under the coin.
    V(dict(shirt=(220, 130, 200), pants=(70, 56, 60), hair=(44, 34, 28), hair_style="cap", skin="ruddy", cap=(210, 90, 80), prop=(224, 60, 60)),
      ...
      note="roles: shirt+pants+hair+cap+prop(candy-knocked) | attrs: age0.45(h0.79) | acc: candy | cap"),
    # K6 piggyback KEPT as-is by the AD — rides the stooped carrier's back.
...
ELDERS = [
    ...
    # E6 feeding-birds KEPT (unique low-palm-out stance with pecking birds); the old
    # E7 padded+cane was CUT — its cane overlapped E1 and its bent/padded mass
    # overlapped E4/E5, so it added no new outline. Pool now 6.
...
VENDORS = [
    # V1 calling KEPT as-is by the AD. Average build, conical hat.
    V(dict(shirt=(150, 86, 70), shirt_dk=(104, 56, 46), apron=(214, 200, 178), pants=(70, 60, 52), hair=(46, 36, 30), skin="tan", hat="conical", hat_c=(196, 158, 92)),
      ...
      note="roles: shirt+apron+pants+hair+hat_c | attrs: pose:call build1.08 | acc: rolled,towel | hat:conical"),
    # V2 weighing — single raised scale arm (UNCHANGED action), now LEAN/TALL build
    # + a tied head-WRAP so it reads apart from the two-handed V4 ladler.
    V(dict(shirt=(78, 124, 124), shirt_dk=(48, 84, 84), apron=(206, 196, 176), pants=(66, 58, 50), hair=(40, 32, 28), skin="warm", hat="wrap", hat_c=(150, 110, 96)),
      ...
      note="roles: shirt+apron+pants+hair+wrap | attrs: pose:weigh build0.90 h1.12 (lean) | head:wrap | 1-arm scale"),
    # V3 fanning grill KEPT as-is by the AD — but pushed HEAVY-SET (barrel belly) so
    # the stout grill cook anchors the body-variety spread.
    V(dict(shirt=(158, 128, 78), shirt_dk=(108, 84, 50), apron=(210, 196, 172), pants=(64, 58, 48), hair=(50, 40, 32), skin="deep", hat="cap", hat_c=(120, 100, 76), towel=(220, 214, 200)),
      ...
      note="roles: shirt+apron+pants+hair+hat_c+towel | attrs: pose:fan build1.24 (heavy-set) | hat:cap"),
    # V4 ladling — re-actioned to a TWO-handed downward scoop into a low pot, and made
    # BARE-HEADED (clean round skull) so a head-shape note exists among the hats.
    V(dict(shirt=(118, 116, 80), shirt_dk=(78, 78, 52), apron=(204, 192, 170), pants=(60, 56, 46), hair=(44, 34, 28), skin="ruddy", hat="none"),
      ...
      note="roles: shirt+apron+pants+hair | attrs: pose:ladle(2-hand downward) | acc: rolled | head:bare"),
    # V5 stacking baskets KEPT as-is by the AD. Conical hat, average build.
    ...
    # V6 skewers — held vertical bundle, cap.
    ...
    # V7 sign-board — the price board now starts muted, rides _retint AND the <=150
    # luma cap, so it no longer out-pops the night street.
    V(dict(shirt=(140, 104, 130), shirt_dk=(94, 66, 90), apron=(206, 194, 174), pants=(64, 56, 58), hair=(50, 40, 32), skin="fair", hat="cloth", hat_c=(150, 100, 84), sign=(168, 78, 70)),
      ...
      note="roles: shirt+apron+pants+hair+hat_c+sign(capped<=150) | attrs: pose:sign | hat:cloth | board muted"),
]
...
def render():
    ...
    y = pad
    _text(sheet, "SKYBIT PROMENADE — DAY-CAST VARIETY (round 2): KIDS 6 · ELDERS 6 · VENDORS 7", pad, y, 17, (250, 246, 236), bold=True)
    y += 22
    _text(sheet, "R2 lands AD gating notes: V7 board cooled+capped(<=150); hot KID props knocked ~18%; run-twins->1 LOW chase; weigh vs 2-hand ladle split; vendor body variety (heavy/lean) + bare/wrap heads; elders trimmed 7->6. FAR crisp.", pad, y, 10, (188, 186, 200))
    y += title_h - 22
    ...
    y += 22
    # a representative market-row mix — every figure in all three pools appears once
    # (the heavy V3, lean V2, bare-head V4 and cooled V7 all present so the night
    # composite can be judged against the coin), KIDS clamp to the new 6-item pool.
    mix = [("vendor", VENDORS[0]), ("kid", KIDS[1]), ("elder", ELDERS[1]),
           ...
           ("vendor", VENDORS[6]), ("kid", KIDS[0]), ("elder", ELDERS[4]),
           ("vendor", VENDORS[3]), ("kid", KIDS[2]), ("elder", ELDERS[5]),
           ("vendor", VENDORS[5]), ("kid", KIDS[4]), ("elder", ELDERS[3])]
    for is_night in (False, True):
        night = 0.95 if is_night else 0.0
        bg = BG_NIGHT if is_night else BG_DAY
    ...
    out = "/home/user/skybit/docs/sidewalk_overhaul/day_cast/round_2.png"
