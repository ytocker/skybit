"""Food-market STALL props — round 2 candidate-sheet generator.
...
def _wisp(surf, x, y0, t, *, n=3, rise=20, spread=3.0, speed=0.55, phase=0.0,
    ...
    """A rising column of `n` translucent puffs reading as RISING MOTION, not a
    static smudge. Each puff cycles 0..1 in t: it appears solid + small at the
    source, then CLIMBS the full `rise` while fattening + drifting, and FADES OUT
    over the top third of its travel so it visibly dissipates as it climbs (rather
    than peaking mid-column and re-condensing). Long enough rise + a fade biased
    toward the top is what separates "climbing plume" from "grey blob" at 1× over
    three sampled frames. Cool/pale by default (steam); pass a warm colour + low
    alpha for thin smoke. Each puff is its own SRCALPHA blit so alpha composites
    cleanly."""
    for i in range(n):
        ph = ((t * speed) + phase + i / n) % 1.0
        # ease-out climb: fast off the source, slowing as it rises — the read of
        # buoyant vapour decelerating into still air.
        ...
        # alpha: quick fade-in off the source, then a long fade biased to the top
        # so the puff is most visible LOW and gone by the crest — vertical travel
        # plus top-end dissipation is what sells the rise.
        if ph < 0.18:
            a = peak_a * (ph / 0.18)
        else:
...
def stall_cauldron(surf, sx, base_y, night, t):
    ...
    # broth steam — broad curls that visibly CLIMB the full plume height and
    # dissipate at the crest (matched to S1's lid-plume legibility).
...
def stall_grill(surf, sx, base_y, night, t):
    ...
    cy = _stall_shell(surf, sx, base_y, night, awning=("rust", "cream"),
                      sign=(176, 110, 70))
    # the long charcoal trough — kept as the wide low box (the good silhouette vs
    # the round pots) but pushed DARKER than r1 so the warm coal bed and the
    # skewer silhouettes pop against it and the apparatus reads occupied, not as a
    # bare counter/bench. Darker walls also let the few coal pixels carry the read.
    ...
    # a thin lit lip on the inner front wall so the trough reads as an open box
    # with a hot interior, not a solid block.
    ...
    # the coal BED: a recessed dark ash trough holding a FEW big unambiguous warm
    # coal pixels (2px chunks, not a fine dotted strip) — at 1× a couple of bright
    # warm nuggets reads "live coals" far better than many faint dots. Warm but
    # _cap150-clamped so the bed never out-pops the coin even at its pulse peak.
    ...
    # three big coal nuggets across the bed; each a 2px-wide warm chunk that pulses
    for j, kx in enumerate((-9, 0, 9)):
        ...
        # a single hottest core pixel on top so a coal point reads at true size
        pygame.draw.circle(surf, _cap150(_mix(coal_hot, (150, 110, 60), pulse)),
                           (sx + kx, cy - 7), 1)
    # CHUNKY skewers laid across the trough — drawn as 2px-thick dark silhouettes
    # whose meat chunks RISE ABOVE the trough top edge, so the broken top edge says
    # "loaded grill" at 1× even before the coals are noticed. Three skewers, fanned.
    char = _retint((40, 32, 30), night)        # charred-stick silhouette
    meat = _retint((128, 80, 58), night)
    meat_hi = _shade(meat, 18)
    for kx, lift in ((-9, 0), (0, -1), (9, 0)):
        sxp = sx + kx
        sky = cy - 10 + lift            # skewer sits on the trough rim, breaking it
        # the stick: a 2px bar running across, its left handle poking out the front
        pygame.draw.line(surf, char, (sxp - 7, sky + 1), (sxp + 7, sky + 1), 2)
        # 2-3 chunky meat cubes threaded on, each a 3×3 block standing PROUD of the
        # trough top edge so the silhouette is unmistakably "skewers over a grill".
    ...
    # it's smoke from dripping fat, not steam. Taller rise so it climbs + fades.
...
def stall_wok(surf, sx, base_y, night, t):
    ...
    # The flame's brightest source colour is the most likely pixel to break the
    # NIGHT cap, so every tongue is _cap150-clamped (verified <=150 luma) and the
    # day variant is shaded down further so it never reads hotter than the coin.
    ...
    # the round wok belly — drawn TILTED (rim higher on the right) so its
    # silhouette is asymmetric, unlike S2's level cast-iron belly, and so the long
    # handle reads as lifting the pan in mid-toss.
    ...
    # tilted rim — a shallow ellipse rotated by drawing its near/far edges offset,
    # giving the open mouth that points up-right toward the lifted handle.
    ...
    # THE SIGNATURE: a long straight wok handle thrust up-and-out to the LEFT,
    # longer + thicker than r1, with a wrapped grip knob at the tip. This single
    # diagonal bar pulling away from the round belly is what separates S4 from the
    # handle-less S2 cauldron at true far-lane size — the wok-hei toss pose.
    hcol = _retint((118, 90, 60), night)
    hx0, hy0 = sx - 12, cy - 14         # socket at the wok rim
    hx1, hy1 = sx - 26, cy - 22         # tip, well clear of the belly
    ...
    pygame.draw.circle(surf, _shade(hcol, -16), (hx1, hy1), 2)   # grip knob
    # steam rolling off the toss — climbs + fades like S1's plume
...
def stall_tea(surf, sx, base_y, night, t):
    ...
    # thickened to 3px so the spout survives the far-lane shrink and stays the
    # silhouette's defining feature rather than dropping to a hairline.
    pygame.draw.lines(surf, _shade(brass, -18), False,
                      [(ux + 5, cy - 13), (ux + 11, cy - 18), (ux + 14, cy - 24),
                       (ux + 13, cy - 28)], 3)
    ...
    # a SINGLE simplified cup beside the urn (the r1 three-bowl stack muddied into
    # a blob at true size) — one clean upturned bowl reads "drinks stall" without
    # competing with the tall urn+spout silhouette.
    ...
    # steam: a fine plume off the SPOUT TIP that climbs + dissipates
...
STALLS = [
    ...
    ("S3 skewer grill", stall_grill,
     "shell: rust/cream awning | grill: DARK low coal trough on legs (long silhouette) + recessed ash bed | "
     "coals: 3 BIG capped warm nuggets (pulse) | skewers: 3 CHUNKY 2px sticks, meat cubes BREAK the trough top edge | "
     "SMOKE: warm-grey ribbons climb+fade + embers | (FANNER's stall)"),
    ("S4 stir-fry wok", stall_wok,
     "shell: jade/cream awning | stove: flared cast cylinder + capped flame-lick (cap-verified) | "
     "wok: TILTED round belly + food + LONG up-left HANDLE w/ grip knob (the signature vs S2) | STEAM: rolling toss plume climbs+fades"),
    ("S5 tea / drinks urn", stall_tea,
     "shell: bamboo/indigo awning | warmer: small capped glow | urn: tall brass body + lid knob + thick 3px arcing SPOUT + handle | "
     "cup: single bowl (simplified) | STEAM: fine spout plume climbs+fades | (non-fire stall, tall silhouette)"),
]
...
def _stall_cell(parent, name, fn, note, x, y, w, h, night):
    ...
    # 3 animation frames at TRUE far-lane size, spaced so each puff has visibly
    # CLIMBED + faded between frames (proving rising motion, not a static smudge)
    frames_t = (0.0, 0.7, 1.4)
    fx0 = 16
...
def render():
    ...
    total_h = title_h + bandA_h + detailB_h + compC_h + PAD * 5 + 20
    sheet = pygame.Surface((W, total_h))
    sheet.fill((26, 28, 38))

    y = PAD
    _text(sheet, "SKYBIT PROMENADE — FOOD-MARKET STALL PROPS (round 2): 5 stalls", PAD, y, 17, (250, 246, 236), bold=True)
    y += 22
    ...
    # ── cap-verification footer (fix 4): print the hottest warm source colours
    # through _cap150 + their luma, plus the coin's brightest pixel, so the
    # "nothing out-pops the coin" contract is auditable on the sheet itself.
    audits = [
        ("S3 coal nugget (peak)", _cap150((150, 88, 38))),
        ("S3 coal core (peak)", _cap150((150, 110, 60))),
        ("S4 flame tip (peak)", _cap150((146, 100, 44))),
        ("S2 firebox mouth", _cap150((128, 70, 36))),
        ("coin core (brightest)", (255, 232, 150)),
    ]
    line = "  ·  ".join(f"{nm} luma={_luma(c):.0f}" for nm, c in audits)
    _text(sheet, "CAP AUDIT (<=150 except coin):  " + line, PAD, total_h - 16, 9,
          (170, 200, 180))

    out = "/home/user/skybit/docs/sidewalk_overhaul/food_stalls/round_2.png"
    os.makedirs(os.path.dirname(out), exist_ok=True)
    pygame.image.save(sheet, out)
    print("saved", out, sheet.get_size())
    print("CAP AUDIT:")
    for nm, c in audits:
        print(f"  {nm:28s} rgb={c} luma={_luma(c):.1f}")
