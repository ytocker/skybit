...
def _bezier_pts(p0, p1, p2, p3, steps):
    pts = []
...
def _name_bookend_swash(big, name, cx, cy, max_w):
    ...
    # Anchor the origin just INSIDE the glyph edge at mid-x-height so the arc
    # grows out of actual ink instead of floating off an empty bounding-box
    # corner, then sweeps into the flank and flicks up. Lifting the whole curve
    # to mid-x-height (not the baseline) buys real vertical travel for the
    # flick, which is the payoff of the flourish.
    base_y = gy + bh * 0.5

    def _swash(ox, sign):
        p0 = (ox - sign * m(1), base_y)
        p1 = (ox + sign * reach * 0.3, base_y + bh * 0.1)
        p2 = (ox + sign * reach * 0.7, base_y - bh * 0.1)
        p3 = (ox + sign * reach, base_y - m(8))
        pts = _bezier_pts(p0, p1, p2, p3, steps)
        w = max(1, m(1))
        for i in range(len(pts) - 1):
            # Hold near-full alpha through the body and rising flick, dissolving
            # only over the final tail — the flick-up is the brightest, most
            # legible part of the arc rather than the faintest.
            frac = i / (len(pts) - 1)
            if frac < 0.7:
                alpha = 220
            else:
                alpha = int(220 * (1 - (frac - 0.7) / 0.3))
            a, b = pts[i], pts[i + 1]
            # A one-pixel sub-alpha gold underlay adds perceived weight so the
            # single-logical-px hairline doesn't vanish at card scale, without
            # actually widening the stroke.
            sh_a = int(alpha * (70 / 220))
            pygame.draw.line(swash_surf, (*CARD_RING_BRIGHT, sh_a),
                             (a[0], a[1] + 1), (b[0], b[1] + 1), w)
            pygame.draw.line(swash_surf, (*CARD_RING_BRIGHT, alpha), a, b, w)

    # Only the outer terminals get a swash, so no arc ever crosses an
    # inter-glyph gap — the flourishes ornament the flanks alone.
    _swash(gx, -1)
    _swash(gx + bw, +1)
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v5 — bookend-swash — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v5/bookend-swash/round_2.png"
