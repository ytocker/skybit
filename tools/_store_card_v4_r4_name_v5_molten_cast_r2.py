...
def _name_molten_cast(big, name, cx, cy, max_w):
    ...
    # and the base settles to champagne (heat pools where the pour has just been
    # poured). Opposite of a natural top-lit gradient — sells "just cast" instead
    # of "gem sheen". The 1.0 stop stays at champagne, NOT ivory, so the body and
    # the warm-white bottom rim can never co-saturate to a white slab; the single
    # hottest value in the letter is supplied only by the meniscus line below.
    THERMAL = [
        (0.0,  (6, 6, 14)),       # near-black crown — held for a plateau so the
        (0.20, (6, 6, 14)),       # cool solidified top gets real device territory
        (0.45, (80, 70, 48)),     # deep warm-shadow
        (0.65, (160, 148, 108)),  # pewter-warm transition
        (0.85, (218, 208, 182)),  # champagne
        (1.0,  (218, 208, 182)),  # champagne base (hottest ramp value; no ivory)
    ]
    ...
    # Near-black contact anchor: the dark crown value mushes into the navy band
    # (only ~2x value apart), so a 1px strip of the coolest metal under the whole
    # word gives every glyph a hard footer that reads as "sitting on" the band at
    # 1x. Re-blit the body over it so only the gaps + the very foot stay dark.
    pygame.draw.rect(big, (6, 6, 14), (gx, gy + bh - m(1), bw, m(1)))
    big.blit(grad, (gx, gy))

    # Warm-white meniscus on the lower lip of each glyph — a razor line where the
    # hot pour meets the dark band, the tell that the metal is still molten. Held
    # to exactly m(1) height and blitted as normal alpha (not additive) over the
    # champagne base so it lands as one crisp warm line, never a saturated bar.
    meniscus_surf = pygame.Surface(big.get_size(), pygame.SRCALPHA)
    shifted = pygame.Surface((bw, bh), pygame.SRCALPHA)
    shifted.blit(master, (0, -m(1)))
    ...
    wp.fill((255, 255, 252, 190))
    wp.blit(bottom_lip, (0, 0), special_flags=pygame.BLEND_RGBA_MIN)
    meniscus_surf.blit(wp, (gx, gy))
    big.blit(meniscus_surf, (0, 0))

    # Tight champagne halo anchored at each glyph's hot lower third — the heat
    # bloom radiating off the pool, kept per-column so gaps don't fill. Held low
    # and dim so it never reaches the meniscus row and co-saturates the rim.
    ...
    for char, adv in zip(name, advances):
        glyph_cx = x + adv // 2
        glyph_hot_y = cy + bh // 4
        soft_glow(halo_surf, glyph_cx, glyph_hot_y, m(2.5), (218, 208, 182), 56, layers=3)
        x += adv
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v5 — molten-cast — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v5/molten-cast/round_2.png"
