"""Round-1 render for the `spot-accent` tl6 numeral treatment.

Flat deep-espresso (55,38,28) digits — zero gold — with a single terracotta
(168,70,48) underline bar below the numeral as the only chromatic accent.
All tag structure identical to tl2 r2.
"""
import os
...
def _draw_price(face, text, center, affordable):
    ...
    spot = (168, 70, 48) if affordable else (60, 75, 110)
    ...
    face.blit(fill, r)
    # Terracotta spot-color underline
    ul_y = r.bottom + max(2, sc.m(1))
    ul_h = max(2, sc.m(1))
    pygame.draw.rect(face, (*spot, 255), (r.left, ul_y, r.width, ul_h))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("SPOT-ACCENT — tl6 hang-tag round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/spot_accent/round_1.png"
