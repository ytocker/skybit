...
def _name_letter_lantern(big, name, cx, cy, max_w):
    ...
    # peak_alpha: 150 at ≤6 chars, down to 96 at 10 chars.  Longer names pack
    # more pucks into the band, so lower each so their overlap doesn't wash out.
    peak_a = max(96, int(150 - (n_chars - 6) * (150 - 96) / 4)) if n_chars > 6 else 150

    # Glow pass: one puck per glyph, BLEND_ADD so each letter reads as its own
    # lamp with dark gaps between — an accumulated additive field, not a bar.
    glow_surf = pygame.Surface(big.get_size(), pygame.SRCALPHA)
    x = cx - total_w // 2
    for i, (char, adv) in enumerate(zip(name, advances)):
        glyph_cx = x + adv // 2
        # radius = clamp(adv * 0.42, m(3), m(6))
        r = max(m(3), min(m(6), int(adv * 0.42)))
        soft_glow(glow_surf, glyph_cx, cy, r, (255, 236, 190), peak_a, layers=4)
        x += adv
    big.blit(glow_surf, (0, 0), special_flags=pygame.BLEND_ADD)

    # Glyph pass: full ivory with near-black keyline so glyph stays brighter than
    # the puck hotspot below it — the letter is the filament, not the halo.
    plain_text(big, name, f, (cx, cy), (246, 240, 216), shadow_a=0,
               weight=m(0.5), keyline=(8, 8, 20), kw=m(1.0))
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v4 — letter-lantern — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v4/letter-lantern/round_1.png"
