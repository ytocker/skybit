"""Round-2 render for tl9 numeral concept: axis_crush.

Flat-brush width compression, second pass. The thousands comma is dropped so
the raw digit string feeds the glyph — no sub-pixel speck noise from the comma
glyph, and the reclaimed ~6px lets the crush settle at 0.86 (vs 0.78 in r1)
while still fitting a 4-digit price cleanly inside the 66px budget. The
shallower squeeze retains condensed proportion while keeping 0-counters open
and vertical-stroke contrast readable at 5-8px. Review sheet only — not wired
live.
"""
import os
...
def _full(text):
    # No comma — raw digits only; axis_crush's narrower counters don't need the
    # comma separator, and dropping it reclaims device-px for the actual glyphs.
    digits = ''.join(c for c in text if c.isdigit())
    return str(int(digits)) if digits else text


def _price_glyph(text):
    """Flat-brush proportion: crush width to 86%, tall strokes dominate.

    Shallower than r1 (0.78) because the absent comma frees enough horizontal
    budget that the 4-digit price fits cleanly without pinching 0-counters.
    """
...
def _draw_price(face, text, center, affordable):
    """Sumi ledger: a width-crushed bold ink pass for the amount, a burnt-copper
    'G' token above it at lighter weight, and the underline as a brush finishing
    stroke — a filled polygon that tapers in height and flicks to a point at the
    right. No alpha layering — value, stroke-weight, and axis contrast carry the
    tag at 5-8px."""
    cx = center[0]
    ...
    # Amount — width-crushed heavy ink pass so vertical strokes dominate.
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 AXIS_CRUSH digit treatment — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9/axis_crush/round_2.png"
