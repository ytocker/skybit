"""Before / after comparison: original amber chip vs locked split-cream design.

BEFORE: sc.draw_card() — original production amber price chip
        h=m(20)=40px, pad=m(13)=26px, PRICE_STOPS gold body

AFTER:  split-cream chip with all geometry decisions locked
        h_content=m(18)=36px, h_frame=m(15)=30px, pad=m(8)=16px

Output: docs/store_card_v5_before_after/comparison.png
"""
import os, sys
...
H_CONTENT = sc.m(18)   # 36 px — locked
H_FRAME   = sc.m(15)   # 30 px — locked
PAD       = sc.m(8)    # 16 px — locked (option E)

docs_dir = os.path.join(os.path.dirname(__file__), "..",
                        "docs", "store_card_v5_before_after")
...
# ── AFTER — split-cream chip (locked geometry) ────────────────────────────────
...
def price_chip_slim(surf, cx, cy, text, affordable=True):
...
def draw_card_after(surf, sid, rect, affordable=True):
    m   = sc.m
...
def render_after():
    ...
    rect = pygame.Rect(sc.m(_INSET), sc.m(_INSET),
                       CARD_W - 2 * sc.m(_INSET),
                       ch - 2 * sc.m(_INSET))
    draw_card_after(surf, SID, rect, affordable=True)
    return surf
...
hf   = hud_font(18, True)
htxt = hf.render(
    f"v5 store card — before vs after  ({SID})",
    True, (210, 206, 224))
...
for col, (card, title, subtitle) in enumerate(PANELS):
    x = MARGIN + col * (CARD_W + GAP)
    canvas.blit(card, (x, panel_y))

    ly  = panel_y + PANEL_H + 6
    tid = lbl_id.render(title, True, (255, 230, 120))
    canvas.blit(tid, (x + (CARD_W - tid.get_width()) // 2, ly))
    ly += tid.get_height() + 2

    t = lbl_sm.render(subtitle, True, (130, 126, 150))
    canvas.blit(t, (x + (CARD_W - t.get_width()) // 2, ly))

out = os.path.join(docs_dir, "comparison.png")
