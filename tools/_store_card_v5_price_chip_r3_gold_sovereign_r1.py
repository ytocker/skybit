"""Round-1 render for the `gold-sovereign` price-chip concept (Levers T + C + P).

All three levers pulled at once so the chip reads as a minted sovereign coin
token: slightly larger numerals (T), a rich 3-stop coin-metal gradient poured
ONLY into the glyph mask (C), and a thick warm-gold 2px rim struck around the
edge (P). The body itself stays a genuinely dark, warm enamel — no glow, no
spectral fill — so wealth reads as struck metal on a near-black field. Locked
tarnishes the whole coin: dimmer rim, flat aged brass numerals, a cool slate
wash over the coin, and a colder enamel body.
"""
import os, sys
...
# ── the concept: dark sovereign enamel + struck coin-metal numerals + gold rim ─
def price_chip_gold_sovereign(surf, cx, cy, text, h, affordable=True):
    ...
    if affordable:
        # warm dark enamel — genuinely dark, no bloom; the coin metal is the light
        stops = [(0.0, (20, 22, 40)), (1.0, (14, 15, 30))]
        sc.chip_body_stops(surf, r, rad, stops, (10, 11, 22), (56, 60, 92),
                           gloss=14, gamma=1.04)
        # thick warm 2px gold rim — the "minted sovereign edge"
        sc.bevel_rim(surf, r, rad, (120, 88, 28, 235), (255, 228, 150, 235), w=2)
        coin_rim = (180, 150, 60)                     # warm gold coin edge
        cool_coin = None
    else:
        ...
        coin_rim = (120, 108, 78)
        cool_coin = (60, 70, 90, 140)                 # slate wash over the coin
    x = r.x + pad
    ccx = x + coin_d // 2
    ...
    if cool_coin is not None:
        ...
        # reads tarnished slate rather than warm gold
        cr = coin_d // 2
    ...
    nx = x + nw // 2
    if affordable:
        # C: 3-stop coin-metal gradient poured ONLY into the glyph mask so the
        # struck metal never bleeds onto the dark enamel body
        mask = sc._stamp_bold(sc._glyph_base(text, f, 0), sc.m(1.0))
        grad = sc.vgrad_stops(mask.get_width(), mask.get_height(), 0,
                              [(0.0, (255, 240, 180)), (0.35, (246, 206, 110)),
        ...
        sc.plain_text(surf, text, f, (nx, cy), color=(150, 132, 92), shadow_a=0,
                      weight=sc.m(1.0), keyline=(18, 20, 34), kw=sc.m(0.7))
    return r
...
def main():
    ...
    text(canvas, f"v5 price-chip r3 — gold-sovereign r1 — {SID} EPIC — T+C+P",
    ...
    # 4x detail strip on a card-body gradient patch, so the chip is judged on
    # the same ground it ships on (not a flat swatch)
    ...
    text(canvas,
         "dark enamel body  ·  3-stop coin-metal numeral gradient  ·  "
         "thick 2px warm-gold sovereign rim  ·  locked = tarnished",
         label_sm, (W // 2, H - footer // 2), (150, 156, 178))

    out = os.path.join(os.path.dirname(__file__), "..",
                       "docs", "store_card_v5_price_chip_r3",
                       "gold-sovereign", "round_1.png")
