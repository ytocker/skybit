"""Round-2 render for tl9 pins2: split_ring (one ring + coil arc — no figure-8)"""
import os
...
def _attachment(card, affordable):
    import math
    # lifted gunmetal (~130 vs r1 ~112) for better cream contrast
    col = (130, 134, 142) if affordable else (96, 100, 110)
    hi  = (180, 184, 190) if affordable else (148, 152, 160)
    kl  = (24, 26, 34)
    cx, cy = 14, 14
    # single main ring r=6 (not figure-8 two-circles)
    pygame.draw.circle(card, kl, (cx, cy), 7, 3)
    pygame.draw.circle(card, col, (cx, cy), 6, 2)
    # coil arc at r=8 covering top half — creates visible wire overlap at top, split gap at right
    coil_rect = pygame.Rect(cx - 8, cy - 8, 16, 16)
    pygame.draw.arc(card, kl, coil_rect.inflate(2, 2), math.radians(0), math.radians(180), 3)
    pygame.draw.arc(card, col, coil_rect, math.radians(0), math.radians(180), 2)
    # highlight on ring top-left arc
    hi_rect = pygame.Rect(cx - 6, cy - 6, 12, 12)
    pygame.draw.arc(card, hi, hi_rect, math.radians(120), math.radians(180), 1)
