"""dark-boutique tl4 hang-tag concept — round 2 render (Midnight Velvet Luxury Tag).

Standalone review harness: monkey-patches store_cards.price_chip with the
dark-boutique swing tag — a deep midnight-blue body with a gold grommet, a gold
...
docs/store_price_tl4/dark_boutique/round_2.png. Exploration only; nothing here
is wired into the live store draw path.

Round 2 folds in art-director notes: the coin-ring brand mark and the upper
ruling line are gone (they fought the grommet at 1x), every gold now rides one
two-stop ramp (base 194,152,48 / highlight 244,212,88), the ribbon gains satin
depth from a dark rim + bright centre stripe, and the locked price is pushed
greyer so affordability survives at 34px without reading hue.
"""
import os
...
# One gold ramp for the whole tag so grommet, ribbon and underrule share a
# family — a mismatched near-gold cluster read as tarnish at card scale.
GOLD_BASE = (194, 152, 48)
GOLD_HI = (244, 212, 88)
GOLD_EDGE = (140, 108, 32)
GOLD_SHADOW = (60, 38, 8)
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    if affordable:
        ...
        _grommet(face, 28, 12, sc.m(5), sc.m(3), sc.m(2),
                 GOLD_BASE, GOLD_HI, GOLD_SHADOW, (20, 14, 4))
        pygame.draw.circle(face, (255, 248, 200), (28 - sc.m(3), 12 - sc.m(3)), max(1, sc.m(1)))
        num_key, num_fill = (80, 64, 16), (242, 224, 176)
        underrule_col = GOLD_BASE
        ribbon_col, ribbon_hi, ribbon_edge = GOLD_BASE, GOLD_HI, GOLD_EDGE
        knot_col, knot_hi = GOLD_BASE, GOLD_HI
    else:
        body = sc.vgrad_stops(FACE_W, FACE_H, sc.m(3),
        ...
        num_key, num_fill = (48, 52, 74), (148, 152, 166)
        underrule_col = (80, 84, 100)
        ribbon_col, ribbon_hi, ribbon_edge = (152, 156, 168), (180, 184, 200), (100, 104, 116)
    ...
    # under the tag so only the exposed length between hole and knot shows. A dark
    # rim under a gold body under a bright centre stripe reads as woven satin
    # without exceeding ~2px at 1x.
    gx, gy = _rot_point(28, 12, FACE_W, FACE_H, (44, 60), TILT)
    a, b = (int(gx), int(gy)), (22, 13)
    pygame.draw.line(surf, ribbon_edge, a, b, max(2, sc.m(2)))
    pygame.draw.line(surf, ribbon_col, a, b, max(1, sc.m(1.5)))
    pygame.draw.line(surf, ribbon_hi, a, b, max(1, sc.m(0.5)))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("DARK-BOUTIQUE — tl4 hang-tag round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
out = "docs/store_price_tl4/dark_boutique/round_2.png"
os.makedirs(os.path.dirname(out), exist_ok=True)
print(f"verify canvas {sheet_w}x{sheet_h}")
