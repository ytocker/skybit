"""Round-1 render for tl9 pins concept: clothespin_peg.

A warm-wood clothespin clamps the top edge of the price tag on the card
canvas. Two tapered legs, each split into a lit outer face and a shadowed
inner face with a hard fill seam, read the peg as turned oak; a small silver
spring coil sits at the pivot. The numeral is axis-crushed — squeezed 14% on
the horizontal so the digits pack tight — with no coin token and no comma, and
the ledger rule stays a brush wedge that flicks to a point. Review sheet only —
not wired live.
"""
import os
...
def _clothespin_peg(surf, tag_center, affordable):
    # Peg center x = tag center x; peg grips just above tag center y
    px = tag_center[0] - 2   # slight left offset
    # The peg grips the top edge of the visible tag; top of peg is above tag
    # tag_center is (44,60) in the 162×100 card canvas
    peg_top = tag_center[1] - 30   # top of handle
    peg_grip = tag_center[1] - 18  # where tines grip the tag edge
    peg_bot = tag_center[1] - 10   # bottom of tines (below tag top edge)

    wood_light = (196, 158, 104)   # light oak
    wood_dark  = (150, 112, 66)    # shadow oak

    hw = sc.m(3)   # half-width of each leg at its widest (bottom), = 6 device px

    # Left leg: two polygons (lit outer + shadow inner)
    left_outer = [
        (px - hw, peg_grip),
        (px - hw + 2, peg_top + 4),
        (px - 1, peg_top),
    ...
    pygame.draw.polygon(surf, wood_light, left_outer)
    pygame.draw.polygon(surf, wood_dark, left_inner)
    ...
    pygame.draw.polygon(surf, wood_light, right_outer)
    pygame.draw.polygon(surf, wood_dark, right_inner)

    # Spring coil circle at pivot point (top of legs)
    spring_col = (180, 186, 196)
    spring_dot = (60, 64, 72)
    spring_pos = (px, peg_top)
    pygame.draw.circle(surf, spring_col, spring_pos, sc.m(2))
    pygame.draw.circle(surf, spring_dot, spring_pos, sc.m(1))

    # Tines extend below grip into the tag, overlapping the blitted face
    tine_col = wood_dark
    pygame.draw.rect(surf, tine_col,
                     pygame.Rect(px - hw, peg_grip, sc.m(1.5), peg_bot - peg_grip))
    pygame.draw.rect(surf, tine_col,
                     pygame.Rect(px + hw - sc.m(1.5), peg_grip, sc.m(1.5), peg_bot - peg_grip))
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    # Clothespin drawn AFTER tag blit
    _clothespin_peg(surf, tag_center, affordable)
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 pins: clothespin_peg — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9_pins/clothespin_peg/round_1.png"
