"""tl5 vellum-border hang-tag — round 1 exploration.

Cream hang-tag with NO ruling line. A thin double gold border runs around the
whole face perimeter (premium gift-tag language); the price sits large in an
open field across the lower 2/3. Gold border + gold grommet + gold ribbon read
as one cohesive gold language. Review-only render — never wired into the game.
"""
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    # ── double gold perimeter border ─────────────────────────────────────────
    # outer gold border at 3dp inset
    b1 = brect.inflate(-sc.m(3), -sc.m(3))
    gold_b = (194, 152, 48) if affordable else (78, 82, 96)
    gold_b2 = (232, 206, 148) if affordable else (120, 124, 140)
    pygame.draw.rect(face, gold_b, b1, 1, border_radius=sc.m(2))
    # inner lighter border at 5dp inset
    b2 = brect.inflate(-sc.m(5), -sc.m(5))
    pygame.draw.rect(face, gold_b2, b2, 1, border_radius=sc.m(2))
...
ht = hf.render("VELLUM-BORDER — tl5 hang-tag round 1", True, (255, 220, 80))
...
out = "docs/store_price_tl5/vellum_border/round_1.png"
