"""Round-1 render for tl7 numeral concept: engraved-tablet.

Recessed rounded plaque in the tag face; numeral bevel-embossed into it
via a shadow groove + highlight face + mid-tone fill (three stacked passes).
...
def _price_glyph(text):
    for fs in (13, 12, 11, 10, 9, 8):
        ...
        if mask.get_width() <= 60:   # plaque is narrower than full face
            return mask
    return mask


def _draw_price(face, text, center, affordable):
    """Inner plaque + three-pass embossed numeral (shadow groove, highlight, fill)."""
    # Plaque geometry
    ...
    # Numeral: three-pass emboss
    mask = _price_glyph(text)
    num_cx = pr.centerx
    num_cy = pr.centery
    r = mask.get_rect(center=(num_cx, num_cy))
    o = max(1, sc.m(1))

    # Pass 1: shadow groove shifted down-right (looks pressed in from top-left light)
    sh = mask.copy()
    sh.fill((*shadow_col, 160), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(sh, (r.x + o, r.y + o))

    # Pass 2: highlight shifted up-left (raised face of the letterform)
    hi = mask.copy()
    hi.fill((*highlight_col, 160), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(hi, (r.x - o, r.y - o))

    # Pass 3: mid-tone face fill at center
    fill = mask.copy()
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("ENGRAVED-TABLET price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl7/engraved_tablet/round_1.png"
