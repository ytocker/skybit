...
# Fixed neutral palette — no tier/rarity colour touches name or band.
...
def _name_footlight_pool(big, text, cx, cy, maxw, rect):
    """Name lit from below like theatre marquee footlights: a dense champagne
    pool anchored at the band's bottom edge, densest under the letters and
    fading to a defined upper falloff. Elliptical shape is faked by stacking
    soft_glow at several x-positions along the name so the field reads wide
    and shallow rather than as one round smudge."""
    f = font(8.5)
    base = _stamp_bold(_glyph_base(text, f, 0), m(0.9))
    nw = base.get_width()
    if nw > maxw:
        # keep the pool footprint tied to the on-card name width
        scale = maxw / nw
        base = pygame.transform.smoothscale(base, (int(nw*scale), int(base.get_height()*scale)))
        nw = base.get_width()

    # Footlight pool on a scratch surface, then composited so it can't tint
    # anything outside the band. Anchored near the band bottom, under letters.
    pool = pygame.Surface(big.get_size(), pygame.SRCALPHA)
    pool_cy = cy + m(9)
    half = nw // 2
    xs = [cx - half*0.72, cx - half*0.24, cx + half*0.24, cx + half*0.72]
    for gx in xs:
        soft_glow(pool, int(gx), pool_cy, m(10), CHAMPAGNE, 180, layers=12)
    big.blit(pool, (0, 0))

    # Bowed hairline base — gives the pool a deliberate bottom edge.
    ...
    for i in range(n + 1):
        t = i / n
        px = lx0 + (lx1 - lx0) * t
        # slight upward bow toward the middle
        py = ey - m(1.4) * math.sin(math.pi * t)
        pts.append((px, py))
    pygame.draw.lines(edge, (*CARD_RING_BRIGHT, 150), False, pts, max(1, m(1)))
    big.blit(edge, (0, 0))

    # Letters: warm-white, lower strokes sitting in the brightest pool.
...
def render_card(sid):
    ...
    _name_footlight_pool(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26), rect)
    return big
...
htxt = hfont.render("store_card_v4_r4_name_v3 — footlight-pool — round 1", True, (236,232,214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v3/footlight-pool/round_1.png"
