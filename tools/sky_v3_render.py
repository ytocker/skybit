"""ALPINE_HAZE fine-tune v3_deepen — colour pools into oxblood at base.

Only sky_bot and horizon differ from live ALPINE_HAZE at phases 0.27–0.52.

    python tools/sky_v3_render.py

Output: docs/sky_transition/v3_deepen/round_1.png
"""
import os
import sys
...
_OVERRIDES = {
    0.27: dict(sky_bot=(236, 154, 113), horizon=(255, 190, 138)),
    0.31: dict(sky_bot=(242, 135,  97), horizon=(255, 168, 104)),
    0.37: dict(sky_bot=(171,  43,  51), horizon=(238,  86,  58)),
    0.42: dict(sky_bot=(133,  32,  49), horizon=(206,  64,  58)),
    0.47: dict(sky_bot=( 96,  25,  44), horizon=(150,  52,  54)),
    0.52: dict(sky_bot=( 62,  24,  39), horizon=( 96,  44,  54)),
}
...
SPEC = BiomeSpec(
    name='alpine_haze_v3_deepen',
    note='v3 deepen - colour pools into oxblood base, bright then cliffs below mid',
    keyframes=_KF,
    sky=ALPINE_HAZE.sky,
)
...
def main():
    ...
    title = f_title.render("v3_deepen — colour pools and sinks to oxblood", True, TEXT_HI)
    ...
    out = os.path.join(_ROOT, "docs", "sky_transition", "v3_deepen", "round_1.png")
