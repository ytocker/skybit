...
def _name_filament_core(big, name, cx, cy, max_w):
    ...
    for char, adv in zip(name, advances):
        glyph_cx = x + adv // 2
        # Per-glyph mask keeps the bloom stroke-bound: each letter blooms from
        # its own filament so light never bridges the dark inter-letter band.
        tile = _glyph_base(char, f, 0)
        tw, th = tile.get_size()
        dst_x = glyph_cx - tw // 2
        dst_y = cy - th // 2

        # 2-tap offset-stamp bloom, additive + warm-tinted so strokes read as
        # heated filament rather than a flat wash.
        bloom1 = tile.copy()
        bloom1.fill((255, 240, 200, 70), special_flags=pygame.BLEND_RGBA_MULT)
        big.blit(bloom1, (dst_x, dst_y), special_flags=pygame.BLEND_ADD)
        bloom2 = tile.copy()
        bloom2.fill((255, 240, 200, 40), special_flags=pygame.BLEND_RGBA_MULT)
        big.blit(bloom2, (dst_x + 1, dst_y + 1), special_flags=pygame.BLEND_ADD)

        x += adv

    # Crisp ivory glyph on top with near-black keyline
    plain_text(big, name, f, (cx, cy), (250, 244, 218), shadow_a=0,
               weight=m(0.6), keyline=(8, 8, 20), kw=m(0.9))
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v4 — filament-core — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v4/filament-core/round_1.png"
