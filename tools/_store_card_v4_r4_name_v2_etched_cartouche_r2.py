...
def _name_etched_cartouche(big, name, cx, cy, max_w, rect):
    """Museum denomination-plate: cream engraved caps in a thin gold-hairline
    cartouche. Gold is the card-frame metal on every tier — furniture, not a
    rarity signal — so the band stays neutral dark and the name stays cream."""
    cream = (250, 248, 240)
    key = (6, 6, 16)
    # Tracking-first, then size: spread the caps like an engraved plate, and
    # only give up the airy tracking (then shrink) once a long name won't fit,
    # so short names read spacious and long names stay legible.
    size = 13.5
    f = font(size)
    tracking = m(1.2)
    while tracking > 0 and _glyph_base(name, f, tracking).get_width() > max_w:
        tracking -= 1
    # Keep a hair of tracking even on the longest names — collapsed-to-zero
    # letterspacing reads as body text, not an engraved plate.
    tracking = max(m(0.4), tracking)
    while size > 9.0 and _glyph_base(name, f, tracking).get_width() > max_w:
        size -= 0.5
        f = font(size)
    cap_h = f.get_height()
    # Incised-groove cue: lay the same glyph mask a hair DOWN in a dim warm
    # grey first, so each stroke keeps a shallow shadow trough on its bottom
    # edge once the cream fill lands on top — the minimum tell that reads as
    # engraved rather than painted-on inlay.
    mask = _glyph_base(name, f, tracking)
    shadow_layer = mask.copy()
    shadow_layer.fill((180, 178, 168, 255), special_flags=pygame.BLEND_RGBA_MULT)
    big.blit(shadow_layer,
             (cx - mask.get_width() // 2,
              cy - mask.get_height() // 2 + max(1, m(0.7))))
    plain_text(big, name, f, (cx, cy), cream, shadow_a=0, tracking=tracking,
               weight=m(0.8), keyline=key, kw=m(0.9))
    # Twin gold rules + end-ticks bracket the caps. Gold hairline furniture, so
    # the rules are quieted below full alpha — top a touch brighter than the
    # bottom for a lit-from-above read — while the end-ticks stay 2 px / full
    # alpha so they don't shimmer or break at card scale.
    gold = CARD_RING_BRIGHT
    top_y = cy - cap_h // 2 - m(3)
    bot_y = cy + cap_h // 2 + m(3)
    lx = rect.left + m(6)
    rx = rect.right - m(6)
    th = m(3)
    for ry, rule_a in ((top_y, 200), (bot_y, 170)):
        pygame.draw.line(big, (*gold, rule_a), (lx, ry), (rx, ry), 1)
        for tx in (lx, rx):
            pygame.draw.line(big, (*gold, 255), (tx, ry - th // 2), (tx, ry + th // 2), 2)
...
def render_card(sid):
    ...
    _name_etched_cartouche(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26), rect)
    return big
...
htxt = hfont.render("store_card_v4_r4_name_v2 — etched-cartouche — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/etched-cartouche/round_2.png"
