#!/usr/bin/env python3
"""
treasure-drawer  ·  confirm_purchase_v8  ·  round 2

Fixes from art-director critique:
- Compartment button gradient inverted (dark-top → light-bottom = true recess illusion)
- top_sheen removed from buttons (was fighting the recess)
- Inner-shadow alpha raised 90/60 → 155/80 (now legible)
- Bottom+right lip raised 18/12 → 70/50 (recess wall edge visible)
- BUY: tier-gem inner glow ring; body value raised ~28%
- CANCEL: gold rim stripped → dim neutral (80,80,100,120); flat dark recess
- Pull pip replaced with recessed finger-notch slot (dark slot + thin bright lip)
- Plate dark-side rim lifted (40,28,8) → (90,70,30) for gold on all 4 edges
"""
import os, sys
...
PLATE_CY, PLATE_W, PLATE_H = 281, 142, 34
BTN_CY,   BTN_W,   BTN_H   = 358, 104, 52
BTN_GAP   = 14
BUY_CX    = CX - (BTN_W + BTN_GAP) // 2   # = 71
CAN_CX    = CX + (BTN_W + BTN_GAP) // 2   # = 189
...
def price_plate(big, price_str, pal):
    """Flat gold-rimmed vault plate — dark-side rim lifted so gold reads on all 4 edges."""
    cx  = m(CX)
    cy  = m(PLATE_CY)
    w   = m(PLATE_W)
    h   = m(PLATE_H)
    ...
    # dark-side rim lifted (40,28,8)→(90,70,30) so gold reads on all edges
    sc.bevel_rim(big, rect, rad, (90, 70, 30), (*CARD_RING_BRIGHT, 230),
                 w=max(1, m(1.8)))
...
def compartment_button(big, cx_log, label, pal, is_buy):
    """Recessed compartment button — inverted gradient creates true recess depth."""
    cx  = m(cx_log)
    cy  = m(BTN_CY)
    w   = m(BTN_W)
    h   = m(BTN_H)
    ...
    if is_buy:
        # inverted gradient: dark top → light bottom (recess reads correctly)
        stops  = [(0.0, (16, 12, 28)), (0.5, (38, 34, 72)), (1.0, (58, 52, 96))]
        lab_c  = (210, 214, 246)
        # dim neutral bevel rim with tier gem glow instead of bright gold
        rim_b  = (*pal["gem"], 180)
    else:
        stops  = [(0.0, (12, 10, 22)), (1.0, (30, 26, 50))]
        lab_c  = (148, 146, 182)
        rim_b  = (80, 80, 100, 120)   # stripped gold rim → dim neutral

    sc.drop_shadow(big, rect, rad, blur=m(4), alpha=120, dy=m(3))
    big.blit(vgrad_stops(w, h, rad, stops, 255, gamma=1.0), rect.topleft)
    # no top_sheen — it fights the recess illusion

    # inner-shadow: top+left edge dark (alpha raised 90/60→155/80)
    inner = pygame.Surface((w, h), pygame.SRCALPHA)
    pygame.draw.line(inner, (0, 0, 0, 155), (rad, 0), (w - rad, 0), max(1, m(2)))
    pygame.draw.line(inner, (0, 0, 0, 80),  (0, rad), (0, h - rad), max(1, m(2)))
    # bottom+right lighter lip (alpha raised 18/12→70/50 — recess wall edge)
    pygame.draw.line(inner, (255, 255, 255, 70), (rad, h - 1), (w - rad, h - 1), max(1, m(1)))
    pygame.draw.line(inner, (255, 255, 255, 50), (w - 1, rad), (w - 1, h - rad), max(1, m(1)))
    big.blit(inner, rect.topleft)

    # BUY: tier-gem inner glow ring (replaces gold bevel on that button)
    if is_buy:
        glow_r = pygame.Surface((w, h), pygame.SRCALPHA)
        pygame.draw.rect(glow_r, (*pal["gem"], 45), glow_r.get_rect(),
                         width=max(2, m(3)), border_radius=rad)
        big.blit(glow_r, rect.topleft)

    sc.bevel_rim(big, rect, rad, CARD_RING_DEEP, rim_b, w=max(1, m(1.6)))

    # recessed finger-notch: dark horizontal slot + thin bright bottom lip
    notch_w  = m(20)
    notch_h  = m(5)
    notch_x  = cx - notch_w // 2
    notch_y  = y0 + h - m(11) - notch_h // 2
    pygame.draw.rect(big, (6, 4, 14),
                     pygame.Rect(notch_x, notch_y, notch_w, notch_h),
                     border_radius=m(2))
    pygame.draw.line(big, (72, 64, 96, 110),
                     (notch_x + m(2), notch_y + notch_h),
                     (notch_x + notch_w - m(2), notch_y + notch_h),
                     max(1, m(1)))

    lbl_fs = 13 if is_buy else 11
    lbl_cy = (rect.centerx, rect.centery - m(3))  # shift label up slightly for notch room
    plain_text(big, label, font(lbl_fs), lbl_cy, lab_c,
...
hdr  = _font(17, True).render("confirm_purchase_v8  ·  treasure-drawer  ·  round 2", True, (232, 226, 208))
sub1 = _font(10, True).render("inverted gradient recess · gem glow BUY · neutral CANCEL · finger-notch pip", True, (140, 148, 168))
...
OUT = "/home/user/skybit/docs/confirm_purchase_v8/treasure-drawer/round_2.png"
