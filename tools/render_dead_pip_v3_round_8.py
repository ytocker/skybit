...
OUT_PATH = pathlib.Path(__file__).parent.parent / "docs" / "dead_pip_v3" / "round_8_options.png"
...
def _draw_bold_x_on_lens(body, palette_key):
    ...
    B-ON: stroke 3, reach ±5. Bright chartreuse core on near-black lens —
    bright-on-dark stamp, eye reads one X.

    C-ON (round 7 fix): stroke 2, reach ±6. The round-6 darker eggplant
    core on the olive lens read as two parallel light highlight slashes per
    arm (plaid). Invert the value relationship so the lightest tone WINS
    (core = light eggplant; highlight = lighter near-white-eggplant) and
    drop stroke 3 → 2 — at thumbnail scale a 2 px stroke + bigger
    core/highlight contrast stops the two values fighting and forces a
    single-glyph read. Reach grows ±5 → ±6 so the thinner stroke still
    breaks the rim (lens radius 6) by ~1 px on every side, preserving the
    "stamped over the lens" idiom.
...
def render():
    ...
    title_str = "DEAD PIP v3  ·  round 8 — sunglasses-ON: bold palette X on lens"
    ...
    # ── Round-7 silhouette-break assert: C-ON left-lens outer-arm
    #    sample at sprite-local (40, 14). LENS_L = (46, 20); the upper-
    #    left X tip at reach 6 lands at (40, 14). After the rim radius is
    #    6 (so the rim is at distance 6 from centre), this pixel sits
    #    JUST outside the lens — confirms the X arm broke the silhouette
    #    rather than being contained inside the rim. Use the 5×5 helper
    #    because the rendered tip can drift one pixel at this scale.
    gx, gy, _zw, _zh, _spr = geom_by_key[("C", "on")]
    tip_x = gx + 3 * 40
    tip_y = gy + 3 * 14
    expected_core = _expected_bold_x_core("C")
    found, (hit_x, hit_y), best_pix = _find_color_in_5x5(
        surf, tip_x, tip_y, expected_core, tol=14
    )
    print(f"[fix-silhouette] C-ON left-lens outer-arm sample at sprite-"
          f"local (40, 14) → panel ({tip_x},{tip_y}) closest = {best_pix} "
          f"at ({hit_x},{hit_y})  expected core ≈ {expected_core}  "
          f"found={found}")
    if not found:
        print(f"[fix-silhouette] WARNING: corner-pixel sample didn't land — "
              f"this is sensitive to pygame's width-2 line endpoint "
              f"coverage. Art-director will judge silhouette-break "
              f"visually from the rendered PNG.")
