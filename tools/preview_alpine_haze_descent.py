"""IN-GAME descending-threshold sweep of the live Coral Ember sky — HONEST TIME AXIS.

Six rows (Original static + 5 descent rates) of
`tools.sky_alpine_haze_descent.VARIANTS` across the day, rendered through the REAL
game path (`App._draw_background -> game.sky_designs.render_active`). Because the
threshold now descends over time, each rate row swaps a per-COLUMN descended spec
into the live registry slot before rendering that column, so the warm line starts
at its original height at golden hour and sinks across the sunset/dusk columns
(rising back at dawn). Rows differ in descent RATE.

Scene-only to match the prior sheets: mountains + pagodas + parrot over the empty
sidewalk; no pillars/coins/HUD/people.

Output: docs/biome_redesign/alpine_haze_threshold_descent_ingame.png

    python tools/preview_alpine_haze_descent.py
"""
import os
import sys
...
def main():
    ...
    sheet.blit(f_title.render(
        ...
        "rates (in-game, HONEST TIME axis, real render path)",
    ...
    sheet.blit(f_sub.render(
        ...
        "dawn). Rows differ in descent RATE. Painted via render_active. Read each "
        "row left->right across golden/sunset/dusk: the warm band sinks.",
    ...
    try:
        for r, (label, drop) in enumerate(rows):
            ...
            for c, phase in enumerate(PHASES):
                x = GUT + c * (TW + PAD)
                # Per-column descended spec (positions vary with phase), swapped
                # into the live slot; clear the per-bucket cache so render_active
                # rebakes with this column's positions.
    ...
    out = os.environ.get("SKY_SHEET_OUT") or os.path.join(
        ...
        "alpine_haze_threshold_descent_ingame.png")
