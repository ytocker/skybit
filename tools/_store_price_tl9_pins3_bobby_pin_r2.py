"""Round-2 render for tl9 pins3: bobby_pin (hair grip — U-loop + straight/crimped prong pair)"""
import os
...
def _attachment(card, affordable):
    import math
    if affordable:
        body     = (34,  34,  38)    # near-black lacquer — dominant read
        hi       = (88,  90,  96)    # subtle shine on outer prong top edge
        super_hi = (148, 150, 156)   # single-pixel specular on U-bend apex
        kl       = (16,  16,  20)
    else:
        body     = (50,  52,  58)
        hi       = (100, 102, 108)
        super_hi = (158, 160, 166)
        kl       = (16,  16,  20)

    # === HORIZONTAL LAYOUT ===
    # Outer straight prong: y=10, x: 6 → 22
    pygame.draw.line(card, kl,   (6, 10), (22, 10), 4)
    pygame.draw.line(card, body, (6, 10), (22, 10), 2)
    pygame.draw.line(card, hi,   (6,  9), (22,  9), 1)   # top-edge shine

    # Inner crimped prong: y≈18, with gentle waves for grip texture
    pts = [(6, 18), (9, 17), (12, 18), (15, 17), (18, 18), (21, 17), (22, 18)]
    pygame.draw.lines(card, kl,   False, pts, 4)
    pygame.draw.lines(card, body, False, pts, 2)

    # U-bend at left connecting the two prongs — left-facing semicircle
    # Rect covers y:10..18 (height=8), center x≈6, radius=4
    pygame.draw.arc(card, kl,   pygame.Rect(2, 10, 8, 8), math.radians(90), math.radians(270), 4)
    pygame.draw.arc(card, body, pygame.Rect(2, 10, 8, 8), math.radians(90), math.radians(270), 2)
    # Specular dot at U-bend apex (leftmost point of arc)
    pygame.draw.circle(card, super_hi, (2, 14), 1)
