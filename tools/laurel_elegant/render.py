"""Headless render harness for the elegant Olympic-LAUREL ring concepts.
...
Run: PYTHONPATH=. SDL_VIDEODRIVER=dummy python tools/laurel_elegant/render.py
"""
import os
...
from tools.laurel_elegant.concepts import CONCEPTS
...
os.makedirs("docs/laurel_elegant", exist_ok=True)
...
def hero(fn, glyph, badge=200, margin=0.16):
    """A large hero with the red badge-bounds square marked, so any leaf spilling
    past the badge square is visible."""
...
def row_strip(fn, glyph):
    ...
    shown a few times + at 32/44/56, to verify it READS at scan size."""
    row_h = 56
    W = 300
...
def concept_figure(idx, name, fame_fn, shame_fn):
    ...
    out.fill(BG)
    out.blit(font(30).render(f"LAUREL · {name.upper()}  (elegant / Olympic)",
                             True, INK), (pad, 18))
    ...
    path = f"docs/laurel_elegant/concept_{idx}_{name}.png"
...
def showcase(pairs):
    ...
    out = pygame.Surface((W, H))
    out.fill(BG)
    out.blit(font(32).render("ELEGANT OLYMPIC LAUREL — Fame vs Shame (5 concepts)",
                             True, INK), (pad, 18))
    ...
    path = "docs/laurel_elegant/showcase.png"
    pygame.image.save(out, path)
    return path
