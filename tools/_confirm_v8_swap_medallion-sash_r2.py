#!/usr/bin/env python3
"""medallion-sash · confirm_purchase_v8 · swap-round-1 · round_2"""
import os, sys, math
...
# gloss_sweep shipped with BLEND_RGBA_MIN mask + BLEND_ADD composite — the
# module default skips BLEND_ADD so highlights disappear on dark card bodies.
...
# CHIP_CY raised 3px from r1=247 → 250 for extra clearance under the name glyph
...
def zone_a_chip(big, price_str, pal):
    # Round-capped medal bar: 122px wide, 27px tall, centred at cy=250.
    # No chain stubs — the gold pill already reads as a struck medal on its own.
    w, h = m(122), m(27)
...
def zone_b_banner(big, tier_word, pal):
    # Dark navy ribbon body — same colour on all tiers so LEGENDARY gold in Zone A
    # cannot collide with the Zone B fill. Tier identity is carried only in:
    #   (a) the bevel rim at alpha=150 (up from r1's 90), and
    #   (b) a 1-2px additive underglow at the bottom edge.
    # Width extended to x=41–219 so sash ends overlap each gem center by 2px;
    # bottom_gems() is called after this function so gems composite on top as brooches.
    w, h = m(178), m(21)
    rad = m(4)
    rect = pygame.Rect(m(41), 0, w, h); rect.centery = m(Y_BANNER)

    # Tier-independent dark fill
    big.blit(vgrad_stops(w, h, rad,
                         [(0.0,(28,30,60)),(1.0,(16,18,48))], 255), rect.topleft)
    sc.top_sheen(big, rect, rad, m(8), peak=28)

    # Bevel rim carries the tier accent; stronger alpha than r1 to compensate for
    # the neutral body — tier colour still registers at a glance without drowning text
    sc.bevel_rim(big, rect, rad, CARD_RING_DEEP, (*CARD_RING_BRIGHT, 150),
                 w=max(1, m(1.2)))

    # 1-2px additive underglow: tier glow bleeds up from sash floor so rarity reads
    # as a warm cast rather than a dominant fill — preserves dark-body legibility
    gc = pal["glow"]
    glow_h = max(1, m(2))
    uglow = pygame.Surface((w, glow_h), pygame.SRCALPHA)
    for gy in range(glow_h):
        # Intensity peaks at the bottom edge and falls to zero moving upward
        v = int(55 * (1 - gy / max(1, glow_h)))
        r_v = min(255, gc[0] * v // 100)
        g_v = min(255, gc[1] * v // 100)
        b_v = min(255, gc[2] * v // 100)
        pygame.draw.line(uglow, (r_v, g_v, b_v, 255), (0, gy), (w - 1, gy))
    big.blit(uglow, (rect.left, rect.bottom - glow_h), special_flags=pygame.BLEND_ADD)

    # Constant light label with dark keyline — identical across all tiers so the
    # text is always legible regardless of whether rim accent is warm or cool
    plain_text(big, tier_word, font(10), rect.center,
               (245, 242, 235), shadow_a=120, weight=m(0.7),
...
def render_popup(tw, sid, ps, pal):
    ...
    # zone_b_banner before bottom_gems — gems composite on top as ribbon brooches
...
idr.text((MARGIN, 18), "medallion-sash · swap-round-1 · round 2", fill=(232,226,208))
...
import pathlib
pathlib.Path("/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/medallion-sash").mkdir(parents=True, exist_ok=True)
OUT = "/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/medallion-sash/round_2.png"
out.save(OUT)
