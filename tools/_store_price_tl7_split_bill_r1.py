"""Round-1 render for tl7 numeral concept: split-bill.

Two-line denomination: coin/dollar glyph above + full comma-price below,
separated by a thin rule. Bank-note denomination character.
...
def _draw_price(face, text, center, affordable):
    """Two-line: coin glyph at top, full price below, thin rule between."""
    cx = center[0]
    ink = (52, 38, 30) if affordable else (58, 62, 74)
    coin_col = (210, 164, 58) if affordable else (150, 156, 172)
    rule_col = (160, 120, 40) if affordable else (96, 100, 116)
    key_col = (30, 20, 14) if affordable else (36, 40, 50)

    # Amount line — full comma-formatted price
    amt_mask = _price_glyph(text)
    amt_cy = int(FACE_H * 0.65)
    amt_r = amt_mask.get_rect(center=(cx, amt_cy))
    ...
    for ang in range(0, 360, 45):
        ...
        face.blit(kl, (amt_r.x + dx, amt_r.y + dy))
    ...
    # Split rule between glyph and amount
    rule_y = amt_r.top - sc.m(3)
    rule_w = min(amt_mask.get_width() + sc.m(4), FACE_W - sc.m(8))
    rule_h = max(2, sc.m(1))
    pygame.draw.rect(face, (*rule_col, 200),
                     (cx - rule_w // 2, rule_y - rule_h // 2, rule_w, rule_h))

    # Coin/dollar glyph above the rule
    glyph_fs = 8
    coin_mask = sc._stamp_bold(sc._glyph_base("$", sc.font(glyph_fs), 0), sc.m(0.8))
    coin_cy = rule_y - sc.m(3) - coin_mask.get_height() // 2
    coin_r = coin_mask.get_rect(center=(cx, coin_cy))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("SPLIT-BILL price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl7/split_bill/round_1.png"
