"""Round-1 render for tl7 numeral concept: ribbon-band.

Full-face-width mahogany sash with cream numeral reversed out.
Fishtail notches on left/right ends. Luxury gift-price-band character.
...
def _draw_price(face, text, center, affordable):
    ...
    band_x = sc.m(2)
    band_w = FACE_W - sc.m(4)

    # Band gradient (affordable: warm mahogany; locked: cool grey)
    if affordable:
        band_stops = [(0.0, (196, 150, 58)), (1.0, (150, 104, 28))]
        notch_col = (120, 84, 20)
        sheen_col = (255, 240, 190, 90)
        num_col = (250, 244, 220)
        key_col = (80, 52, 12)
    else:
        band_stops = [(0.0, (150, 156, 172)), (1.0, (104, 110, 128))]
        notch_col = (78, 84, 100)
        sheen_col = (220, 224, 236, 70)
        num_col = (230, 234, 244)
        key_col = (48, 52, 66)

    band_surf = sc.vgrad_stops(band_w, band_h, 0, band_stops, 255)
    face.blit(band_surf, (band_x, band_top))

    # Fishtail notches: triangle bites cut into band at left and right ends
    notch_d = sc.m(4)  # notch depth
    # Left notch: dark triangle (covers the band corner)
    lx = band_x
    mid_y = band_top + band_h // 2
    pygame.draw.polygon(face, (*notch_col, 255), [
        (lx, band_top), (lx + notch_d, mid_y), (lx, band_top + band_h)
    ])
    # Right notch
    rx = band_x + band_w
    pygame.draw.polygon(face, (*notch_col, 255), [
        (rx, band_top), (rx - notch_d, mid_y), (rx, band_top + band_h)
    ])

    # Top sheen line along the band
    pygame.draw.line(face, sheen_col,
                     (band_x + notch_d, band_top),
                     (band_x + band_w - notch_d, band_top),
                     max(2, sc.m(1)))

    # Numeral: cream reversed out of the band, with dark keyline for crisp edge
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("RIBBON-BAND price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl7/ribbon_band/round_1.png"
