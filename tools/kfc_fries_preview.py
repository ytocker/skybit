"""Render 5 separate review images, one per KFC fry variant.

Each file shows a single fry on a navy backdrop at 6× scale so the
detail is legible. Writes docs/kfc_fries_<slug>.png.

Run:
    python tools/kfc_fries_preview.py
"""
import os
import pathlib
import sys
...
from game.kfc_fries import VARIANTS
...
def render_variant(name: str, draw_fn) -> pygame.Surface:
    ...
    _draw_centered(surf, f"Skybit — KFC fry — {name}",
...
def main():
    ...
    for name, fn in VARIANTS:
        ...
        slug = name.lower()
        out  = OUT_DIR / f"kfc_fries_{slug}.png"
