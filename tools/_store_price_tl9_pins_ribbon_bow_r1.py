"""Round-1 render for tl9 numeral concept: ribbon_bow.
...
split-lit polygon wings with a hard lit/shadow diagonal, a center knot rect, and
two chevron-notched tails dropping below. Warm rose when affordable, cool slate
when locked. Review sheet only — not wired live.
"""
import os
...
def _ribbon_bow(surf, tag_center, affordable):
    ...
    lit   = (214, 96, 118) if affordable else (100, 108, 148)
    shadow = (158, 58, 80)  if affordable else (68, 74, 108)

    # Left wing: lit half (outer) + shadow half (inner edge toward center)
    # Each wing is two polygons sharing a diagonal edge
    left_lit = [
        (bx - 14, by - 4),
        (bx - 2, by),
        (bx - 2, by - 10),
    ...
    # Center knot rect — lit color, clears sc.m(2) in both dims
    knot_w, knot_h = sc.m(4), sc.m(3)
    knot_rect = pygame.Rect(bx - knot_w // 2, by - knot_h // 2, knot_w, knot_h)
    pygame.draw.rect(surf, lit, knot_rect)

    # Tails: two strips dropping from knot bottom, angled slightly outward
    tail_top = by + knot_h // 2
    tail_len = sc.m(5)
    # Left tail strip (polygon with chevron notch at bottom)
    lt = [
        (bx - knot_w // 2, tail_top),
        (bx - knot_w // 2 - sc.m(2), tail_top + tail_len),
        (bx - sc.m(1), tail_top + tail_len - sc.m(2)),   # chevron notch
        (bx, tail_top),
    ]
    # Right tail strip
    rt = [
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 pins: ribbon_bow — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9_pins/ribbon_bow/round_1.png"
