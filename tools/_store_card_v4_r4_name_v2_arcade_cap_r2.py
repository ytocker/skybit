...
def _name_arcade_cap(big, name, cx, cy, max_w):
    # Extruded plastic cap: a bright top lip peeks ABOVE the cream cap line and a
    # dark under-lip drops BELOW the baseline, reading as a moulded arcade key.
    # Both lips are full-opacity so the extrusion holds at card size; colors are
    # fixed (never tier-reactive) to keep this the highest-legibility name in the set.
    sz = 13.0
    while sz > 9.0:
        f = font(sz)
        if _stamp_bold(_glyph_base(name, f, 0), m(1.1)).get_width() <= max_w:
            break
        sz -= 0.5
    f = font(sz)
    w_mask = _stamp_bold(_glyph_base(name, f, 0), m(1.1))
    mw, mh = w_mask.get_size()
    # Device-pixel offset (not sub-pixel) so both lips clear the cream fill's edge.
    lift = max(2, m(1.5))

    # Bottom dark lip first — dark navy-indigo drop that the cream fill sits over,
    # so only the sliver below the baseline shows as the cap's shadowed underside.
    bot_lip = w_mask.copy()
    bot_lip.fill((22, 20, 36, 255), special_flags=pygame.BLEND_RGBA_MULT)
    big.blit(bot_lip, (cx - mw // 2, cy - mh // 2 + lift))

    # Top bright lip — bright-white, a wide value gap from the cream so it reads
    # as a lit rim sitting proud of the cap's top face.
    top_lip = w_mask.copy()
    top_lip.fill((255, 255, 252, 255), special_flags=pygame.BLEND_RGBA_MULT)
    big.blit(top_lip, (cx - mw // 2, cy - mh // 2 - lift))

    # Flat cream face + chunky dark keyline — the crisp arcade-cap weight.
    plain_text(big, name, f, (cx, cy), (250, 248, 240), shadow_a=0,
               weight=m(1.1), keyline=(12, 12, 24), kw=m(1.2))
...
def render_card(sid):
    ...
    _name_arcade_cap(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26))
    return big
...
htxt = hfont.render("store_card_v4_r4_name_v2 — arcade-cap — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/arcade-cap/round_2.png"
