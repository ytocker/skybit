"""poster-backdrop — store_card_v3_tl concept, round 2 headless render.
...
palette with two silhouetted sandstone pillars staggered at depth and tinted
...
Round 2 (art-director notes): gold-keylined price chip; a 2px keyline + a
gradient scrim so the band always reads as a brighter poster strip on every
tier; deeper, warm-rimmed pillars pushed to LEGENDARY contrast; a soft contact
shadow lifting the medallion off cooler skies; and a cyan RARE that survives
deuteranopia against the purple EPIC.
...
live store; writes docs/store_card_v3_tl/poster-backdrop/round_2.png.
"""
import os
...
def _disc_contact_shadow(surf, cx, cy, disc_r):
    """A soft elliptical drop shadow directly under the medallion, faked from 3
    concentric ellipses of decreasing spread + rising alpha so it feathers like
    a blur. It seats the disc on the sky on the cooler tiers where the hero
    would otherwise float; on LEGENDARY the sky is already high-contrast so this
    is skipped by the caller."""
    base_w = m(R * 1.3)
    base_h = m(8)
    scy = cy + disc_r - m(2)
    pad = m(6)
    sh = pygame.Surface((base_w + pad * 2, base_h + pad * 2), pygame.SRCALPHA)
    sc = (sh.get_width() // 2, sh.get_height() // 2)
    for grow, a in ((m(4), 20), (m(2), 38), (0, 55)):
        er = pygame.Rect(0, 0, base_w + grow * 2, base_h + grow)
        er.center = sc
        pygame.draw.ellipse(sh, (0, 0, 0, a), er)
    surf.blit(sh, (cx - sc[0], scy - sc[1]))
...
def _pillar(surf, clip_rect, x, top_y, w, h, color, alpha):
    ...
    A thin warm rim runs the sunlit LEFT edge so the slab reads as lit
    architecture, not a flat cut-out. Clipped to the card body so a pillar that
    overruns the bottom never spills onto the bevel."""
    ...
    pygame.draw.line(surf, (200, 185, 145, 80),
                     (x - w // 2, top_y), (x - w // 2, top_y + h), max(1, m(1)))
    surf.set_clip(prev)


def render_card(tier, sid, pal, price, name):
    ...
    # 2. PILLAR SILHOUETTES — warm sandstone tinted to the tier, deepened to
    #    ~30 luma below their local sky so they read at LEGENDARY contrast on
    #    every tier. Two staggered slabs (a third would crowd the 162px width);
    #    each carries a warm sunlit rim on its left edge.
    pillar_col = lerp_color(pal["deep"], (30, 26, 20), 0.55)
    ...
    # 6. CONTACT SHADOW under the hero on the cooler tiers — LEGENDARY's sky is
    #    already high contrast, so it's skipped there to avoid muddying the gold.
    if not is_legend:
        _disc_contact_shadow(big, cx, cy, m(R))

    # 7. HERO MEDALLION — domed glass well → skin → tier tint → glass dome →
    ...
    # 8. gutter halo → floods the sky around the disc with the tier hue so the
    #    rarity read carries beyond the bezel.
    _gutter_aura(big, cx, cy, m(R), m(R + 24), pal["glow"], peak=80, layers=16)

    # 9. BOTTOM POSTER-TITLE BAND — a full-width tier-gradient strip (deep→glow)
    #    carrying the item name. A gradient scrim darkens the 12px of sky
    #    directly above it and a solid 2px gold keyline caps it, so the band
    #    always reads as a brighter poster strip regardless of tier. LEGENDARY
    #    fills toward pal["gem"] gold so the band separates from its amber sky.
    band_h = m(20)
    band_rect = pygame.Rect(rect.x, rect.bottom - band_h, rect.w, band_h)
    scrim_h = m(12)
    scrim = pygame.Surface((rect.w, scrim_h), pygame.SRCALPHA)
    for sy in range(scrim_h):
        sa = int(60 * sy / max(1, scrim_h - 1))
        pygame.draw.line(scrim, (0, 0, 0, sa), (0, sy), (rect.w, sy))
    big.blit(scrim, (rect.x, band_rect.top - scrim_h))
    band_glow = pal["gem"] if is_legend else pal["glow"]
    band = pygame.Surface((band_rect.w, band_rect.h), pygame.SRCALPHA)
    for bx in range(band_rect.w):
        t = bx / max(1, band_rect.w - 1)
        bc = lerp_color(pal["deep"], band_glow, t)
        pygame.draw.line(band, (*bc, 210), (bx, 0), (bx, band_h))
    pygame.draw.line(band, (*CARD_RING_BRIGHT, 220), (0, 0),
                     (band_rect.w, 0), max(1, m(2)))
    big.blit(band, band_rect.topleft)
    plain_text(big, name, font(9.0), band_rect.center, (236, 230, 208),
               shadow_a=160, weight=m(0.7))

    # 10. PRICE FROSTED CHIP (top-right) — a dark-glass pill so the price stays
    #     legible over the bright sky. A gold keyline ties it to the bevel
    #     identity and gives it a hard edge against the gradient behind it.
    ph = m(14)
    price_str = f"{price:,}"
    pf = font(8.0)
    ...
    pygame.draw.rect(chip, (8, 10, 28, 220), (0, 0, pw, ph), border_radius=ph // 2)
    pygame.draw.rect(chip, (*CARD_RING_BRIGHT, 200), (0, 0, pw, ph),
                     width=max(1, m(1.5)), border_radius=ph // 2)
    big.blit(chip, (pcx - pw // 2, pcy - ph // 2))
    plain_text(big, price_str, pf, (pcx, pcy), (236, 230, 208),
               shadow_a=0, weight=m(0.7))

    # 11. GEM CREST (top-left) — faceted tier badge per the locked spine.
    facet_gem(big, rect.x + m(19), rect.y + m(19), m(GEM_R + 2),
              pal["gem"], pal["deep"])

    # 12. bevel rim + dark keyline LAST so the card frame stays crisp over the
...
# ── review sheet ──────────────────────────────────────────────────────────────
# RARE is pushed cool/cyan (was blue) so RARE vs EPIC survives deuteranopia —
# cyan reads apart from EPIC's purple where the original blue collapsed into it.
VARIANTS = [
    ("RARE",      "skin_lorikeet", {"gem": (80, 200, 240),  "glow": (40, 160, 210), "deep": (10, 50, 100)}, 600),
...
ffont = _font(20, True)
sfont = _font(16, True)
htxt = hfont.render("store_card_v3_tl — poster-backdrop — round 2", True, (236, 232, 214))
...
for i, (tier, sid, pal, price) in enumerate(VARIANTS):
    ...
    panel, _ = render_card(tier, sid, pal, price, ITEM_NAMES[sid])
    panels.append(panel)
...
out = "/home/user/skybit/docs/store_card_v3_tl/poster-backdrop/round_2.png"
