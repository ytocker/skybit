"""Promenade PLANTS & GREENERY variety — round 2 candidate-sheet generator.
...
Round 2 acts on the art-director's ITERATE notes — the vessel-driven silhouette
strategy is right and ~2/3 read cleanly; the rounded-mound family was crowded.
The pool is TRIMMED from 12 to 10 distinct silhouettes:

  KEPT untouched (approved):  G1 terracotta shrub (default), G2 trough conifer,
    G3 urn topiary-3, G7 bamboo (the standout).
  MERGES (kill the recolored twins):
    - floral pair G5/G6 -> ONE flowering design, KEEP the white-urn G5 (the urn
      separates it from the terracotta crowd). The 2nd floral is now a genuinely
      DIFFERENT FORM: a cascading flowering VINE, not a recolored mound.
    - wish-tree pair G11/G12 -> ONE, differentiated from any other tree by a
      broad WEEPING canopy form (not ribbon density, which doesn't survive size).
  RESCUES:
    - G4 topiary-1: the bare-stem "lollipop" is EXAGGERATED — a clearly tall
      bare stem + one ball, unmistakable vs the solid domes.
    - G8 vine/fern: FEWER, THICKER trailing strands (continuous 1-2px drape) so
      it survives 1x and night instead of vanishing.
    - G9 kumquat: now a TALLER, more-OPEN fruiting-tree canopy on a short trunk,
      so it's not "shrub + orange dots" beside G1.
  CUT: G10 snow-dust (decorative-only; thinning the rounded-mound herd).
  TONED: G5 blossom chroma (day) and the wish-tree ribbon-red (night) each
    knocked down a notch so neither approaches focal beside the coin in
    peripheral motion. G2 granite vessel lifted ~5% on night so it doesn't merge
...
def _sp_vine(surf, cx, rim_y, v, night, t):
    """Trailing vine / fern — a small upright tuft with FEW, THICK strands forming
    a CONTINUOUS drape over the vessel's front lip onto the deck. Round 1's many
    thin strands nearly vanished at 1x/night; this draws 2 bold draped ropes plus
    one short accent, each a solid 2px core with a 1px highlight, so the 'spilling
    over' silhouette survives true size and the night cool."""
    P = v.palette
    dark, mid, top = _foliage(P, night)
    base = rim_y
    # small upright fern tuft on top of the trough
    _dome(surf, cx, base - 3, 5, 4, dark, mid, top)
    for ang in (1.9, 2.3, 0.9, 1.3):
        _leaf_tuft(surf, cx, base - 4, ang, 6, mid, n=2, spread=0.4)
    # FEW, THICK cascading ropes: a continuous 2px drape (1px highlight) over the
    # front lip and down the vessel face, sway-animated at the tail only.
    sway = math.sin(t * 1.8)
    ropes = ((-6, 17, 2.0), (1, 21, 2.6), (6, 13, 1.6))
    for k, (sx0, ln, swv) in enumerate(ropes):
        pts = [(cx + sx0, base + 1)]
        for i in range(1, ln + 1):
            tt = i / ln
            px = cx + sx0 - int(math.sin(tt * 1.9) * swv) + int(sway * tt * 1.5)
            py = base + 1 + int(tt * (ln + 3))
            pts.append((px, py))
        # solid continuous core so the drape never breaks into dots
        pygame.draw.lines(surf, dark, False, pts, 2)
        pygame.draw.lines(surf, mid, False, pts, 1)
        # a few bold teardrop leaves clinging to the rope, spaced so they read
        for ni in (max(1, ln // 3), max(2, (2 * ln) // 3), ln - 1):
            if ni >= len(pts):
                continue
            px, py = pts[ni]
...
def _sp_flovine(surf, cx, rim_y, v, night, t):
    """Cascading FLOWERING vine — the 2nd floral, a genuinely DIFFERENT FORM from
    the rounded flowering mound: a compact leafy head whose blossomed strands
    SPILL down over the urn lip (wisteria/bougainvillea idiom). The read is a
    vertical curtain of muted blossom, not a dome — so it separates cleanly from
    G5 in silhouette, not just colour."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    # a small leafy head sitting on the rim
    _dome(surf, cx, base - 4, 6, 4, dark, mid, top)
    bloom = P.get("accent", (200, 132, 170))
    dc = A.get("day_chroma", 188)
    ac = _accent(bloom, night, day_ceil=dc)
    ac_dk = _accent(_shade(bloom, -28), night, day_ceil=dc)
    ac_lt = _accent(_shade(bloom, 22), night, day_ceil=dc)
    sway = math.sin(t * 1.7)
    # blossomed strands cascading down the front of the urn — continuous 1-2px
    # green ropes with blossom dots clustered along them
    strands = ((-7, 16, 1.6), (-2, 21, 2.2), (3, 19, 2.0), (7, 14, 1.4))
    for k, (sx0, ln, swv) in enumerate(strands):
        pts = [(cx + sx0, base - 1)]
        for i in range(1, ln + 1):
            tt = i / ln
            px = cx + sx0 - int(math.sin(tt * 1.7) * swv) + int(sway * tt * 1.4)
            py = base - 1 + int(tt * (ln + 2))
            pts.append((px, py))
        pygame.draw.lines(surf, dark, False, pts, 1)
        # blossom dots clustered down the strand (denser toward the tip)
        for j, (px, py) in enumerate(pts):
            if j < 2 or j % 2:
                continue
            col = ac if (j + k) % 3 else ac_lt
            pygame.draw.circle(surf, ac_dk, (px, py), 1)
            if j % 3 == 0:
                pygame.draw.circle(surf, col, (px, py), 0)
        pygame.draw.circle(surf, ac, pts[-1], 1)
...
# TRIMMED to 10 distinct silhouettes (was 12): VESSEL x SPECIES combinations.
# ════════════════════════════════════════════════════════════════════════════
...
POOL = [
    ("G1 terracotta SHRUB", _row(
     ...
     "vessel:terracotta(flared tapered pot) species:shrub(round bushy domes) accent:muted-rose dots | warm clay + leafy green — DEFAULT, replaces today's shrub planter [KEPT untouched: approved]"),

    ("G2 stone-trough CONIFER", _row(
        STONE, FOL_DARK,
        vessel="trough", species="conifer", vlift=0.10),
     "vessel:trough(low granite basin, NIGHT value +5% so it won't merge with dark ground) species:conifer(tall stacked-tier cone) | dark cypress SPIRE — replaces today's conifer planter [approved + night lift]"),

    ("G3 glazed-urn TOPIARY-3", _row(
        URN, FOL_CLIP,
        vessel="urn", species="topiary", tiers=3, trunk=(96, 84, 66)),
     "vessel:urn(blue-white belly+neck, painted glaze band) species:topiary tiers:3(stacked clipped balls on a bare stem) | porcelain + geometric balls [KEPT untouched: approved]"),

    ("G4 terracotta TOPIARY-1 (lollipop)", _row(
        TERRA, FOL_CLIP,
        vessel="terracotta", species="topiary", tiers=1, trunk=(110, 84, 56)),
     "vessel:terracotta species:topiary tiers:1(EXAGGERATED lollipop: long BARE stem + one ball lifted clear of the rim) | unmistakable vs the solid domes [RESCUED]"),

    ("G5 glazed-urn FLOWERING(plum)", _row(
        URN, FOL_DARK, dict(accent=(204, 122, 142)),
        vessel="urn", species="flowering", day_chroma=170),
     "vessel:urn species:flowering accent:muted PLUM blossom (dense 5-petal scatter, DAY chroma knocked down a notch) | white urn separates it from the terracotta crowd [MERGED G5/G6 -> kept G5, toned]"),

    ("G6 glazed-urn FLOWERING-VINE(cascade)", _row(
        URN, FOL_LEAFY, dict(accent=(192, 120, 168)),
        vessel="urn", species="flovine", day_chroma=172),
     "vessel:urn species:flovine(compact leafy head + blossomed strands SPILLING down the urn face — a vertical CURTAIN of bloom, not a dome) | the 2nd floral as a genuinely DIFFERENT FORM [NEW, replaces recolored twin]"),

    ("G7 bamboo-planter BAMBOO", _row(
        BAMBOO_V, FOL_LEAFY, dict(trunk=(150, 178, 108)),
        vessel="bamboo", species="bamboo"),
     "vessel:bamboo(lashed cane cylinder) species:bamboo(tall thin segmented canes, sparse leaf tufts, NO crown) | the tallest/airiest vertical silhouette [KEPT untouched: the standout]"),

    ("G8 stone-trough VINE/FERN", _row(
        STONE, FOL_LEAFY, dict(vlift=0.10),
        vessel="trough", species="vine"),
     "vessel:trough species:vine(upright fern tuft + FEW, THICK strands forming a CONTINUOUS 2px drape over the front lip onto the deck) | survives 1x + night now [FIXED: was the failure]"),
    ...
    ("G9 terracotta KUMQUAT (fruiting tree)", _row(
     ...
     "vessel:terracotta species:kumquat(short bare TRUNK lifting a TALLER, more-OPEN airy canopy of orange fruit — a little tree, not a dome) | distinct vs G1 shrub now [DIFFERENTIATED]"),
    ...
    ("G10 WISH-TREE (broad weeping)", _row(
     ...
     "vessel:urn species:wishtree mass:1.15(TALLER than a pot) FORM:broad WEEPING crown with drooping outer fronds + red prayer ribbons (NIGHT red toned a notch) | the single wish-tree, distinct by canopy FORM [MERGED G11/G12 -> one]"),
]
...
def render():
    ...
    y = PAD
    _text(sheet, "SKYBIT PROMENADE — PLANTS & GREENERY (round 2): TRIMMED to 10 distinct VESSEL x SPECIES silhouettes",
          PAD, y, 17, (250, 246, 236), bold=True)
    y += 22
    _text(sheet, "Round-1 herd thinned: floral pair MERGED (kept white-urn G5, toned) + 2nd floral is now a DIFFERENT FORM (G6 cascading flowering-vine); wish-tree pair MERGED to ONE "
                 "(G10 broad WEEPING form). RESCUED: G4 exaggerated lollipop, G8 thick continuous vine drape, G9 open fruiting-TREE kumquat. CUT: snow-dust. KEPT untouched: G1/G2*/G3/G7. "
                 "*G2 granite +5% on night. Night cooled <=150, accents (blossom/fruit/ribbon) held under the coin.",
          PAD, y, 9, (188, 186, 200))
    y += title_h - 22
    ...
    _text(sheet, "C.  ON-STREET COMPOSITE — the TRIMMED final 10 side by side at true size among human-cast figures + a food stall for scale, with the coin reference  (DAY then NIGHT)",
          PAD, y, 12, (240, 220, 150), bold=True)
    y += 18
    for is_night in (False, True):
        night = 0.95 if is_night else 0.0
    ...
    out = "/home/user/skybit/docs/sidewalk_overhaul/greenery/round_2.png"
