"""Round-2 render for tl9 pins2: paper_clip (racetrack shape + inner stub)"""
import os
...
def _attachment(card, affordable):
    metal = (112, 116, 124) if affordable else (80, 84, 94)
    hi = (160, 164, 172) if affordable else (120, 124, 134)
    kl = (24, 26, 34)
    # outer racetrack: elongated, flat-sided — reads as paper clip, not oval
    outer = pygame.Rect(3, 8, 22, 11)
    pygame.draw.rect(card, kl, outer.inflate(2, 2), 2, border_radius=6)
    pygame.draw.rect(card, metal, outer, 2, border_radius=5)
    # inner racetrack: shifted right, 2px cream channel gap on both long sides
    inner = pygame.Rect(9, 10, 12, 7)
    pygame.draw.rect(card, metal, inner, 2, border_radius=3)
    # inner wire stub/tail extending left — the open-end signature of a paper clip
    pygame.draw.line(card, metal, (9, 13), (5, 13), 2)
    # top highlight on outer wire
    pygame.draw.line(card, hi, (6, 9), (20, 9), 1)
