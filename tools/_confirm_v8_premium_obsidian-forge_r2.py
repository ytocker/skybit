#!/usr/bin/env python3
"""obsidian-forge · confirm_purchase_v8 · premium-v1 · round_2

Brutalist machined luxury — premium by restraint. Round 2 addresses all
art-director notes: visible forge-iron grain (m(1) width, alpha 50, ±12 tonal
swing), a debossed two-line score (groove + warm lip), countersink arc on bolt
dots, bullion-gold keyline on the BUY button, and a cool steel edge highlight
on the LEGENDARY lozenge that gives the forged-steel plate its own voice against
the bright bullion gold Zone A owns.
"""
import os, sys, math
...
def _forge_grain(big, rect, rad):
    """Directional brushed forge-iron: horizontal passes in CARD_T ±12 tone at
    alpha 50, drawn at m(1) width so each line is a solid pixel on the 2× canvas
    and the finish reads as machined metal at 1× rather than sub-pixel noise.
    8-device-px pitch (m(4) at SS=2) keeps the scan lines coarse enough to see."""
    grain = pygame.Surface(rect.size, pygame.SRCALPHA)
    hi = (min(255, CARD_T[0] + 12), min(255, CARD_T[1] + 12), min(255, CARD_T[2] + 12))
    lo = (max(0, CARD_T[0] - 12), max(0, CARD_T[1] - 12), max(0, CARD_T[2] - 12))
    pitch = m(4)  # 8 device px between lines
    for i, y in enumerate(range(m(2), rect.h - m(2), pitch)):
        col = hi if i % 2 == 0 else lo
        pygame.draw.line(grain, (*col, 50), (m(3), y), (rect.w - m(3), y), max(1, m(1)))
...
def _bolt_dot(big, bx, by):
    """Countersunk machined hole: dark sunk disc inside thin warm ring, plus a
    1-px bright warm arc on the lower-right rim — the chamfered countersink edge
    catching directional light. Without this arc the bolts read as dimples; with
    it they read as a fastener hole cut by a lathe."""
    ...
    # countersink directional highlight: lower-right rim arc in pygame angle space.
    # pygame.draw.arc: angle=0→right, angle=π→left, angle=3π/2→bottom (y-down).
    # Lower-right quadrant spans 3π/2 (bottom) → 2π (right) counterclockwise in
    # pygame coords, which appears as the bottom-to-right arc on screen.
    ...
    big.blit(arc_s, (bx - m(3), by - m(3)))


# ── dead-zone score line ───────────────────────────────────────────────────────
def dead_zone_score(big):
    """Debossed score in the ~76 px void between chip and shelf. Two 1-px lines
    1 device pixel apart: the upper dark groove (the recessed channel) and the
    lower warm lip (the machined light-catch on its far edge). Together they read
    as a deliberate brutalist beat, not an oversight — engraved depth at a glance."""
    band_cy = (259 + 335) // 2
    line = pygame.Surface((POP_W * SS, POP_H * SS), pygame.SRCALPHA)
    # groove — dark recessed channel
    pygame.draw.line(line, (20, 22, 48, 65), (m(30), m(band_cy)),
                     (m(230), m(band_cy)), max(1, m(1)))
    # lip — warm machined light-catch on the lower edge of the groove
    pygame.draw.line(line, (120, 100, 60, 80), (m(30), m(band_cy) + 1),
                     (m(230), m(band_cy) + 1), max(1, m(1)))
    big.blit(line, (0, 0))
...
def buttons(big):
    ...
    for cx_b, lbl, stops, lab_c, sheen, is_cancel in [
        ...
        if is_cancel:
            # CANCEL recedes: standard cool navy bevel, no hierarchy competition
            bevel_rim(big, r, br, CARD_RING_DEEP, (*CARD_RING_BRIGHT, 230), w=max(1, rim_w))
        else:
            # BUY is primary: bullion-gold keyline closes the gold through-line
            # from Zone A chip to CTA button — more restrained than a fill change,
            # unmistakably the dominant action.
            bevel_rim(big, r, br, GOLD_A_RIM_DARK, (*GOLD_A_RIM_BRIGHT, 220), w=max(1, rim_w))
...
def zone_b(big, tier_word, pal):
    ...
    if tier_word == "LEGENDARY":
        # Forged steel top-bevel: a 1-px cool bright line on the upper flat edge
        # of the lozenge so the dark gunmetal banner reads as a machined steel
        # plate — giving the steel family its own voice in the gold-vs-gunmetal
        # dialogue rather than sitting as flat grey next to the bright chip.
        _lozenge_steel_bevel(big, tier_word, m(CX), m(Y_BANNER), m(146))


def _lozenge_steel_bevel(big, tier_word, cx, cy, max_w):
    """Replicate _ribbon_lozenge's geometry to place a 1-px cool-steel highlight
    exactly on the upper flat edge of the lozenge polygon. The lozenge height is
    m(12) and the pointed ends consume pt = h//2 from each side, so the flat top
    spans from (x0+pt, y0) to (x0+w-pt, y0) in device pixels."""
    f = font(8.5)
    ...
    x0 = cx - w // 2
    y0 = cy - h // 2
    # draw 1 px inside the lozenge outline so the steel sheen sits on the face
    hl = pygame.Surface((big.get_width(), big.get_height()), pygame.SRCALPHA)
    pygame.draw.line(hl, (160, 180, 210, 180),
                     (x0 + pt, y0 + 1), (x0 + w - pt, y0 + 1), 1)
    big.blit(hl, (0, 0))
...
idr.text((MARGIN, 18), "obsidian-forge  ·  confirm_purchase_v8  ·  premium-v1  ·  round_2",
         fill=(232, 226, 208))
...
OUT = os.path.join(OUTDIR, "round_2.png")
out.save(OUT)
