...
def _name_backlit_panel(big, name, cx, cy, max_w):
    # An opaque lit lightbox: the panel width tracks the caps so the bright
    # core sits directly behind the letters; shrink the type before the box
    # so the glass silhouette stays inside the plinth.
    sz = 13.5
    f = font(sz)
    ...
    ph = m(14)
    rad = m(3)
    # Opaque warm fill — warm-white settling to champagne toward the base.
    panel = vgrad_stops(pw, ph, rad, [(0.0, (255, 255, 252)), (1.0, (230, 220, 188))], 255)
    # Internal lamp: brightest exactly behind the caps.
    soft_glow(panel, pw // 2, ph // 2, m(10), (255, 255, 248), 220, layers=10)
    # Trim the additive glow back to the rounded silhouette.
    ...
    # Glass edge gives the chip its cut without any transparency.
    bevel_rim(big, prect, rad, (160, 152, 132), (255, 255, 248), w=max(1, m(1.5)))
    # Near-black keyline lifts the warm-white caps off the bright core.
    plain_text(big, name, f, (cx, cy), (255, 255, 252), shadow_a=0,
...
htxt = hfont.render("store_card_v4_r4_name_v3 — backlit-panel — round 1", True, (236,232,214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v3/backlit-panel/round_1.png"
