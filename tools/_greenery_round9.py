"""Promenade PLANTS & GREENERY — round 9 candidate-sheet generator.

GREENERY POOL EXPANSION, batch 3 of 4 — FIVE fresh COMBOS of the EXISTING
shipped species (NO new species drawers, NO new vessels), pool indices 20-24.
The shipped 20 are UNCHANGED. This is a REVISION of round 8: the art-director
passed P21/P22/P23/P24 (kept EXACTLY here — no palette/size drift) and blocked
ONLY P25.

WHY P25 changed (the blocking fix): the round-8 P25 was a stone-trough KUMQUAT
carrying a cooler tangerine fruit — but at far size the hue delta against the
shipped kumquat G9 (terracotta, warm orange) didn't survive; it read as "the
same kumquat in a different pot," the exact failure this batch must avoid. The
fix had to change the SILHOUETTE/DENSITY, not just hue.

The preferred rescue (a sparser/taller citrus STANDARD) is IMPOSSIBLE as a pure
DATA combo: `_sp_kumquat` exposes NO knobs for it — it reads only `accent`
(fruit), `trunk` and the foliage banks; its canopy (5 fixed domes), fruit count
(9 fixed dots) and trunk length (th=8) are all hardcoded, with no mass /
fruit-count / canopy-size / canopy-lift attr. `vlift` only lifts the VESSEL, not
the canopy. So lifting the canopy / thinning the fruit / shrinking the mass can
NOT be done without authoring a new drawer — which the constraints forbid.

Per the critique's fallback, P25 is therefore SWAPPED to a combo that GUARANTEES
a distinct far-size silhouette and is clearly not a re-potted G9:

  P25 → bamboo-planter VINE DRAPE.  A trailing vine/fern cascading from the tall
  GREEN BAMBOO planter. Its silhouette is a drooping CURTAIN of hanging strands —
  the polar opposite of G9's round fruiting BUSH, so the two never read alike at
  the smallest far band (different posture AND fullness, not just colour). It is
  also distinct from the shipped trough-vine G8 (a vine in the COOL STONE trough,
  shorter/narrower bank): the tall green cane planter raises the spill point and
  changes the vessel read, and a fresh deeper drape-foliage bank separates the
  green. The vine carries NO fruit/bloom accent at all, so it strictly LOWERS the
  measured night-cap vs the round-8 fruiting kumquat (it cannot nudge luma up).

THE 5 COMBOS:
  P21 wooden-tub LUSH SHRUB    — big fuller bush (mass 1.2) in the staved wooden
                                 TUB, deep leafy, no bloom. (HELD from round 8.)
  P22 terracotta DWARF PINE    — small potted dwarf conifer cone in a warm
                                 TERRACOTTA pot, cool needles. (HELD.)
  P23 bamboo-planter TOPIARY   — clipped topiary BALLS (tiers=2) in the green
                                 BAMBOO planter, brighter clipped-box green. (HELD.)
  P24 terracotta PEACH AZALEA  — warm PEACH / azalea-pink flowering shrub in
                                 TERRACOTTA, higher day_chroma. (HELD.)
  P25 bamboo-planter VINE DRAPE — trailing vine cascading from the tall green
                                 BAMBOO planter (REVISED — was the trough kumquat).
...
Sheet layout MIRRORS round 8: true far-lane DAY + NIGHT bands with an adult +
...
# a deeper cooler DRAPE green for the bamboo-planter trailing vine — set a step
# darker + bluer than the shipped trough-vine's _FOL_LEAFY so the cascading
# curtain reads as its own ivy-green, not the shipped trough vine in a new pot.
...
POOL = [
    ...
    ("P25 bamboo-planter VINE DRAPE", _row(
        _BAMBOO_V, _FOL_DRAPE,
        vessel="bamboo", species="vine"),
     "vessel:bamboo(shipped green cane planter) species:vine | a trailing vine CASCADING from the tall green BAMBOO planter — a drooping CURTAIN of hanging strands, the opposite posture to G9's round fruiting bush (silhouette differs in fullness AND posture, not just hue). vs the shipped trough-vine G8 (vine in a COOL STONE trough): the taller cane planter raises the spill + a deeper cooler ivy-green drape bank separates it. NO fruit/bloom accent — strictly lowers the night-cap"),
]
...
# P25 is now a VINE drape, so its sibling is the shipped trough-VINE G8 (idx 7) —
# the vine row it must not duplicate (G9 kumquat is no longer the relevant pair).
SIBLING = {0: 0, 1: 1, 2: 2, 3: 4, 4: 7}  # P21->shrub G1, P22->conifer G2, P23->topiary G3, P24->flowering G5, P25->vine G8


# ════════════════════════════════════════════════════════════════════════════
# SHEET RENDERER  (mirrors tools/_greenery_round8.py house style)
# ════════════════════════════════════════════════════════════════════════════
...
def render():
    ...
    y = PAD
    _text(sheet, "SKYBIT PROMENADE — GREENERY POOL EXPANSION (round 9): batch 3 of 4 — FIVE fresh COMBOS of the shipped species (pool indices 20-24); NO new drawers/vessels; the shipped 20 UNCHANGED",
    ...
    _text(sheet, "Pure DATA combos — new vessel x species x palette x size pairings of the SHIPPED 19 species: "
                 ...
                 "P23 BAMBOO-planter TOPIARY (tiers=2, a NEW count) · P24 TERRACOTTA peach AZALEA (warm flowering) · P25 BAMBOO-planter VINE DRAPE (REVISED: was the trough kumquat — swapped to a trailing vine for a distinct far silhouette). "
    ...
    _text(sheet, "C.  ON-STREET COMPOSITE — each NEW combo at true size placed DIRECTLY BESIDE the shipped sibling it must not duplicate (P21|G1 shrub · P22|G2 conifer · P23|G3 topiary · P24|G5 flowering · P25|G8 vine) + human cast + a stall + the coin.  (DAY then NIGHT)",
          PAD, y, 12, (240, 220, 150), bold=True)
    y += 18
    for is_night in (False, True):
        night = 0.95 if is_night else 0.0
        ...
        draw_greenery(strip, 670, base, POOL[4][1], night, 1.0)    # P25 bamboo vine drape
        _shipped_ref(strip, 714, base, night, SIBLING[4], 0.9)     # ref G8 vine (stone trough)
    ...
    out = "/home/user/skybit/docs/sidewalk_overhaul/greenery/round_9.png"
