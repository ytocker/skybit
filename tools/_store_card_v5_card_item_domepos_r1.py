"""v5_card_item_domepos r1 — dome + item moved down together.

All previous decisions locked:
  box_px=84  dome r=56  item +5 log px up inside dome
  lozenge ribbon  ribbon -8 dev px up

Shifts the dome centre (cy) downward; the item follows since it is
always positioned relative to cy.

Panels:
  #0  ORIGINAL   cy = m(CY_DISC)          — no shift
  #1  +4 dev     2 logical px down
  #2  +8 dev     4 logical px down
  #3  +12 dev    6 logical px down
  #4  +16 dev    8 logical px down
  #5  +20 dev    10 logical px down

Output: docs/store_card_v5_card_item/domepos_r1.png
"""
import os, sys
...
def draw_card(surf, sid, rect, dome_y_shift):
    """Full card with dome+item shifted down by dome_y_shift device px."""
    ...
    cx = rect.centerx
    cy = rect.y + sc.m(sc.CY_DISC) + dome_y_shift   # whole band shifted
    ...
    _ribbon_lozenge(surf, tier_word, cx, rect.y + sc.m(55) - sc._RIBN_DY,
                    rect.w - sc.m(34), pal)
...
def render_panel(dome_y_shift):
    ...
    draw_card(big, SID, rect, dome_y_shift)
...
PANELS = [
    (0,  "ORIGINAL",   "no shift"),
    (4,  "#1  +4 dev", "2 log px down"),
    (8,  "#2  +8 dev", "4 log px down"),
    (12, "#3  +12 dev","6 log px down"),
    (16, "#4  +16 dev","8 log px down"),
    (20, "#5  +20 dev","10 log px down"),
]
...
hf   = hud_font(18, True)
htxt = hf.render(
    f"v5 domepos r1  —  {SID}  —  dome + item down together  (ribbon fixed)",
    True, (210, 206, 224))
...
for col, (shift, title, subtitle) in enumerate(PANELS):
    x = MARGIN + col * (PANEL_W + GAP)
    canvas.blit(render_panel(shift), (x, panel_y))
...
out = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_card_v5_card_item", "domepos_r1.png")
