"""v5 price-chip concept: GEM-RING (Lever P) — round 2.

Round 1 landed the value-stepped gem bevel ring (the strongest read), but the
aura leaned a flat blue-purple for BOTH tiers, persisted when unaffordable, and
the locked numerals sat too close to the affordable ones. Round 2 keeps the
ring untouched and addresses the art-director notes: the tier aura is now
gem-tinted per rarity (EPIC violet / LEGENDARY warm gold) and gated to the
affordable state only, the body loses its excess blue, the dark channel is
guaranteed a visible dip at SS scale, and the locked numerals drop to a clearly
dimmer neutral.
...
Output: docs/store_card_v5_price_chip_r2/gem-ring/round_2.png
"""
import os, sys
...
# fill is a near-flat cool slate that never competes with the gem ring. R2 pulls
# the blue channel down so the body reads slate, not navy.
...
# Locked numerals: a clear value drop (luma ~145) so "can't afford" reads at a
# glance rather than sitting a hair under the affordable near-white.
NUM_LOCKED = (140, 145, 158)

# Per-tier aura tint — a tight rarity bloom (only a few px past the rim) in the
# gem hue, not the flat blue-purple the shared glow produced in r1.
AURA_TINT = {"epic": (100, 60, 180), "legendary": (180, 130, 40)}
...
def gem_ring_chip(surf, cx, cy, text, h, sid, affordable=True):
    ...
    gem outer ring -> coin -> near-white numerals. Can't-afford drops the aura
    entirely and cools the ring + digits to a dim neutral, keeping the coin
    legible as currency under a cool tint."""
    ...
    if affordable:
        ...
        deep = pal["deep"]
        # tight, gem-tinted tier bloom BEFORE the body lands. Only a few px peek
        # past the rim — enough to read as a rarity halo, not a soft cloud. Gated
        # to the affordable state so "can't afford" reads as unlit.
        tint = AURA_TINT.get(rarity, pal["glow"])
        sc._alpha_aura(surf, cx, cy, int(h * 0.75), tint, peak=42, layers=12)
    else:
        # cool-dim neutral ring: the pill still reads as a defined object, the
        # rarity signal simply goes quiet (and the aura is gone entirely) when
        # unaffordable.
    ...
    # A 2px-min stroke at SS scale guarantees the dip is visible at gameplay size.
    pygame.draw.rect(surf, (10, 12, 20), r.inflate(sc.m(2), sc.m(2)),
                     width=max(2, sc.m(1)), border_radius=rad + 1)
    ...
    if not affordable:
        ctint = pygame.Surface((coin_d, coin_d), pygame.SRCALPHA)
        pygame.draw.circle(ctint, (60, 70, 90, 160),
                           (coin_d // 2, coin_d // 2), coin_d // 2)
        surf.blit(ctint, (x, cy - coin_d // 2))

    x += coin_d + gapc
    center = (x + nw // 2, cy)
    col = NUM_COL if affordable else NUM_LOCKED
...
hf = hud_font(18, True)
htxt = hf.render("v5 gem-ring r2 — tinted tier aura, quiet slate body",
                 True, (210, 206, 224))
...
ft = sub_font.render(
    "quiet slate body · value-stepped gem bevel ring (EPIC +35% / LEG +28% toward white) · 2px dark channel gap · gem-tinted tier aura (buy only) · near-white rarity-agnostic numerals",
    True, (120, 118, 140))
canvas.blit(ft, ((canvas_w - ft.get_width()) // 2, canvas_h - MARGIN - ft.get_height()))

out = os.path.join(os.path.dirname(__file__), "..",
                   "docs", "store_card_v5_price_chip_r2", "gem-ring", "round_2.png")
