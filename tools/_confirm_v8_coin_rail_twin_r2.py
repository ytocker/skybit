#!/usr/bin/env python3
"""
coin-rail-twin  ·  confirm_purchase_v8  ·  round 2

Fixes from art-director critique:
- Rail tightened to content: RAIL_W 164→104
- Rail gradient lifted to lum ~45 + faint tier glow tint (was lum 16 — too dark)
- CANCEL pip removed (only BUY pip remains, saturated tier gem)
- DISC_CY 135→133 (raised 2px to clear y=188 disc bottom)
"""
import os, sys
...
DISC_CY, DISC_R = 133, 53    # DISC_CY raised 135→133 to clear y=188 bottom
GEM_L_X, GEM_R_X, GEM_CY, GEM_R = 43, 217, 152, 14

# bottom zone (logical px)
RAIL_CY, RAIL_W, RAIL_H = 276, 104, 26   # RAIL_W tightened 164→104
BTN_CY,  BTN_W,  BTN_H  = 368, 104, 46
BTN_GAP  = 14
...
def rail_cartouche(big, price_str, pal):
    """Slim pill cartouche holding coin+price — lifted lum, tier glow tint."""
    cx  = m(CX)
    cy  = m(RAIL_CY)
    w   = m(RAIL_W)
    h   = m(RAIL_H)
    ...
    # faint tier aura (kept compact with tightened rail)
    sc._alpha_aura(big, cx, cy, m(RAIL_W // 2 + 12), pal["glow"], peak=32, layers=8)

    sc.drop_shadow(big, rect, rad, blur=m(4), alpha=100, dy=m(2))

    # gradient lifted to lum ~45 with 15% tier glow tint
    rail_top = lerp_color((44, 42, 62), pal["glow"], 0.15)
    rail_bot = lerp_color((28, 24, 44), pal["glow"], 0.08)
    big.blit(vgrad_stops(w, h, rad, [(0.0, rail_top), (1.0, rail_bot)], 255, gamma=1.0),
             rect.topleft)

    sc.top_sheen(big, rect, rad, m(10), peak=26)
...
def gem_plate(big, cx_log, label, pal, is_buy=True):
    """Polished gem-tinted button plate. Only BUY gets the accent pip."""
    cx  = m(cx_log)
    cy  = m(BTN_CY)
    w   = m(BTN_W)
    h   = m(BTN_H)
    ...
    if is_buy:
        gem_c  = pal["gem"]                               # full saturated tier gem
    ...
    # gem pip only on BUY, using full saturated tier colour
    if is_buy:
        sc.facet_gem(big, cx, cy - m(BTN_H // 2) - m(4), m(5), pal["gem"], pal["deep"])
...
hdr  = _font(17, True).render("confirm_purchase_v8  ·  coin-rail-twin  ·  round 2", True, (232, 226, 208))
sub1 = _font(10, True).render("raised-lum rail cartouche · BUY gem pip only · disc raised 2px", True, (140, 148, 168))
...
OUT = "/home/user/skybit/docs/confirm_purchase_v8/coin-rail-twin/round_2.png"
