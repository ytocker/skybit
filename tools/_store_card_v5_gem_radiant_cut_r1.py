"""Round-1 render sheet for the radiant-cut gem badge concept.
...
def my_facet_gem(surf, cx, cy, r, base, deep, mystery=False):
    ...
    flank alive. A horizontally-elongated specular echoes the landscape form."""
    if mystery:
    ...
    # ── girdle: a landscape clipped-corner octagon (wider than tall) ──
    hw = r
    hh = int(r * 0.75)
    c = int(r * 0.32)
    girdle = [
    ...
    # ── specular: a horizontally-elongated lens (rx > ry) echoing the landscape
    # format, plus a distinct hot pip — both additive so they read as glass ──
    lx, ly = cx - int(r * 0.30), cy - int(r * 0.24)
    lrx = min(int(r * 0.18), int(r * (0.18 + 0.05 * gf)))
    lrx = max(1, lrx)
    lry = max(1, int(r * (0.10 + 0.02 * gf)))
    ...
    px, py = cx - int(r * 0.40), cy - int(r * 0.32)
...
def main():
    ...
    header = hf.render("gem badge — radiant-cut r1", True, (236, 232, 250))
