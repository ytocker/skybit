"""Round-1 render for the `aurora-lip` price-chip concept (Levers P + C).
...
cream NOT baked into the gradient but seeping in as a separate light element. A
soft cream bloom crests the top edge so the pill reads as lit from behind/above,
an "aurora lip" of warm light over the cool spectrum. Numerals go near-white,
front-lit by that bloom. Can't-afford collapses the whole thing to flat slate
with the bloom removed, so wealth reads as colour + light, poverty as their
absence.
"""
import os, sys
...
def price_chip_aurora_lip(surf, cx, cy, text, h, affordable=True):
    ...
    if affordable:
        # EXACTLY two stops — the cream lives only in the bloom, never here
        stops = [(0.0, (150, 70, 230)), (1.0, (60, 190, 235))]
        sc.chip_body_stops(surf, r, rad, stops, (16, 22, 44),
                           (200, 225, 255), gloss=44, gamma=1.02)
        # cream light cresting the crown from above — sits ON the spectral body
        # so it reads as spill-over onto the pill, not an underglow behind it
        sc._alpha_aura(surf, cx, r.top + sc.m(4), sc.m(14), (250, 246, 232),
                       peak=30, layers=12)
        ...
        # flat desaturated slate, no bloom — colour + light simply gone
        stops = [(0.0, (70, 78, 100)), (1.0, (44, 50, 70))]
        sc.chip_body_stops(surf, r, rad, stops, (16, 22, 44),
                           (150, 162, 188), gloss=44, gamma=1.02)
        num_col = (150, 158, 178)
        key_col = (20, 24, 44)
        coin_rim = (78, 84, 104)
...
def main():
    ...
    # a card-body gradient patch behind the 4x strip, so the chip is judged on
    ...
    text(canvas, f"v5 price-chip r2 — aurora-lip r1 — {SID} EPIC — P+C",
    ...
    text(canvas,
         "spectral violet->cyan body  ·  separate cream aurora-lip bloom  ·  "
         "near-white front-lit numerals  ·  locked = flat slate, no bloom",
         label_sm, (W // 2, H - footer // 2), (150, 156, 178))

    out = os.path.join(os.path.dirname(__file__), "..",
                       "docs", "store_card_v5_price_chip_r2",
                       "aurora-lip", "round_1.png")
