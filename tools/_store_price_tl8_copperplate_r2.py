"""Round-2 render for tl8 numeral concept: copperplate.

Round 1's thick/thin WIDTH swell was unrenderable at 7px numeral height — a
~1.5px width delta vanishes under 2x->1x smoothscale. This pass pivots to a
VALUE swell, which is the only form of pen pressure that survives 5-8px: a fat
DARK pass lays the stroke shoulders in iron-gall, then a plain (un-bolded)
LIGHTER mid-tone overlay lights the stroke core on the same rect centre. The
dark-edge -> light-centre tonal gradient IS the copperplate pressure modulation
at this scale, and the bolded fat pass leaves dark shoulder pixels proud of the
plain overlay so charcoal-on-cream contrast reads after downscale. Amount +
"G" ledger token both use this two-pass value build. Review sheet only.
"""
import os
...
def _price_glyph_fat(text):
    for fs in (13, 12, 11, 10, 9, 8):
        mask = sc._stamp_bold(sc._glyph_base(text, sc.font(fs), 0), sc.m(1.5))
...
def _draw_price(face, text, center, affordable):
    ...
    dip-pen price mid-face, thin underline ruled beneath the amount. Each glyph
    is a two-pass VALUE swell — a fat DARK shoulder pass with a plain LIGHTER
    core overlay — because a value gradient is the only pen-pressure cue that
    survives smoothscale down to a 7px numeral. The fat pass is bolder than the
    plain overlay, so its dark shoulders stay proud of the lit core."""
    cx = center[0]
    if affordable:
        ink_dark, ink_light = (46, 32, 24), (120, 92, 70)
        coin_dark, coin_light = (120, 88, 24), (180, 140, 60)
        rule_col = (46, 32, 24)
    else:
        ink_dark, ink_light = (70, 68, 80), (140, 138, 155)
        coin_dark, coin_light = (60, 62, 78), (110, 112, 132)
        rule_col = (70, 68, 80)

    def _value_swell(fat_mask, plain_mask, cy, dark, light):
        fr = fat_mask.get_rect(center=(cx, cy))
        fat_fill = fat_mask.copy()
        fat_fill.fill((*dark, 255), special_flags=pygame.BLEND_RGBA_MULT)
        face.blit(fat_fill, fr)
        pr = plain_mask.get_rect(center=(cx, cy))
        plain_fill = plain_mask.copy()
        plain_fill.fill((*light, 255), special_flags=pygame.BLEND_RGBA_MULT)
        face.blit(plain_fill, pr)
        return fr

    # Coin token — small "G", pushed high but under the grommet's clear line.
    # Smaller than the amount, so the fat pass is thinner (sc.m(1.2)) to keep
    # the counter open while still carrying a dark copper shoulder.
    cy_token = int(FACE_H * 0.40)
    fat_coin = sc._stamp_bold(sc._glyph_base("G", sc.font(8), 0), sc.m(1.2))
    plain_coin = sc._glyph_base("G", sc.font(8), 0)
    _value_swell(fat_coin, plain_coin, cy_token, coin_dark, coin_light)

    # Amount line — flat cream field, no keyline. Dark iron-gall shoulders + a
    # lighter mid-tone core give the pen-pressure value gradient; the fat pass
    # holds the 0/8/comma counters open.
    fat_mask, plain_mask = _price_glyph_fat(text)
    cy_amount = int(FACE_H * 0.62)
    _value_swell(fat_mask, plain_mask, cy_amount, ink_dark, ink_light)
    amt_h = fat_mask.get_height()

    # Ledger underline directly beneath the amount, in the dark ink so it reads
    # as a ruled ledger line rather than a bright keyline.
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("COPPERPLATE price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/copperplate/round_2.png"
