...
def _name_shelf_deboss(big, name, cx, cy, max_w):
    ...
    debossed carve reads at 1x: the plate is a warmed-ivory ramp seated as a
    recessed inlay (dark contact keyline all round, a light-catch lip on the
    lower-right), and each glyph gets a bright DOWN-RIGHT lip under a dark fill
    so it reads pressed INTO the plate rather than raised off it."""
    ...
    # Bake the pale shelf plate: a warmed-ivory vertical ramp, alpha 244 so it
    # sits IN the band rather than stickering ON it.
    ph, pw = m(14), glyph_w + m(24)
    plate = vgrad_stops(pw, ph, m(3), [(0.0, (226, 222, 210)), (1.0, (198, 193, 178))], 244, gamma=1.0)
    px, py = cx - pw // 2, cy - ph // 2
    big.blit(plate, (px, py))

    # Seat the inlay: a dark contact keyline severs the plate from the navy so
    # it reads pressed-in, then a warm-bright lip on the BOTTOM+RIGHT is where
    # the light catches the lower edge of a recess.
    plate_rect = pygame.Rect(px, py, pw, ph)
    pygame.draw.rect(big, (14, 15, 26), plate_rect, width=max(1, m(1)), border_radius=m(3))
    lip = max(1, m(1))
    pygame.draw.line(big, (250, 248, 238), (px + lip, py + ph - 1), (px + pw - 1 - lip, py + ph - 1), lip)
    pygame.draw.line(big, (250, 248, 238), (px + pw - 1, py + lip), (px + pw - 1, py + ph - 1 - lip), lip)

    # Carve the letters: first a warm-bright glyph offset DOWN-RIGHT (the light
    # lip of the groove), then the dark fill on top — that stacking order is
    # what makes the type read debossed (carved-in) instead of embossed.
    off = max(1, m(0.8))
    plain_text(big, name, f, (cx + off, cy + off), (250, 248, 238), shadow_a=0, weight=m(0.8), kw=0)
...
htxt = hfont.render("store_card_v4_r4_name_v2 — shelf-deboss — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/shelf-deboss/round_1.png"
