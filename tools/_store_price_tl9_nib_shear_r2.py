"""Round-2 render for tl9 numeral concept: nib_shear.

Three changes over r1:
  1. Shade-axis offset doubled to 2 device px so the thick/thin contrast
     survives the 2x→1x smoothscale without additive merging.
  2. Cream-ghost hairline blits at (+1,+1) and (-1,-1) before the main ink
     deposit. Those offsets are the hairline (NE-SW) axis, so the ghost bleeds
     into the stroke edges perpendicular to the shade direction — a
     value-contrast trick that reads as calligraphic thinning without true
     erosion.
  3. Chiseled terminal cuts at the numeral baseline — small 2×2 device-px
     dark rects spaced across the foot of the digit zone, reading as the ink
     wedge a flat nib deposits when lifting at the end of a downstroke.
"""
import os
...
def _shade_stamp(base):
    """Shade-axis stamp at 2 device px — minimum surviving offset after
    2x→1x downscale. The 1px intermediate blits fill the band so no gap
    appears between center and outer edge."""
    w, h = base.get_size()
    pad = 2
    out = pygame.Surface((w + pad * 2, h + pad * 2), pygame.SRCALPHA)
    # Both the outer 2px and inner 1px shade positions — thick direction only.
    for dx, dy in [(-2, 2), (1, -1), (-1, 1), (2, -2)]:
        out.blit(base, (pad + dx, pad + dy))
    out.blit(base, (pad, pad))  # crisp face on top
    return out
...
def _draw_price(face, text, center, affordable):
    """Nib ledger: shade-axis stamp at 2 device px so the thick stroke
    survives downscale. A cream ghost at the hairline diagonals thins those
    edges by value contrast before the dark ink covers the center. Chiseled
    terminal cuts at the baseline complete the nib-character read. No alpha
    layering — value and stroke-weight contrast alone carry the tag at 5-8px.
    """
    cx = center[0]
    ink = (40, 30, 26) if affordable else (40, 40, 52)
    # Burnt-copper token when affordable; cool grey when locked — two states
    # unmistakable at a glance.
    coin_col = (150, 100, 28) if affordable else (88, 92, 110)
    rule_col_solid = (56, 42, 30) if affordable else (40, 40, 52)

    # Cream ghost color sits between the parchment background and the dark ink
    # so it reads as a lighter-value edge, not a full-opacity halo.
    ghost_col = (215, 200, 165) if affordable else (130, 135, 155)

    # Coin token — lighter weight than the amount so the numeral stays dominant.
    ...
    # Amount mask — shade-axis stamp at 2 device px.
    amt_mask = _price_glyph(text)
    cy_amount = int(FACE_H * 0.62)
    amt_r = amt_mask.get_rect(center=(cx, cy_amount))

    # Hairline-axis ghost: deposit the glyph in cream at the (+1,+1) and
    # (-1,-1) diagonal before the dark ink arrives. Wherever the ghost sticks
    # out beyond the main ink footprint (the NE and SW stroke edges), it
    # leaves a lighter zone that the eye reads as calligraphic thinning.
    ghost_fill = amt_mask.copy()
    ghost_fill.fill((*ghost_col, 255), special_flags=pygame.BLEND_RGBA_MULT)
    for gdx, gdy in [(1, 1), (-1, -1)]:
        face.blit(ghost_fill, (amt_r.left + gdx, amt_r.top + gdy))

    # Main ink on the shade axis — covers the ghost in the stroke interior
    # while leaving the hairline edges in the lighter ghost tone.
    ...
    # Chiseled terminal cuts — 2×2 device-px dark rects at the baseline foot,
    # spaced across the numeral width. They read as the wedge of ink a flat nib
    # deposits when lifting the pen at the end of each downstroke.
    term_y = amt_r.bottom - sc.m(1)
    term_w = max(2, sc.m(1))
    nw = amt_mask.get_width()
    for tx in range(amt_r.left + 2, amt_r.right - 2, max(4, nw // 4)):
        pygame.draw.rect(face, ink, (tx, term_y, term_w, term_w))

    # Brush finishing stroke: wedge tapering to a near-point at the right,
    # reading as the brush lifting and flicking off the page.
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 NIB_SHEAR digit treatment — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9/nib_shear/round_2.png"
