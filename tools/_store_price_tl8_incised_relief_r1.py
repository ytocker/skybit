"""Round-1 render for tl8 numeral concept: incised-relief.

Engraver's V-cut inscription. Inverse of letterpress: the numeral reads as
cut INTO the tag, with a single pale-gold highlight catching only the top-left
bevel edge. A bevel stamp is laid at the (-1, -1) up-left offset first, then the
dark-ink main stamp blits on top, covering the centre and leaving just the glint
edge. One directional highlight, no shadow, all solid fills — the technique is
the single offset, so it survives the 2x->1x downscale. Review sheet only —
not wired live.
"""
import os
...
def _draw_price(face, text, center, affordable):
    """Incised V-cut stack: a pale-gold bevel stamped up-left of each glyph,
    then the dark-ink face blitted on top so only the top-left edge glints, as
    if light rakes across a chiselled groove."""
    cx = center[0]
    # Ink is the dark groove face; bevel is the single lit edge. Locked drops
    # to cool slate so it reads distinct from the affordable warm-cream tag.
    ink = (54, 38, 28) if affordable else (64, 62, 76)
    bevel = (236, 214, 150) if affordable else (208, 212, 226)
    rule_col = (54, 38, 28) if affordable else (64, 62, 76)
    ...
    coin_bevel = coin_mask.copy()
    coin_bevel.fill((*bevel, 255), special_flags=pygame.BLEND_RGBA_MULT)
    coin_ink = coin_mask.copy()
    coin_ink.fill((*ink, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(coin_bevel, (coin_r.x - 1, coin_r.y - 1))
    face.blit(coin_ink, coin_r)

    # Amount line — the incised numeral. Bevel first at the up-left offset, dark
    # ink covering the centre so only the top-left groove edge stays lit.
    amt_mask = _price_glyph(text)
    cy_amount = int(FACE_H * 0.62)
    amt_r = amt_mask.get_rect(center=(cx, cy_amount))
    amt_bevel = amt_mask.copy()
    amt_bevel.fill((*bevel, 255), special_flags=pygame.BLEND_RGBA_MULT)
    amt_ink = amt_mask.copy()
    amt_ink.fill((*ink, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(amt_bevel, (amt_r.x - 1, amt_r.y - 1))
    face.blit(amt_ink, amt_r)
    amt_h = amt_mask.get_height()

    # Ledger underline directly beneath the amount, in dark ink.
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("INCISED-RELIEF price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/incised_relief/round_1.png"
