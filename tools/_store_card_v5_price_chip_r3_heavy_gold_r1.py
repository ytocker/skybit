"""Round-1 render for the `heavy-gold` price-chip concept (Lever T — type size).
...
heavy, jewel-dark slab. Numerals carry a top-lit warm-gold vertical ramp (pale
crown -> deep amber foot) via the glyph-mask pipeline. Can't-afford drops the
gold to flat slate and cools the coin, so wealth reads as gold TYPE and poverty
as its grey absence — the size stays, the value drains.
"""
import os, sys
...
def price_chip_heavy_gold(surf, cx, cy, text, h, affordable=True):
    ...
    # the LARGE font is the whole thesis: ~1.4x the default 0.50 numeral height
    f = sc.font(h * 0.70 / sc.SS)
    ...
    if affordable:
        ...
        # rim_bright is a 3-tuple (chip_body_stops appends alpha=235 itself).
    ...
    if not affordable:
        # coin_glyph() ignores `rim`; drain the locked coin to cool slate with a
        # 55% post-draw tint so it can't read as spendable warm gold.
        cr = coin_d // 2
        tint = pygame.Surface((coin_d, coin_d), pygame.SRCALPHA)
        pygame.draw.circle(tint, (60, 70, 90, 140), (cr, cr), cr)
        surf.blit(tint, (ccx - cr, cy - cr))

    nx = r.x + pad + coin_d + gapc + nw // 2
    if affordable:
        # glyph-mask pipeline: a bold white mask multiplied by a warm-gold
        # vertical ramp (pale crown -> deep amber foot) so the numerals read as
        # struck metal, top-lit, WITHOUT any additive light spilling on the body.
        mask = sc._stamp_bold(sc._glyph_base(text, f, 0), sc.m(1.0))
        grad = sc.vgrad_stops(
            mask.get_width(), mask.get_height(), 0,
            [(0.0, (255, 238, 170)), (0.5, (240, 196, 90)), (1.0, (190, 140, 40))],
            255, 1.0)
        img = mask.copy()
...
def main():
    ...
    text(canvas, f"v5 price-chip r3 — heavy-gold r1 — {SID} EPIC — T(size)",
    ...
    text(canvas,
         "oversized warm-gold numerals (~1.4x)  ·  quiet near-black obsidian body  "
         "·  no glow/halo  ·  locked = flat slate type + cooled coin",
         label_sm, (W // 2, H - footer // 2), (150, 156, 178))

    out = os.path.join(os.path.dirname(__file__), "..",
                       "docs", "store_card_v5_price_chip_r3",
                       "heavy-gold", "round_1.png")
