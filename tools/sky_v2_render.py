"""ALPINE_HAZE fine-tune v2_haze — diffuse haze (sky_bot lighter than sky_mid).

Only sky_bot and horizon differ from live ALPINE_HAZE at phases 0.27–0.52.

    python tools/sky_v2_render.py

Output: docs/sky_transition/v2_haze/round_1.png
"""
import os
import sys
...
_OVERRIDES = {
    0.27: dict(sky_bot=(245, 193, 162), horizon=(246, 215, 192)),
    0.31: dict(sky_bot=(249, 172, 149), horizon=(245, 200, 181)),
    0.37: dict(sky_bot=(252, 146, 136), horizon=(245, 182, 172)),
    0.42: dict(sky_bot=(255, 122, 149), horizon=(249, 164, 176)),
    0.47: dict(sky_bot=(247, 124, 156), horizon=(243, 151, 172)),
    0.52: dict(sky_bot=(140, 78, 101), horizon=(171, 99, 123)),
}
...
SPEC = BiomeSpec(
    name='alpine_haze_v2_haze',
    note='v2 haze - sky_bot lighter than sky_mid, milky scattered light',
    keyframes=_KF,
    sky=ALPINE_HAZE.sky,
)
...
def main():
    ...
    title = f_title.render("v2_haze — diffuse haze (sky_bot lighter than sky_mid)",
                           True, TEXT_HI)
    ...
    out = os.path.join(_ROOT, "docs", "sky_transition", "v2_haze", "round_1.png")
