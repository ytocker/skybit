"""Round-1 render for the `ornament-rule` tl5 store-card price tag.

Combined base (before + tl4 brass-classic + dark-boutique pin). Concept:
the mahogany ruling line is replaced by a procedural bronze diamond ornament
divider; the zone below picks up a subtle cool overlay; price in champagne
with keyline; gold grommet + specular pip; hemp cord.
"""
import os
...
def _ornament_divider(face, y, col):
    """Horizontal hairlines flanking a centered diamond — sovereign-bronze."""
    diamond_r = sc.m(2)
    cx = FACE_W // 2
    # left line
    pygame.draw.line(face, col, (6, y), (cx - diamond_r - 3, y), max(1, sc.m(0.8)))
    # right line
    pygame.draw.line(face, col, (cx + diamond_r + 3, y), (FACE_W - 6, y), max(1, sc.m(0.8)))
    # diamond
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    if affordable:
        ...
        # ornament divider replacing ruling line
        _ornament_divider(face, 38, (186, 142, 48))

        # subtle cool overlay on the price zone below the ornament
        cool = pygame.Surface((FACE_W - 8, FACE_H - 40), pygame.SRCALPHA)
        cool.fill((168, 188, 218, 18))
        face.blit(cool, (4, 40))

        # grommet + specular pip
        ...
        pygame.draw.circle(face, (255, 248, 200),
                           (gx_local - sc.m(3), gy_local - sc.m(3)),
                           max(1, sc.m(0.6)))

        # price: champagne, keyline mahogany
        _draw_price(face, text, (FACE_W // 2, 58), (80, 48, 12),
                    col_fill_flat=(240, 222, 168), fs=14)
    ...
    _cord(surf, (gx, gy), knot, cord_a, cord_b, sc.m(3), sc.m(1.5))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("ORNAMENT-RULE — tl5 hang-tag round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
out = "docs/store_price_tl5/ornament_rule/round_1.png"
