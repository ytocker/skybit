"""Round-1 render for the `kiss-impression` tl6 numeral treatment.

Letterpress kiss-impression: warm charcoal-brown flat ink (56,40,28) fills the
numeral — no gradient, no keyline ring. A single hairline rule below the number
is the only decorative element. All tag structure identical to tl2 r2.
"""
import os
...
def _draw_price(face, text, center, affordable):
    ink = (56, 40, 28) if affordable else (98, 102, 114)
    ...
    rule_h = max(2, sc.m(1))
    pygame.draw.rect(face, (*ink, 180), (r.left, rule_y, r.width, rule_h))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("KISS-IMPRESSION — tl6 hang-tag round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/kiss_impression/round_1.png"
