"""Round-2 render for tl8 numeral concept: incised-relief.

Engraver's V-cut inscription, now carved with BOTH walls. Round 1's single
(-1,-1) glint was sub-pixel at final 1x and vanished in smoothscale. This round
lays a full two-tone groove: a deep shadow wall down-right (+2,+2), a lit bevel
up-left (-2,-2), and the ink face between them. At SS=2 the ±2 offsets resolve
to a solid pixel at final 1x, so the carved-in read survives the downscale.
Ink is deepened for chroma survival and the ledger rule thickened to 2px native.
...
def _price_glyph(text):
    for fs in (13, 12, 11, 10, 9, 8):
        mask_face = sc._stamp_bold(sc._glyph_base(text, sc.font(fs), 0), sc.m(1.0))
        if mask_face.get_width() <= 66:
            mask_rim = sc._stamp_bold(sc._glyph_base(text, sc.font(fs), 0), sc.m(1.2))
            return mask_face, mask_rim
    mask_rim = sc._stamp_bold(sc._glyph_base(text, sc.font(fs), 0), sc.m(1.2))
    return mask_face, mask_rim


def _incise(face, mask_face, mask_rim, rect, ink, bevel, shadow):
    """Stack a numeral as a chiselled V-groove: darkest shadow wall down-right,
    lit bevel up-left, ink face on top. The rim mask is stamped slightly bolder
    than the face so the lit/shadow walls peek out on every side; the ±2 offsets
    (a solid pixel at final 1x) bias them into a directional cut."""
    shadow_stamp = mask_rim.copy()
    shadow_stamp.fill((*shadow, 255), special_flags=pygame.BLEND_RGBA_MULT)
    bevel_stamp = mask_rim.copy()
    bevel_stamp.fill((*bevel, 255), special_flags=pygame.BLEND_RGBA_MULT)
    ink_stamp = mask_face.copy()
    ink_stamp.fill((*ink, 255), special_flags=pygame.BLEND_RGBA_MULT)
    face.blit(shadow_stamp, (rect.x + 2, rect.y + 2))
    face.blit(bevel_stamp, (rect.x - 2, rect.y - 2))
    face.blit(ink_stamp, rect)


def _draw_price(face, text, center, affordable):
    """Incised two-tone V-cut stack for the token + amount, plus the ledger
    rule. Locked drops to cool slate so it reads distinct from the affordable
    warm-cream tag."""
    cx = center[0]
    ink = (44, 28, 18) if affordable else (64, 62, 76)
    bevel = (236, 214, 150) if affordable else (208, 212, 226)
    shadow = (28, 16, 10) if affordable else (38, 36, 50)
    rule_col = ink

    # Coin token — small "G", pushed high but clear of the grommet's radius.
    cy_token = int(FACE_H * 0.40)
    coin_face = sc._stamp_bold(sc._glyph_base("G", sc.font(8), 0), sc.m(1.0))
    coin_rim = sc._stamp_bold(sc._glyph_base("G", sc.font(8), 0), sc.m(1.2))
    coin_r = coin_face.get_rect(center=(cx, cy_token))
    _incise(face, coin_face, coin_rim, coin_r, ink, bevel, shadow)

    # Amount line — the incised numeral.
    amt_face, amt_rim = _price_glyph(text)
    cy_amount = int(FACE_H * 0.62)
    amt_r = amt_face.get_rect(center=(cx, cy_amount))
    _incise(face, amt_face, amt_rim, amt_r, ink, bevel, shadow)
    amt_h = amt_face.get_height()

    # Ledger underline directly beneath the amount, thickened to 2px native.
    rule_y = cy_amount + amt_h // 2 + sc.m(2)
    rule_w = min(amt_face.get_width() + sc.m(4), FACE_W - sc.m(8))
    rule_h = max(2, sc.m(2))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("INCISED-RELIEF price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl8/incised_relief/round_2.png"
