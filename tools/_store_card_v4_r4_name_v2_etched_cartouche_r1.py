...
def _name_etched_cartouche(big, name, cx, cy, max_w, rect):
    ...
    while size > 9.0 and _glyph_base(name, f, tracking).get_width() > max_w:
        ...
        f = font(size)
    cap_h = f.get_height()
    plain_text(big, name, f, (cx, cy), cream, shadow_a=150, tracking=tracking,
               weight=m(0.8), keyline=key, kw=m(0.9))
    # Twin gold rules + end-ticks bracket the caps. Authored at full alpha and
    # 1 device px so the supersample downscale is what softens them to a
    # hairline — never pre-dimmed, so they read crisp and intentional.
    ...
    for ry in (top_y, bot_y):
        pygame.draw.line(big, (*gold, 255), (lx, ry), (rx, ry), 1)
        for tx in (lx, rx):
            pygame.draw.line(big, (*gold, 255), (tx, ry - th // 2), (tx, ry + th // 2), 1)
...
htxt = hfont.render("store_card_v4_r4_name_v2 — etched-cartouche — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v2/etched-cartouche/round_1.png"
