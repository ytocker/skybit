#!/usr/bin/env python3
"""
enamel-split-bar  ·  confirm_purchase_v8  ·  round 1

Bottom zone: a small dark-enamel cartouche (coin+price) floats above a
single pill-shaped enamel bar that is split at the centre seam into BUY
(warm gold left half) and CANCEL (matte slate right half). One bevel_rim
wraps the entire pill — no per-half outlines.
"""
import os, sys
...
CART_CY, CART_W, CART_H = 292, 142, 30    # price cartouche, y=277→307
PILL_CY, PILL_W, PILL_H = 368, 216, 58    # split pill,      y=339→397
PILL_X   = CX - PILL_W // 2              # = 130 - 108 = 22
...
def price_cartouche(big, price_str, pal):
    ...
    # verify "2,600" fits with coin: measure at text_size=11
    f_test = font(11)
    tw = sc._glyph_base(price_str, f_test, 0).get_width()
    coin_d = m(10) * 2
    group_needed = coin_d + m(5) + tw + m(14)
    print(f"  cartouche check: group={group_needed}px, cart_w={w}px, fits={group_needed<=w}")
...
def split_pill(big, pal):
    """Single enamel pill split at the seam: BUY (warm left) / CANCEL (slate right)."""
    pill_x = m(PILL_X)
    ...
    rad    = m(PILL_H // 2)
    seam_x = m(SEAM_X)
    half_w = seam_x - pill_x   # width of each half in SS px

    pill_rect = pygame.Rect(pill_x, pill_y, pw, ph)
    sc.drop_shadow(big, pill_rect, rad, blur=m(5), alpha=130, dy=m(4))

    # BUY half: warm gold enamel fill (clip to left half)
    buy_fill = pygame.Surface((pw, ph), pygame.SRCALPHA)
    buy_stops = [(0.0, (148, 104, 28)), (0.5, (104, 70, 14)), (1.0, (62, 40, 8))]
    buy_body  = vgrad_stops(pw, ph, rad, buy_stops, 255, gamma=1.1)
    # clip right side: overwrite right half with transparent
    clip = pygame.Surface((pw - half_w, ph), pygame.SRCALPHA)
    buy_body.blit(clip, (half_w, 0))
    big.blit(buy_body, pill_rect.topleft)

    # CANCEL half: matte slate fill (clip to right half)
    can_surf = pygame.Surface((pw, ph), pygame.SRCALPHA)
    can_stops = [(0.0, (36, 32, 52)), (1.0, (22, 20, 34))]
    can_body  = vgrad_stops(pw, ph, rad, can_stops, 255, gamma=1.0)
    # clip left side: paint only the right half portion
    clip_l = pygame.Surface((half_w, ph), pygame.SRCALPHA)
    can_body.blit(clip_l, (0, 0))
    big.blit(can_body, pill_rect.topleft)

    # asymmetric sheen: brighter on BUY (left), dimmer on CANCEL (right)
...
hdr  = _font(17, True).render("confirm_purchase_v8  ·  enamel-split-bar  ·  round 1", True, (232, 226, 208))
sub1 = _font(10, True).render("dark enamel cartouche (coin+price) · single pill / gold seam / BUY-warm CANCEL-slate", True, (140, 148, 168))
...
OUT = "/home/user/skybit/docs/confirm_purchase_v8/enamel-split-bar/round_1.png"
