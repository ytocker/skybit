...
# Escalates the hot-core rim from a 1px hairline to a 2-3px contour-traced bevel:
# a modest champagne atmospheric halo sits underneath, then three tight rings hug
# the glyph silhouette falling in temperature from the stroke edge outward. The
# outermost ring is a warm-amber shoulder (clearly cooler/darker than the ivory
# body), the mid ring is warm-ivory, the core is white-hot — so the bevel reads
# as a graded amber -> ivory -> white-hot shoulder of struck metal lit from above,
# not a uniform cream slab. The amber outer step is pushed well off the body value
# (blue channel dropped to ~170 vs the body's 225) so the temperature ramp survives
# at 1x gameplay scale rather than going near-metameric. Normal-alpha (not additive):
# compositing displaces the navy's blue toward the warm hue so the fixed palette can
# read both warm and bright at once.
def _name_rim_forge(big, name, cx, cy, max_w):
    sz = 13.5
    f = font(sz)
    ...
    for char, adv in zip(name, advances):
        ...
        # Layer 1: modest atmospheric champagne halo. Tint nudged warmer/lighter
        # (228,216,186 vs 218,208,182) so the thin outer fringe over navy holds a
        # champagne cast instead of collapsing to grey-purple where blue>=red.
        halo = tile.copy()
        halo.fill((228, 216, 186, 60), special_flags=pygame.BLEND_RGBA_MULT)
        steps = max(16, m(6) * 4)
        for i in range(steps):
            angle = 2 * math.pi * i / steps
            ox = int(round(math.cos(angle) * m(6)))
            oy = int(round(math.sin(angle) * m(6)))
            big.blit(halo, (gx + ox, gy + oy))

        # Layer 2: 3-ring contour bevel — a graded amber -> warm-ivory -> white-hot
        # shoulder that traces the glyph silhouette. The outer amber ring is the
        # concept payoff: it is a distinct warm shoulder, not the body color.
        BEVEL = [
            (m(3), (232, 214, 170), 120),   # amber outer shoulder (distinct from ivory body)
            (m(2), (252, 250, 238), 180),   # warm-ivory mid bevel ring
            (m(1), (255, 255, 252), 235),   # white-hot inner bevel (core)
        ]
        for radius, col, alpha in BEVEL:
            tinted = tile.copy()
    ...
    # Crisp ivory body last — stays flat; all relief comes from the bevel rim
...
def render_card(sid):
    ...
    _name_rim_forge(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26))
    return big
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v6 — rim-forge — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v6/rim-forge/round_2.png"
