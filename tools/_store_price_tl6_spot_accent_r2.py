"""Round-2 render for the `spot-accent` tl6 numeral treatment.

Flat deep-espresso (55,38,28) digits — zero gold — with a single terracotta
(178,66,42) accent tab centered under the numeral as the only chromatic accent.
The tab is a centered swatch (~70% glyph width) rather than a full-width rule,
so it reads as a deliberate boutique accent and won't be confused with the
kiss-impression underline. Tab height is sc.m(2) so it survives the 2x->1x
smoothscale after the -7 deg tilt. All tag structure identical to tl2 r2.
"""
import os
...
def _draw_price(face, text, center, affordable):
    ink = (55, 38, 28) if affordable else (98, 102, 114)
    # Saturated terracotta reads as an intentional accent on the cream body;
    # locked blue-grey stays the cool counterpart.
    spot = (178, 66, 42) if affordable else (60, 75, 110)
    ...
    face.blit(fill, r)
    # Centered accent tab — a boutique swatch, not a text underline. Height
    # sc.m(2) (4px in the 2x buffer) survives the downscale as a visible 2px bar.
    tab_w = int(r.width * 0.70)
    tab_x = r.centerx - tab_w // 2
    tab_y = r.bottom + max(2, sc.m(1))
    pygame.draw.rect(face, (*spot, 255), (tab_x, tab_y, tab_w, sc.m(2)))
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("SPOT-ACCENT price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/spot_accent/round_2.png"
