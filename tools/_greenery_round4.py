"""Promenade PLANTS & GREENERY — round 4 candidate-sheet generator.

GREENERY POOL EXPANSION, batch 1 of 4. The shipped pool is 10 designs
...
THE 5 NEW SPECIES (CNY / Chinese-garden flowering + seasonal theme):
  P11 PEONY (mudan, the King of Flowers) — 2-3 big LAYERED double-blooms, lush
      ruffled heads over broad leaves, in a GLAZED URN. Refs: Tang-aesthetic
      mudan — deep cinnabar-rose petals, white petal base, grass-green leaves.
  P12 CHRYSANTHEMUM mound — a dense LOW cushion of many small spoon-petal blooms
      (gold / russet), pincushion habit, in a TERRACOTTA pot.
  P13 PLUM-BLOSSOM branch (winter meihua) — bare DARK zig-zag dragon-branches
      with SPARSE pink-white 5-dot blossoms, elegant + airy, in a SLIM vessel
      (the bamboo planter, read here as a slim cylinder). Branch ink-dark, the
      blossom a muted pink-white accent.
  P14 RED MAPLE (Acer palmatum) — a small palmate-leaf tree with an AUTUMN
      red/orange canopy of clustered shaded leaf masses, in a WOODEN TUB (the
      previously-unused _TUB bank). Canopy itself is the warm accent (capped),
      raised on a slim trunk so it reads as a little tree, not a dome.
  P15 NARCISSUS / paperwhite (shuixian, the Water Fairy) — a CLUMP of strappy
      blades with small WHITE+GOLD cup flowers, in a SHALLOW BOWL/DISH (a new
      low porcelain dish-of-pebbles vessel). The most delicate, lowest read.

Silhouette legibility at TRUE far size is the bar: peony = big ruffled heads on
an urn; chrysanthemum = a low dense dome solidly studded with bloom; plum = bare
zig-zag twigs + sparse dots; maple = warm palmate canopy on a slim trunk in a
tub; narcissus = a low fan of blades + tiny white-gold cups in a flat dish. Each
distinct from the existing 10 AND from each other.

Sheet layout MIRRORS tools/_greenery_round3.py: true far-lane DAY + NIGHT bands
with an adult + gold-coin yardstick, per-design DAY/NIGHT cells (true far size +
4x nearest zoom + vessel/species/attrs note), an on-street composite, and the
_measure_night_cap() audit footer (PASS only when hottest greenery <=150 and the
coin ~230 stays sole-brightest).
"""
...
def _sp_peony(surf, cx, rim_y, v, night, t):
    ...
    # broad low leaf mass — wide flat domes so the blooms sit ABOVE foliage
    for dx, dy, rw, rh in ((-6, -1, 7, 4), (6, -1, 7, 4), (0, -3, 8, 5)):
...
def _sp_chrysanthemum(surf, cx, rim_y, v, night, t):
    ...
    gold blooms catching light up top."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    ...
    spots = ((-7, -2, 0), (-4, -6, 1), (-1, -9, 1), (2, -10, 1), (5, -7, 1),
             (7, -3, 0), (-5, -3, 0), (0, -6, 1), (3, -6, 1), (-2, -4, 0),
             (5, -3, 0), (-7, -5, 0), (1, -12, 1), (-3, -10, 1), (6, -9, 1))
    for bx, byp, gold in spots:
        px, py = cx + bx + int(sway * (byp < -8)), base + byp
        if gold:
            pygame.draw.circle(surf, b_dk, (px, py), 2)
            pygame.draw.circle(surf, b, (px, py), 1)
            pygame.draw.circle(surf, b_lt, (px, py - 1), 0)
        else:
...
def _sp_plum(surf, cx, rim_y, v, night, t):
    """PLUM-BLOSSOM branch (winter meihua) — bare DARK zig-zag dragon-branches
    rising from a slim vessel with SPARSE pink-white 5-dot blossoms clinging at
    the joints + tips. Sparse + elegant: the read is twiggy ANGULAR ink branches
    (almost no foliage), the blossom a few muted 5-petal stars, not a mass. The
    angular bare-twig silhouette + sparseness is what separates it from every
    other (full, leafy) design."""
    P, A = v.palette, v.attrs
    branch = _retint(P.get("trunk", (58, 44, 40)), night)
    br_lt = _hi(branch, 18, night)
    base = rim_y
    sway = int(round(math.sin(t * 1.4) * 1.0))
    # 3 zig-zag dragon-branches: each a poly-line of sharp angular segments
    # rising and forking — twisted/rugged, the meihua dragon-branch read.
    branches = (
        [(-1, 0), (-3, -6), (1, -11), (-2, -17), (sway, -22)],
        [(2, -3), (6, -8), (3, -13), (7, -18)],
        [(0, -5), (-5, -9), (-8, -14)],
    )
    nodes = []
    ...
    bloom = P.get("accent", (232, 196, 206))
    dc = A.get("day_chroma", 176)
    pet = _accent(bloom, night, day_ceil=dc)
    pet_dk = _accent(_shade(bloom, -34), night, day_ceil=dc)
    bud = _accent(P.get("accent2", (206, 120, 138)), night, day_ceil=dc)
    # SPARSE 5-petal blossoms only at select joints/tips — a tiny dark-cored
    # rosette of dots; a few unopened buds as single warm pips.
    for i, (nx, ny) in enumerate(nodes):
        if i % 3 == 0:
            for a in range(5):
                ang = a / 5 * math.tau - 1.2
                ex = nx + int(round(math.cos(ang) * 1.7))
                ey = ny + int(round(math.sin(ang) * 1.7))
                pygame.draw.circle(surf, pet, (ex, ey), 0)
            pygame.draw.circle(surf, pet_dk, (nx, ny), 0)
        elif i % 3 == 1:
...
def _sp_maple(surf, cx, rim_y, v, night, t):
    ...
    fruit dots — an autumn fire-canopy on a little tree, in a wooden tub."""
    P, A = v.palette, v.attrs
...
def _sp_narcissus(surf, cx, rim_y, v, night, t):
    ...
    around a GOLD cup. The lowest, most delicate read: a spray of thin green
    straps + a few tiny white-and-gold cup flowers nodding at the tips. The flat
    pebble dish + blade-fan + white/gold cups distinguish it from every leafy
    dome and from the maple/plum twigs."""
    P, A = v.palette, v.attrs
    dark, mid, top = _foliage(P, night)
    base = rim_y
    ...
    # small white cup-flowers nodding at a few blade tips: a ring of white petal
    # dots around a single gold cup pip.
    for ox, ln in ((-3, 14), (2, 15), (5, 12), (-6, 11)):
        fx = cx + ox + int(sway)
        fy = base - ln
        for a in range(6):
            ang = a / 6 * math.tau
            ex = fx + int(round(math.cos(ang) * 1.6))
            ey = fy + int(round(math.sin(ang) * 1.6))
            pygame.draw.circle(surf, pet, (ex, ey), 0)
        pygame.draw.circle(surf, pet_dk, (fx, fy), 1, 1)
...
def _draw_dish(surf, cx, base_y, v, night):
    ...
    # a glaze rim band + a few grey pebbles bedding the bulbs
    glaze = _retint(P.get("glaze", (72, 104, 168)), night)
...
POOL = [
    ("P11 glazed-urn PEONY (mudan)", _row(
     ...
     "vessel:urn(blue-white belly+neck) species:peony accent:cinnabar-rose double-bloom + pale white petal-base | 2-3 BIG layered ruffled heads (King of Flowers) over broad grass-green leaves — far fewer/bigger heads than the mum"),
    ...
    ("P12 terracotta CHRYSANTHEMUM", _row(
     ...
     "vessel:terracotta species:chrysanthemum accent:gold spoon-bloom + accent2:russet | a DENSE LOW cushion mound SOLIDLY studded with many tiny spoke-rosettes (pincushion habit) — gold up top, russet on the flanks; opposite of the peony's few big heads"),

    ("P13 bamboo-planter PLUM (meihua)", _row(
        _BAMBOO_V, dict(trunk=(58, 44, 40), accent=(234, 198, 208), accent2=(208, 122, 140)),
        vessel="bamboo", species="plum", day_chroma=176),
     "vessel:bamboo(slim lashed-cane cylinder) species:plum trunk:ink-dark branch accent:pink-white 5-dot blossom + accent2:warm bud | bare DRAGON zig-zag twigs, SPARSE blossom at joints/tips — the airy angular winter-plum read, almost no foliage"),

    ("P14 wooden-tub RED MAPLE", _row(
        _TUB, _FOL_MAPLE, dict(trunk=(96, 70, 50)),
        vessel="tub", species="maple", day_chroma=182),
     "vessel:tub(staved wooden barrel, iron hoops — the unused _TUB bank) species:maple | small palmate-leaf tree: slim forking trunk + a broad AUTUMN canopy (maroon shadow / vermilion body / amber lit edge — warm foliage capped via _accent), a little fire-tree, not a dome"),
    ...
    ("P15 dish NARCISSUS (water fairy)", _row(
     ...
     "vessel:dish(NEW low porcelain bowl of pebbles, flattest vessel) species:narcissus accent:white petals + accent2:gold cup | a low FAN of strappy blades topped with tiny white+gold cup flowers (shuixian) — the most delicate, lowest read in the family"),
]
...
def render():
    ...
    y = PAD
    _text(sheet, "SKYBIT PROMENADE — GREENERY POOL EXPANSION (round 4): batch 1 of 4 — FIVE NEW flowering/seasonal species (pool indices 10-14); the shipped 10 UNCHANGED",
          PAD, y, 17, (250, 246, 236), bold=True)
    y += 22
    _text(sheet, "NET-NEW species authored to the EXACT same instructions as the 10: same crisp native far-lane pipeline (pure pygame.draw.*, no smoothscale), same palette banks, same night-cap contract "
                 "(foliage/vessels cool toward (54,64,96) <=150 via _retint; every bloom/seasonal accent <=132 at night via _accent — nothing out-pops the ~230 coin). "
                 "P11 PEONY(mudan, urn) · P12 CHRYSANTHEMUM(cushion, terracotta) · P13 PLUM(meihua zig-zag, bamboo) · P14 RED MAPLE(autumn canopy, wooden TUB) · P15 NARCISSUS(water fairy, new DISH).",
          PAD, y, 9, (188, 186, 200))
    y += title_h - 22
    ...
    out = "/home/user/skybit/docs/sidewalk_overhaul/greenery/round_4.png"
