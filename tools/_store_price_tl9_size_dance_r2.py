"""Round-2 render for tl9 numeral concept: size_dance.

Stroke-weight dance. The digits alternate in mass on a thick/thin/thick cadence,
but the beat is now anchored from the RIGHT: the ones-place digit always lands
heavy, so amounts like "1,100" read with a consistent mass rhythm regardless of
digit count. The heavy/light font delta is widened to 12/8 so the pulse survives
the 2x→1x downscale (and keeps total_w low enough that the fit-cap won't fire and
quietly eat the contrast). The comma is excluded from the parity count so it
never absorbs a beat. Every digit sits on one stable bottom-aligned baseline: the
rhythm is carried by mass, never by position. Review sheet only — not wired live.
"""
import os
...
def _draw_price(face, text, center, affordable):
    ...
    stable baseline, anchored from the right so the ones column always beats
    heavy. Heavy digits get a larger font plus a bold stamp; light digits are a
    smaller un-stamped glyph. The comma sits outside the parity count. The amount
    stays one steady line whose pulse is weight, not offset."""
    cx = center[0]
    ...
    # Wider heavy/light delta so the pulse reads after the 2x→1x downscale.
    chars = list(text)
    base_fs = 11   # logical font size base
    FS_HEAVY, FS_LIGHT = base_fs + 1, base_fs - 3   # 12 and 8 logical px

    # Count total digits first so parity can be anchored from the right: the
    # ones-place digit always lands on the same (heavy) beat.
    n_digits = sum(1 for ch in chars if ch.isdigit())

    char_masks = []
    digit_idx = 0
    for ch in chars:
        if ch.isdigit():
            # Right-anchored parity: even distance from the ones column → heavy.
            if (n_digits - 1 - digit_idx) % 2 == 0:   # even from right → heavy
                fs = FS_HEAVY
                mask = sc._stamp_bold(sc._glyph_base(ch, sc.font(fs), 0), sc.m(1.8))
            else:                                       # odd from right → light
                fs = FS_LIGHT
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 SIZE_DANCE digit treatment — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9/size_dance/round_2.png"
