#!/usr/bin/env python3
"""assay-balance · confirm_purchase_v8 · swap-round-1"""
import os, sys, math
...
def zone_a_chip(big, price_str, pal):
    ...
    # Depth is sold by three stacked cues — a center-lit vertical ramp, a dark
    # inner lip that the rim casts down, and a lit arc hugging the bowl floor.
    PAN_W, PAN_H = 180, 28
    ...
    # Rim highlight: a thin lit arc along the bowl floor reads as reflected metal.
    arc_rect = rect.inflate(-m(7), -m(6))
    pygame.draw.arc(big,(*CARD_RING_BRIGHT,200),arc_rect,
    ...
    sc.coin_glyph(big, coin_cx, cy, m(9))
...
def zone_b_banner(big, tier_word, pal):
    # The balance beam: a lone 1px gold spine dropping from the pan's underside
    # to the plaque — pure structure that ties the two seats into one instrument,
    # with the flanking bottom gems reading as the beam's counterweights.
    beam = pygame.Surface((max(1,m(1)), m(394-261)), pygame.SRCALPHA)
    beam.fill((*CARD_RING_BRIGHT, 140))
    big.blit(beam, (m(CX)-beam.get_width()//2, m(261)))

    # Reserved plaque: a compact seat that names the tier without shouting.
    PLQ_W, PLQ_H = 110, 16
    ...
    plain_text(big, tier_word, font(10), rect.center, (210,205,190),
...
idr=ImageDraw.Draw(strip); idr.text((MARGIN,18),"assay-balance · swap-round-1",fill=(232,226,208))
...
OUT="/home/user/skybit/docs/confirm_purchase_v8/swap-round-1/assay-balance/round_1.png"
