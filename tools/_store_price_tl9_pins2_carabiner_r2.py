"""Round-2 render for tl9 pins2: carabiner (D-shape flat spine + diagonal gate)"""
import os
...
def _attachment(card, affordable):
    col = (170, 130, 42) if affordable else (112, 116, 124)
    kl  = (70, 50, 16) if affordable else (24, 26, 34)
    hi  = (210, 176, 92) if affordable else (168, 172, 180)
    # D-body: right side curved, left side explicitly flattened — w=12 for readable interior
    pygame.draw.rect(card, kl, (7, 5, 14, 20), 2,
                     border_top_right_radius=6, border_bottom_right_radius=6,
                     border_top_left_radius=0, border_bottom_left_radius=0)
    pygame.draw.rect(card, col, (8, 6, 12, 18), 2,
                     border_top_right_radius=5, border_bottom_right_radius=5,
                     border_top_left_radius=0, border_bottom_left_radius=0)
    # flat left spine overdraw (straight vertical, erases rounded corner artifact)
    pygame.draw.line(card, kl, (7, 5), (7, 25), 3)
    pygame.draw.line(card, col, (8, 6), (8, 24), 2)
    # diagonal gate on right opening (hinged look — not parallel to wall)
    pygame.draw.line(card, kl, (20, 9), (17, 20), 4)
    pygame.draw.line(card, col, (20, 9), (17, 20), 2)
    # highlight on flat spine
    pygame.draw.line(card, hi, (8, 7), (8, 23), 1)
