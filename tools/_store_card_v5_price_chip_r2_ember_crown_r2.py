"""Ember-crown price chip — round 2 review render (Levers C + P).
...
surface. Warmth lives ONLY in a halo ring behind the body and never in the
numerals, which stay cream so the "value in cold metal" read holds.

r2 answers the art-director notes on r1:
  - The r1 halo used soft_glow (BLEND_ADD) and bloomed TOP-ONLY while washing
    the interior toward white. r2 swaps to _alpha_aura — an alpha-composite
    corona at radius h*1.2 drawn BEFORE the body, so the warm ring wraps the
    pill silhouette evenly on all four sides and never adds into the interior.
  - Gloss drops 20 -> 8 so the obsidian body stays genuinely dark: the only
    warm signal is the halo RING around the edge plus the lit crown bevel.

  - Body: near-black two-stop fill on the canonical chip finish (double rim,
    one restrained gloss sweep). The rim's bright top-left bevel is the only
    warm gold on the pill itself — an amber-lit crown edge on a dark coin.
  - Halo: a soft alpha-composite corona drawn BEHIND the body; the body
    occludes the centre so only the wrapping ring reads.
...
def ember_chip(surf, cx, cy, text, h, affordable=True):
    ...
    # WARMTH FIRST: the corona is an alpha-composite aura (NOT additive) drawn
    # BEFORE the body so the near-black pill occludes its centre and only the
    # ring bleeds past the edge — wrapping the silhouette evenly on all four
    # sides. Radius h*1.2 seats the ring just outside the pill. Killed entirely
    # when locked so absence-of-warmth is the can't-afford signal.
    if affordable:
        sc._alpha_aura(surf, cx, cy, int(h * 1.2), HALO_COL, peak=55, layers=12)

    rim_bright = EMBER_RIM_WARM if affordable else EMBER_RIM_COOL
    # gloss 8 (was 20): a whisper of a sweep only — the obsidian body must stay
    # genuinely dark so the halo ring + crown bevel are the sole warm signal.
...
def render_chip_tile(text, affordable):
    ...
    (alpha 255) so the alpha-composite halo wraps the pill exactly as it will on
    the live dark card body."""
    h = sc.m(20)
    coin_d = int(h * 0.66)
    pad = sc.m(13)
    gapc = sc.m(8)
...
def main():
    ...
    # 4x zoom of the in-game (1x) chip so the halo wrap + crown edge + cream
    ...
    t = title_f.render(f"v5 ember-crown r2 — {SID} EPIC — warmth = scarcity "
                       "(Levers C + P)", True, (246, 244, 232))
    canvas.blit(t, (MARGIN, MARGIN + 4))
    s = sub_f.render("symmetric alpha-aura halo wraps all 4 sides  ·  gloss 8 keeps "
                     "obsidian dark  ·  cream numerals  ·  locked kills the halo",
                     True, (150, 150, 176))
    ...
    y += strip_h + 4
    z = lbl_f.render("4x ZOOM — symmetric halo wrap + crown edge + cream keyline",
                     True, (176, 176, 200))
