"""Round-2 render for tl9 pins2: chain (proper interlock via draw order)"""
import os
...
def _attachment(card, affordable):
    link = (172, 135, 46) if affordable else (140, 144, 156)
    kl = (70, 50, 16) if affordable else (30, 32, 40)
    # rivet anchor — 1px gap above top link (separate mounting point)
    pygame.draw.circle(card, kl, (12, 4), 3)
    pygame.draw.circle(card, link, (12, 4), 2)
    # link geometry: 5px overlap creates visible interlock
    link1 = pygame.Rect(6, 6, 12, 8)    # upper link y:6..14
    link2 = pygame.Rect(9, 11, 12, 8)   # lower link y:11..19, shifted right 3px
    # draw lower link first, then upper link on top:
    # upper link's stroke appears in front at overlap → reads as upper threading through lower
    pygame.draw.ellipse(card, kl, link2.inflate(2, 2), 2)
    pygame.draw.ellipse(card, link, link2, 2)
    pygame.draw.ellipse(card, kl, link1.inflate(2, 2), 2)
    pygame.draw.ellipse(card, link, link1, 2)
