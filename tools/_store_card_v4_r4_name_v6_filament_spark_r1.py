...
def _name_filament_spark(big, name, cx, cy, max_w):
    ...
    # PASS 2: Spark stipple — confined to each glyph's ink columns, on stroke
    # contour. Sparks only land on edge pixels of the ink and never on the inter-
    # glyph gap columns (each glyph is walked in its own tile), so the dark gaps
    # between letters stay clean while the hot strokes shed glowing specks.
    ...
    x = x0
    for char_idx, (char, adv) in enumerate(zip(name, advances)):
        tile = _stamp_bold(_glyph_base(char, f, 0), m(0.8))
        tw, th = tile.get_size()
        gx = x + adv // 2 - tw // 2
        gy = cy - th // 2

        # Build contour band: pixels in dilated mask but NOT in the glyph ink.
        # Walk the glyph alpha to find stroke edge pixels.
        placed = []
        for py in range(th):
            for px in range(tw):
                # Only sample within the ink's x-span (gx..gx+tw) to stay in this glyph's column
                alpha_here = tile.get_at((px, py)).a
                if alpha_here < 30:
                    continue  # not ink
                ...
                # Candidate spark position
                sx = gx + px
                sy = gy + py
                # Enforce minimum spacing m(2) from already-placed sparks
                too_close = any(abs(sx - px2) + abs(sy - py2) < m(2) for px2, py2 in placed)
                if too_close:
                    continue
                if rng.random() > 0.35:  # sparse — only ~35% of edge pixels become sparks
                    continue
                placed.append((sx, sy))
                # 1px solid warm-white dot (floor size)
                spark_surf.set_at((sx, sy), (255, 255, 252, 255))
                # Optional single ivory neighbor for the brightest ~20%
                if rng.random() < 0.2:
                    nb_dx, nb_dy = rng.choice([(-1,0),(1,0),(0,-1),(0,1)])
                    spark_surf.set_at((sx + nb_dx, sy + nb_dy), (250, 244, 225, 180))

        x += adv

    big.blit(spark_surf, (0, 0))
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v6 — filament-spark — round 1", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v6/filament-spark/round_1.png"
