"""Round-2 render sheet for the `price-scroll` store-card price badge.
...
Locked state stays legible WITHOUT flipping the body: the vellum drops to a
distinctly darker, cooler grey (so the sealed scroll reads inert from its BODY,
not just its numerals), the rolls and wax darken, and the numerals dim — the
scroll is plainly "sealed/inert" while its shape and dimensions are unchanged.

Round-2 revisions off the art-director critique: locked vellum pushed much
darker/cooler for a real body-tone gap; the panel widened and the price
abbreviated (e.g. 1500 -> "1.5k") so numerals breathe; the wax seal re-seated
onto the BOTTOM ROLL so it leaves the numeral axis entirely; thicker cast
roll-shadows + a deeper roll ramp so the turned cylinders survive smoothscale;
and a stronger numeral stamp (darker under-stamp + a 1px keyline ring) so the
gold price seats crisply on the cream vellum.
...
# Warm parchment body when affordable; a distinctly darker, cooler grey-blue
# parchment when sealed so the locked scroll reads inert from its BODY tone
# alone — the numerals no longer have to carry the whole "sealed" read.
VELLUM_TOP,  VELLUM_BOT  = (232, 214, 176), (210, 188, 146)
VELLUM_GREY_TOP, VELLUM_GREY_BOT = (180, 168, 152), (154, 144, 130)
ROLL_TOP,    ROLL_BOT    = (196, 172, 130), (148, 122, 78)
ROLL_LOCK_TOP, ROLL_LOCK_BOT = (150, 142, 128), (132, 122, 106)
...
def _roll_shadow_band(w, h):
    """A short vertical fade (alpha 100 -> 0) used as the shadow the turned roll
    casts onto the vellum. Cast, not a curl hairline — this is what sells the
    roll as a rounded edge lifting off the panel; the deeper alpha keeps the
    band alive after the 2x -> 1x smoothscale."""
    band = pygame.Surface((w, h), pygame.SRCALPHA)
    for y in range(h):
        a = int(100 * (1 - y / max(1, h)))
...
# Vellum panel inner width the price must clear (44px panel less a margin each
# side). font(8.5) overflows a long price, so the size auto-shrinks to fit — a
# tiny scroll can't carry big numerals even after abbreviation.
_NUM_MAX_W = sc.m(19)


def _fit_price_mask(text):
    """The glyph mask, auto-shrunk from font(8.5) until the price clears the
    vellum panel. Returns the mask."""
    sz = 8.5
    while sz > 5.0:
...
def _numerals_affordable(surf, text, cx, cy):
    ...
    with a dark under-stamp AND a 1px keyline ring behind it: gold-on-cream is
    low contrast, so the dark ring + stamp give the numerals a struck edge that
    seats them firmly on the vellum. The gold face stays on top, so the numeral
    still reads warm and light."""
    ...
    # 1px dark keyline ring so the gold face reads against the cream vellum from
    # every side rather than only where the single-offset stamp falls
    ring = mask.copy()
    ring.fill((40, 22, 8, 180), special_flags=pygame.BLEND_RGBA_MULT)
    for dx, dy in ((-1, -1), (1, -1), (-1, 1), (1, 1)):
        surf.blit(ring, (r.x + dx, r.y + dy))
...
def _draw_scroll(surf, bx, by, text, affordable):
    ...
    vellum + a top-left highlight arc) + wax seal (on the bottom roll) +
    numerals."""
    ...
    pan = pygame.Rect(bx - 22, by - 14, 44, 28)          # x28..72  y36..64
    top_roll = pygame.Rect(bx - 23, by - 19, 46, 9)      # x27..73  y31..40
    bot_roll = pygame.Rect(bx - 23, by + 10, 46, 9)      # x27..73  y60..69
    ...
    # 4) numerals — centred on the panel; the wax now sits below on the roll
    if affordable:
        _numerals_affordable(surf, text, bx, by - 3)
    else:
        _numerals_locked(surf, text, bx, by - 3)

    # 5) wax seal (oxblood) — the accent, in place of a coin glyph. Seated on the
    # BOTTOM ROLL rather than the vellum so it leaves the numeral axis clear.
    d = sc.m(6)
    ...
    pygame.draw.circle(disc, (255, 210, 200, pip_a),
                       (int(d * 0.34), int(d * 0.32)), max(1, sc.m(0.9)))
    # dark keyline so the disc seats on the roll
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    return pygame.Rect(badge_cx - sc.m(23), badge_cy - sc.m(19),
                       sc.m(46), sc.m(38))
...
def verify():
    ...
    assert sl < sa - 100, (
        f"locked badge should read distinctly darker than affordable: "
        f"{sl} vs {sa}")
...
def main():
    ...
    hf = hud_font(26, True)
    lf = hud_font(13)
    canvas.blit(hf.render("store price — top-left badge · price-scroll · round 2",
