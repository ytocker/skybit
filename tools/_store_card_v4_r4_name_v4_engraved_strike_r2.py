...
def _name_engraved_strike(big, name, cx, cy, max_w):
    # Per-glyph CNC incision: the letter is carved INTO the navy band, lit from
    # top-left. Read comes purely from rim contrast — a dark shadowed cut wall on
    # the top-left, a warm ivory+gold catch-light on the bottom-right lit wall,
    # and a recess floor that kills the interior back to the band. No fill.
    sz = 13.5
    f = font(sz)
    ...
    for char, adv in zip(name, advances):
        glyph_cx = x + adv // 2
        tile = _glyph_base(char, f, 0)
        tw, th = tile.get_size()
        dst_x = glyph_cx - tw // 2
        dst_y = cy - th // 2

        # Z-order is the whole trick: halos go DOWN first, the floor lands LAST
        # so it re-buries the interior and only the 1px offset rims survive.

        # 1. Shadow wall at top-left (-1,-1): near-black lip on the wall that
        #    recedes away from the light source. Genuine dark, no gray.
        shadow = tile.copy()
        shadow.fill((8, 7, 16, 200), special_flags=pygame.BLEND_RGBA_MULT)
        big.blit(shadow, (dst_x - 1, dst_y - 1))
        ...
        # 4. Recess floor at (0,0), LAST: a near-black stencil that carves the
        #    interior back to the band, leaving only the offset rims. Held just
...
def render_card(sid):
    ...
    _name_engraved_strike(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26))
    return big
...
hfont = _font(20, True)
ffont = _font(18, True)
htxt = hfont.render("store_card_v4_r4_name_v4 — engraved-strike — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v4/engraved-strike/round_2.png"
