...
Uses the split-cream (design B) price chip from r3 round_2 as the base card.
Patches sc._name_on per panel so each variant shows only the name change on
top of the exact chip that won the design review.
...
# ── split-cream chip (copied verbatim from _store_card_v5_price_chip_r3_split_cream_r2) ──
...
def draw_card_split_cream(surf, sid, rect, affordable=True):
    """Full card draw using the split-cream price chip, calling sc._name_on so
    monkey-patching it changes only the name rendering."""
    m   = sc.m
    ...
    sc._ribbon_lozenge(surf, tier_word, cx, rect.y + m(55) - sc._RIBN_DY,
                       rect.w - m(34), pal)
    # name is drawn via sc._name_on so monkey-patching it controls this call
...
def make_name_on(sz_logical, dy_logical):
    """Closure: replace _name_on with given logical font-size and y-shift (up = positive)."""
...
os.makedirs(out.rsplit("/", 1)[0], exist_ok=True)
