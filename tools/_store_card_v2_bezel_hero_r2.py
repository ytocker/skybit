"""bezel-hero — store_card_v2 concept, round 2 headless render.

Maximum skin, minimum chrome: a large medallion (R=35) floating on a visible
indigo body, where rarity is carried almost entirely by a tier HALO that bleeds
into the ~40px side gutters — plus a warm tier tint inside the disc glass and a
single gold price chip. No name band (the grid row supplies the label), no
micro-ring stack on the bezel.

Round-2 director fixes over round 1:
  1. the tier glow now REACHES the side gutters. The under-disc additive
     soft_glow (peak 70) was swallowed at the disc edge, leaving every tier's
     gutter byte-identical (19,21,54). Replaced with a gutter-only feathered
     aura (normal alpha carry, not additive) that seats at the disc rim and
     feathers ~28px out — half the gutter — so the tier colour is measurable
     15px past the disc edge.
  2. the EPIC/LEGENDARY hero no longer blows to pure white. The under-disc
     additive glow is GONE (it was stacking with the glass sheen on the light
     prism/kitsune skins), and a colour tier veil now sits over the skin so the
     centre reads as tier hue, never (255,255,255).
  3. name band DROPPED entirely (the "minimum chrome" read the director asked
     to try first): disc + halo + gold chip only.
  4. disc edge simplified to ONE tier bezel ring at R+2 — the second outer ring
     and the fixed white highlight ring are gone.
  5. disc body tinted per tier so the glass warms away from the cool indigo body
     even before the gutter halo lands.
  6. EPIC glow pushed to magenta-violet (140,40,230) to separate from RARE blue.
...
writes docs/store_card_v2/bezel-hero/round_2.png.
"""
import os
...
def render_bezel_hero(sid, pal, price):
    """Draw ONE bezel-hero card onto a fresh SS panel (324×200) and return it.
    ...
    # 6. the domed glass well → hero skin (NO under-disc additive glow, which was
    #    stacking with the glass sheen and blowing the light skins to white).
    ...
    # 9. ONE tier-coloured bezel ring at R+2 (the whole edge treatment now).
    pygame.draw.circle(big, (*pal["gem"], 100), (cx, cy), m(R) + m(2), width=m(2))

    # 10. THE tier read: a gutter-only feathered halo that floods the ~40px side
    #     gutters with tier colour. Drawn AFTER the disc so nothing covers it.
    _gutter_aura(big, cx, cy, m(R), m(R + 28), pal["glow"], peak=90, layers=18)

    # 11. name band DROPPED — minimum chrome; the grid row supplies the label.

    # 12. price — the canonical GOLD RAMP-A chip, bottom kissing the body edge.
...
ffont = _font(20, True)
sfont = _font(16, True)
htxt = hfont.render("store_card_v2 — bezel-hero — round 2", True, (236, 232, 214))
...
out = "/home/user/skybit/docs/store_card_v2/bezel-hero/round_2.png"
os.makedirs(os.path.dirname(out), exist_ok=True)
pygame.image.save(sheet, out)
print("saved", out, sheet.get_size())

# gutter + hero sanity: sample a gutter point 15px right of the disc edge (must
# be clearly tier-tinted, NOT the round-1 (19,21,54)), and the disc centre (must
# NOT be pure white on EPIC/LEGENDARY).
for (tier, sid, pal, price), panel, (cx, cy) in zip(VARIANTS, panels, centers):
    ...
    print(f"{tier:9s} gutter+15px {gutter_px}   disc-centre {center_px}")
