...
OUT_PATH = pathlib.Path(__file__).parent.parent / "docs" / "dead_pip_v3" / "round_10_options.png"
...
def build_c_sunglasses_on(angle):
    ...
    lens. Mouth shut so the cumulative noise (wing + chips + brow + X)
    calms. Round 10: greenish toxic glow + poison ring removed per user —
    bruise palette + withered wing + stink lines carry C alone."""
    out = _new_parrot_surface()
...
def build_c_sunglasses_off(angle):
    ...
    + stink lines. Round 10: greenish toxic glow + poison ring removed
    per user."""
    out = _new_parrot_surface()
    P = _withered_palette(P_BRUISE)
...
def render():
    ...
    title_str = "DEAD PIP v3  ·  round 10 — C: greenish head-aura removed"
    ...
    panels = [
        ...
        ("C · SUNGLASSES ON",  "decay — bold X + wither + stink (mouth shut)",         build_c_sunglasses_on,  "C", False),
        ("C · SHADES OFF",     "decay — X-eyes + tongue + stink + shades",            build_c_sunglasses_off, "C", True),
    ]
