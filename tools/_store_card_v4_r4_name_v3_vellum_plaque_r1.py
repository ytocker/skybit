...
# Fixed vellum palette — deliberately tier-agnostic so the plaque reads as
# aged paper, never as a colour-coded rarity chip.
IVORY = (250, 244, 225)
...
def _name_vellum_plaque(big, txt, cx, cy, maxw, rect):
    ...
    # Ivory crown fading to champagne foot — warm paper falling into shadow.
    plq = vgrad_stops(pw, ph, prad, [(0.0, IVORY), (1.0, CHAMPAGNE)], 255)

    # Mottled fibre: low-alpha cream/pewter blotches so the fill never reads flat
    # or machined. Seeded so the grain is stable across builds.
    rnd = random.Random(0x5EED)
    for _ in range(46):
        bx, by = rnd.randint(0, pw - 1), rnd.randint(0, ph - 1)
        br = rnd.randint(m(2), m(6))
        col = CREAM if rnd.random() < 0.5 else (198, 188, 162)
        a = rnd.randint(15, 25)
        blob = pygame.Surface((br * 2 + 2, br * 2 + 2), pygame.SRCALPHA)
        pygame.draw.circle(blob, (*col, a), (br + 1, br + 1), br)
        plq.blit(blob, (bx - br, by - br))

    # Inner luminosity baked into the paper (not the band) so it glows from
    # within rather than casting light onto the surrounding groove.
    soft_glow(plq, pw // 2, ph // 2, m(11), (255, 250, 232), 24, layers=8)

    # Clip everything to the rounded plaque silhouette.
    ...
    # Deckled edge: short jittered ticks along top + bottom so the border wobbles
    # like torn paper instead of a clean machined rect.
    ...
    for ex in range(prect.left + prad, prect.right - prad, step):
        jt = ernd.randint(-m(1), 0)
        pygame.draw.line(deck, (*IVORY, 150), (ex, prect.top + jt), (ex + step - 1, prect.top + jt), max(1, m(0.8)))
        jb = ernd.randint(0, m(1))
        pygame.draw.line(deck, (*PEWTER, 150), (ex, prect.bottom - 1 + jb), (ex + step - 1, prect.bottom - 1 + jb), max(1, m(0.8)))
    big.blit(deck, (0, 0))

    # Seat the plaque in the band: bright ivory lit rim, pewter undercut.
    bevel_rim(big, prect, prad, PEWTER, (*IVORY, 235), w=max(1, m(1.0)))

    # Size the type to fit the plaque interior.
    size = 10.5
    while size > 6:
        f = font(size)
    ...
    # Debossed ink: a light lower-right bevel first (the pressed groove catching
    # light), then near-black ink stamped on top. No glow — ink doesn't bloom.
    off = max(1, m(0.7))
    plain_text(big, txt, f, (cx + off, cy + off), (140, 132, 110), shadow_a=0, weight=m(0.7))
    plain_text(big, txt, f, (cx, cy), INK, shadow_a=0, weight=m(0.7))
...
def render_card(sid):
    ...
    _name_vellum_plaque(big, name.upper(), rect.centerx, rect.y + m(81), rect.w - m(26), rect)
    return big
...
htxt = hfont.render("store_card_v4_r4_name_v3 — vellum-plaque — round 1", True, (236,232,214))
...
out = "/home/user/skybit/docs/store_card_v4_r4_name_v3/vellum-plaque/round_1.png"
