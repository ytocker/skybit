"""Render docs/dead_pip_v3/round_6_options.png — round-6 sheet.

Round 5 shipped SHIP-READY with the user. The single follow-up note,
verbatim:

    "For the sunglasses on version put X on the lens. Make them large
     and bold and match colors of designs."

Round 6 is a surgical edit on top of round 5:

 - Sunglasses-ON variants drop the inside-lens Y-shape crack-fork
   entirely.
 - Each ON lens gets a LARGE, BOLD palette-keyed X stamped on top of
   the lens body: two stroke-3 diagonals reaching ±5 px from the lens
   centre (1 px past the rim on every side so the X visibly breaks the
   silhouette), plus a 1 px highlight slash along the upper-left edge
   of each stroke for cartoon "wet pop".
 - Palette-matched colours, no new constants:
     B-ON core = P_CHARTREUSE['head_crown']  = (225, 235, 130)
     B-ON glint = (255, 245, 200)
     C-ON core = P_BRUISE['lens_frame']      = (95, 60, 115)
     C-ON glint = (165, 130, 185)

Everything else — the E baseline strip with the SHIPPED ribbon, B-OFF
and C-OFF panels (bold X-eyes + tongue + fallen aviators), ON-variant
brow + chips, C-row aura ring + stink lines + withered wing, panel
chrome, column tint + divider, all asserts — is preserved verbatim
from round 5.
"""
import math
import os
...
OUT_PATH = pathlib.Path(__file__).parent.parent / "docs" / "dead_pip_v3" / "round_6_options.png"
...
def _draw_bold_x_on_lens(body, palette_key):
    ...
    Two stroke-3 diagonals reaching ±5 px from the lens centre (lens disc
    radius is 6 → the X arms extend ~1 px past the rim on every side, so
    the X visibly BREAKS the lens silhouette — that breakage sells the
    "stamped over the lens" read instead of "highlight inside the lens").
    ...
    if palette_key == "B":
        ...
        # P_BRUISE['lens_frame'] = (95, 60, 115) — C's signature dark
        # eggplant; (165, 130, 185) is a lighter eggplant for the
        # upper-left sheen.
        core    = (95, 60, 115)
        highlight = (165, 130, 185)

    for cx, cy in (LENS_L, LENS_R):
        # Two stroke-3 diagonals, ±5 reach.
        pygame.draw.line(body, core, (cx - 5, cy - 5), (cx + 5, cy + 5), 3)
        pygame.draw.line(body, core, (cx - 5, cy + 5), (cx + 5, cy - 5), 3)
...
def _expected_bold_x_core(palette_key):
    """Bold-X core stroke colour on the ON-lens for the new round-6 check."""
    return (225, 235, 130) if palette_key == "B" else (95, 60, 115)
...
def render():
    ...
    title_str = "DEAD PIP v3  ·  round 6 — sunglasses-ON: bold palette X on lens"
    ...
    # ── NEW round-6 asserts: bold-X core lands on each ON-variant lens ───
