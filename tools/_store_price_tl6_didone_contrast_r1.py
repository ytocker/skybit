"""Round-1 render for tl6 numeral concept: didone-contrast.

Simulates Didone thick/thin stroke modulation by compositing three
horizontal offsets of the glyph mask to fatten vertical strokes while
horizontal strokes stay at base weight. Charcoal-brown ink gives the
high contrast feel of classic letterpress without any gradient.
...
def _draw_price(face, text, center, affordable):
    """Composite three horizontal shifts of the glyph mask to fatten
    vertical strokes while horizontal strokes stay at base weight —
    simulating Didone thick/thin modulation without a real serif font.
    Flat charcoal ink; no gradient, no keyline."""
    ink = (52, 38, 30) if affordable else (98, 102, 114)
    for fs in (13, 12, 11, 10):
        base_mask = sc._glyph_base(text, sc.font(fs), 0)
        if base_mask.get_width() <= 58:
            break
    w, h = base_mask.get_size()
    fat = pygame.Surface((w, h), pygame.SRCALPHA)
    for ox in (-sc.m(1), 0, sc.m(1)):
        shifted = pygame.Surface((w, h), pygame.SRCALPHA)
        shifted.blit(base_mask, (ox, 0))
        fat.blit(shifted, (0, 0))
    fill = fat.copy()
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("DIDONE-CONTRAST price tag — round 1", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/didone_contrast/round_1.png"
