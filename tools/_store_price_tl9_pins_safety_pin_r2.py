"""Round-2 render for tl9 pins concept: safety_pin.

The r1 pin dissolved: drawn inside the 2x super-sample buffer, the 3.7px shaft
crushed to ~1.5 final px, the coil smudged, and the clasp fused with the numeral
ink. The metal has to survive at output resolution, not upstream of the
smoothscale that blurs everything hairline-thin. So the tag body + digits still
render in the SS buffer (where the glyph anti-aliasing wants the extra samples),
but the pin is drawn onto the FINAL 162x100 card, post-smoothscale, at whole
final pixels: a 3px dark/silver/dark shaft, a 7px two-tone coil that reads as a
ring via its cream hole, and a flat clasp cap wider than the shaft. The pin
fastens only the tag's top-left corner tip and stays clear above the numeral so
neither mark eats the other. Review sheet only — not wired live.
"""
import os
...
def my_price_chip(surf, cx, cy, text, h, variant=sc.PRICE_VARIANT, **kw):
    ...
    # Attachment is drawn post-smoothscale on the final card, not here.


sc.price_chip = my_price_chip


def _safety_pin(card, affordable):
    # Whole-pixel metal on the FINAL 162x100 card so nothing survives as a
    # sub-pixel hairline after the 2x->1x crush the r1 pin died to.
    if affordable:
        dark = (46, 56, 70)
        silver = (195, 205, 216)
        hole = (235, 225, 200)
    else:
        # Locked steel runs a touch warmer/greyer so the state reads different
        # without changing the pin's construction.
        dark = (55, 58, 70)
        silver = (180, 185, 195)
        hole = (220, 218, 215)
    ...
    # Shaft: dark-1 / silver-1 / dark-1 cross-section from a fat dark base under
    # a thin silver centre — the keyline keeps the metal off the cream face.
    pygame.draw.line(card, dark, coil, clasp, 3)
    pygame.draw.line(card, silver, coil, clasp, 1)

    # Coil: cream hole punches the dark disc into a ring rather than a blob.
    pygame.draw.circle(card, dark, coil, 4)
    pygame.draw.circle(card, hole, coil, 2)

    # Clasp: flat cap wider than the shaft, with a single bright top-edge dot.
    pygame.draw.circle(card, dark, clasp, 3)
    pygame.draw.circle(card, silver, (clasp[0], clasp[1] - 1), 1)
...
print(f"dark pixel at {dark_hit[0]}: {dark_hit[1]}")

pa, pl = _va.get_at((20, 30))[:3], _vl.get_at((20, 30))[:3]
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("tl9 pins: safety_pin — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl9_pins/safety_pin/round_2.png"
