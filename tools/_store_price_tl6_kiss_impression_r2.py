"""Round-2 render for the `kiss-impression` tl6 numeral treatment.

Letterpress kiss-impression: warm charcoal-brown flat ink (56,40,28) fills the
affordable numeral — no gradient, no keyline ring. The locked ink is pushed
dark/cool so it clears the WCAG ~3.5:1 legibility floor against the unchanged
grey body (r1's (98,102,114) sat near ~1.45:1 and dissolved). The hairline
rule is committed as a centered inset register mark — narrower than the
numeral so it reads as a deliberate tick, not a stray underline at 1x.
"""
import os
...
def _draw_price(face, text, center, affordable):
    # Warm flat ink when affordable; a dark cool ink when locked so the numeral
    # clears the legibility floor against grey without touching the body ramp.
    ink = (56, 40, 28) if affordable else (34, 38, 52)
    ...
    face.blit(fill, r)
    # Inset centered register mark: narrower than the glyph so it survives the
    # 1x downscale as an intentional tick rather than a full-width underline.
    rule_h = max(2, sc.m(1))
    rule_w = max(int(r.width * 0.5), r.width - 2 * sc.m(4))
    rule_x = r.centerx - rule_w // 2
    rule_y = r.bottom + max(2, sc.m(1))
    pygame.draw.rect(face, (*ink, 180), (rule_x, rule_y, rule_w, rule_h))
...
# Sample the darkest locked-numeral pixel against a nearby body pixel to confirm
# the ink clears the ~3.5:1 legibility floor the art-director flagged in r1.
_ink_px, _body_px = None, None
for _px in range(14, 42):
    for _py in range(22, 44):
        c = _vl.get_at((_px, _py))[:3]
        if _ink_px is None or sum(c) < sum(_ink_px):
            _ink_px = c
_body_px = _vl.get_at((10, 30))[:3]
_l1, _l2 = _rel_lum(_body_px), _rel_lum(_ink_px)
_ratio = (max(_l1, _l2) + 0.05) / (min(_l1, _l2) + 0.05)
print(f"locked contrast body{_body_px} vs ink{_ink_px} = {_ratio:.2f}:1")
...
fh = hud_font(22)
fl = hud_font(13)
sheet.blit(fh.render("KISS-IMPRESSION price tag — round 2", True, (240, 224, 180)),
           (PAD, PAD // 2))
...
for crop, lbl in crops:
    ...
    x += crop_w * zoom + GAP
out = "docs/store_price_tl6/kiss_impression/round_2.png"
