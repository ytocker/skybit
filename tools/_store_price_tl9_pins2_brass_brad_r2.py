"""Round-2 render for tl9 pins2: brass_brad (domed head + splayed prongs, fixed locked contrast)"""
import os
...
def _attachment(card, affordable):
    import math
    # locked steel darkened to (96,102,116) — L≈38% vs grey tag L≈58% = visible contrast
    brass = (182, 142, 46) if affordable else (96, 102, 116)
    hi = (214, 180, 96) if affordable else (140, 146, 160)
    kl = (70, 50, 16) if affordable else (30, 32, 40)
    # prongs splay from dome base (y=17) to tips (y=24)
    pygame.draw.line(card, kl, (16, 17), (10, 24), 3)
    pygame.draw.line(card, kl, (16, 17), (22, 24), 3)
    pygame.draw.line(card, brass, (16, 17), (10, 24), 2)
    pygame.draw.line(card, brass, (16, 17), (22, 24), 2)
    # bright tips at true line endpoints
    pygame.draw.rect(card, brass, (8, 23, 4, 3))
    pygame.draw.rect(card, brass, (20, 23, 4, 3))
    # dome: keyline r=7, fill r=6
    pygame.draw.circle(card, kl, (16, 12), 7)
    pygame.draw.circle(card, brass, (16, 12), 6)
    # inner highlight spot
    pygame.draw.circle(card, hi, (14, 10), 2)
    # specular arc top-left inner rim (~100°-170°)
    arc_rect = pygame.Rect(10, 6, 12, 12)
    pygame.draw.arc(card, hi, arc_rect, math.radians(100), math.radians(170), 1)
