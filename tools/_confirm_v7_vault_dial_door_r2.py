#!/usr/bin/env python3
"""
vault-dial-door  ·  confirm_purchase_v7  ·  round 2

Round 2 addresses all art-director notes:
  - Central coin_glyph removed; dark dial face is now the sole background so
    the gold price reads against a clean dark ground (no gold-on-gold)
  - Price font bumped to m(14), centred at the new dial cy so the numeral is
    the unambiguous focal point inside the rings
  - Dial shifted down to cy=235 (was 227) so the dial top clears the disc
    glow field without a visual collision
  - BUY lever now uses a genuine gold gradient + top_sheen peak=55 so it reads
    as the primary CTA; CANCEL stays dark
  - Tick studs pruned to 4 cardinal positions (12/3/6/9 o'clock) so clutter
    inside the rings does not compete with the price
"""
import os
import sys
import math
...
# must live in the RGB channels of the sweep surface — otherwise a near-black
# button body blows straight to white.
...
# ── the dial (combination dial — price as the centrepiece) ────────────────────
def vault_dial(base, price, pal):
    """The price seated inside concentric gold rings with 4 cardinal gem tick
    studs. Removing the coin_glyph lets the dark face carry the price numeral
    as a single uncontested focal point; the rings + studs frame it without
    competing. Dial shifted to cy=235 so the top of the outer ring clears the
    disc glow field cleanly."""
    cx, cy = m(CX), m(235)          # cy nudged from 227 → 235 for disc clearance
    ...
    pygame.draw.circle(base, (12, 14, 36), (cx, cy), m(36))                # dark dial face

    # 3) 4 cardinal gem tick studs (12/3/6/9 o'clock) — pruned from 12 so the
    #    price has breathing room and nothing inside the rings competes with it
    for i in [0, 3, 6, 9]:
        a = math.radians(-90 + 360 * i / 12)
        cx_s = int(cx + m(41) * math.cos(a))
        cy_s = int(cy + m(41) * math.sin(a))
        sc.facet_gem(base, cx_s, cy_s, m(3), pal["gem"], pal["deep"])

    # 4) gold price numeral struck on the dark dial face — now the dial's sole
    #    centrepiece with no coin glyph underneath competing for space
    plain_text(base, price, font(14), (m(CX), m(235)), CARD_RING_BRIGHT,
...
def lever_button(base, cx, cy, label, stops, bevel_col, bevel_a=255,
    ...
    bevel, one gloss sweep, and a cream label. Gold BUY vs. dark CANCEL reads
    as commit vs. dismiss at a glance. Optional top_sheen on the BUY lever
    to reinforce its polished gold surface."""
    w, h = m(106), m(44)
    rad = m(22)
    ...
    if sheen_peak > 0:
        # top_sheen adds a corona of specular gloss to the crown of the lever —
        # reinforces that the gold body is polished rather than matte
        sc.top_sheen(base, r, rad, m(14), peak=sheen_peak)
...
def render_popup(tier_word, sid, price, name, pal):
    ...
    # the dial (combination lock as the price treatment — dial at cy=235)
    ...
    # action levers — BUY in genuine gold to signal the primary CTA;
    # CANCEL stays dark so the pair reads commit vs. dismiss without ambiguity
    lever_button(big, m(70), m(392), "BUY",
                 [(0, (155, 115, 32)), (0.5, (95, 68, 16)), (1, (55, 38, 8))],
                 CARD_RING_BRIGHT, sheen_peak=55)
...
title = _font(34, True).render(
    "confirm_purchase_v7  ·  vault-dial-door  ·  round 2", True, (232, 226, 208))
txt.blit(title, (MARGIN, 30))
sub = _font(20, True).render(
    "dark dial face · gold price focal point · 4 cardinal studs · gold BUY lever",
    True, (150, 156, 178))
...
OUT = "/home/user/skybit/docs/confirm_purchase_v7/vault-dial-door/round_2.png"
