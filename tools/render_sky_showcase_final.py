"""2-row sky showcase: ORIGINAL + vA Vivid (final fix).

Output: docs/sky_transition/showcase_final.png

    python tools/render_sky_showcase_final.py
"""
import os
import sys
...
CONCEPTS = [
    ("ORIGINAL",       None),
    ("Final Fix",      "vA_vivid/round_1.png"),
]
...
def main():
    ...
    mt = f_main.render(
        "Alpine Haze — Final Fix: Vivid Rose Base  (Phases 0.42 / 0.47 / 0.52)",
        True, TEXT_HI)
    ...
    out = os.path.join(_ROOT, "docs", "sky_transition", "showcase_final.png")
    ...
    for i, (name, _) in enumerate(CONCEPTS):
        ...
        print(f"  {name:15s} panel4 mid: {px}")
