"""
Skybit audio: plays curated CC0 sample files (Kenney.nl) on both backends.

* Native (desktop / pygbag main thread):
    pygame.mixer.Sound(file=...) loads the OGGs once at init() time and
    plays them via mixer channels.

* Browser (pygbag / Pyodide / emscripten):
    Each play_X() call routes to JS window.skyPlay(name, volume), which
    is defined in inject_theme.py. The JS side fetches the same OGG files
    that inject_theme.py copies to build/web/sounds/ at build time, then
    plays them through Web Audio.

Both paths play the same source files, so what you hear locally and on
the deployed build is identical.
...
# ── Native backend (pygame.mixer + pre-recorded OGG files) ──────────────────
