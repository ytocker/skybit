"""Render docs/cloud_redesign/ruyi_soft_round_N.png.

6 rows × 5 phases. Row 0 = byte-identical round-23 baseline
(`cloud_variants.draw_cloud_ruyi`); rows 1–5 = the 5 soft-linework
variants from `ruyi_soft_variants`. Sky + V14 backdrop + ground per
cell via the shared `_scene_backdrop` from `render_cloud_variants`.
Int-hashed seed pattern (no tuples)."""
...
def make_sheet() -> pygame.Surface:
    ...
    title = font_head.render(
        "RUYI SOFT-LINEWORK — 5 base-faithful variants + baseline",
        True, (240, 240, 240))
    ...
    for r in range(n_rows):
        ...
        if r == 0:
            ...
            name = rsv.VARIANT_NAMES[r]
            src = rsv.VARIANT_SOURCES[r]
            idx_label = f"#{r}"
