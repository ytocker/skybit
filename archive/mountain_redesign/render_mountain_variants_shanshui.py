"""Render the round-3 shan-shui-derivative contact sheet.

Rows = current V4 reference on top + 5 new shan-shui descendants pushed for
presence (V11..V15). Cols = 5 times of day (day / sunrise / sunset / dusk /
night). Each tile is a FULL-SIZE 360×640 game-style composite. Output:
docs/mountain_redesign/_comparison_shanshui.png

Run from anywhere:
    python archive/mountain_redesign/render_mountain_variants_shanshui.py
"""
...
def render_scene(variant_id: int, phase: float) -> pygame.Surface:
    ...
    # Module-level phase carries extra palette keys for variants that need
    # foliage/horizon/accent — the drop-in signature can't pass them.
    mv.set_phase(phase)
...
def make_contact_sheet(images: dict) -> pygame.Surface:
    ...
    for r, vid in enumerate(rows):
        ...
        # V4 row gets a distinct tag colour so it reads as "reference".
        tag_color = (255, 180, 110) if vid == 4 else (255, 220, 130)
        tag = head.render(f"V{vid}", True, tag_color)
        ...
        # V4 reference row also gets a "REF" badge below the name.
        if vid == 4:
            ref = font.render("REFERENCE", True, (255, 200, 120))
            sheet.blit(ref, (8, ly + 26))
...
def main() -> None:
    images: dict = {}
    for vid in mv.ROW_ORDER_SHANSHUI:
    ...
    sheet_path = OUT / "_comparison_shanshui.png"
