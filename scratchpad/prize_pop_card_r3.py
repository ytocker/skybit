...
from game.store_cards import (
    ...
    vgrad_stops, gold_a_fill, plain_text, bevel_rim,
...
# intensity in the RGB channels so the price chip + BUY pill stay gold end-to-end.
# Kept as the module's RECT signature (surf, rect, radius, peak) so price_chip ->
# chip_body_stops resolves it unchanged.
...
# Patch the module global (chip_body_stops -> price_chip resolves it by name) AND
# the directly-imported name the BUY pill calls below. Both must happen before any
# gold fill is drawn.
...
path = os.path.join(outdir, "round_3.png")
pygame.image.save(out, path)

# ── verify AD constraints on the FINAL 1x render ─────────────────────────────
px = out
# BUY pill body must be GOLD (R>G>B), NOT a white slab — sample the pill centre
# on a row clear of the "BUY" glyph so we read the fill, not the text.
buy_center_y = buy_cy
r, g, b = px.get_at((180, buy_center_y))[:3]
gold_ok = (r > g > b) and (r, g, b) != (255, 255, 255)
print("BUY centre px (180,%d) RGB:" % buy_center_y, (r, g, b),
      "GOLD" if gold_ok else "FAIL")
# Also sample a few off-glyph rows to confirm the whole body reads gold.
for yy in (buy_cy - 18, buy_cy - 12, buy_cy + 12, buy_cy + 18):
    c = px.get_at((180, yy))[:3]
    print("  BUY body px (180,%d):" % yy, c, "R>G>B" if c[0] > c[1] > c[2] else "check")

cancel_cy_v = cancel_cy
def is_keyline(c):
    r, g, b = c[0], c[1], c[2]
    return 120 < r < 230 and 120 < g < 240 and 150 < b < 255 and abs(r - b) < 90
top_e = bot_e = None
for y in range(cancel_cy_v - 40, cancel_cy_v + 40):
...
print("price->BUY gap:", (buy_cy - buy_h // 2) - (price_cy + 13))
print("BUY->CANCEL gap:", (cancel_cy - cancel_h // 2) - (buy_cy + buy_h // 2))
print("saved", path, out.get_size(), os.path.getsize(path))
